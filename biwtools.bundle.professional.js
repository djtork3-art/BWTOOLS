...
.bwo-fab-menu-version::after {
  content: "v1.7.21 saldos reconstruidos";
  font-size: 10px;
}
...
bwoDiagKpi=function(e,t,n,r=""){return bwoDiagKpiVersionBase(e,e==="Versión"?"v1.7.21":t,n,r)};
...
globalThis.__BIWTOOLS_BALANCE_FIX__="v1.7.21-saldos-por-reparto-y-movimientos";
...
async function bwoBalanceBuildShadows(e,t,n,r){let o={};for(let l of r){let s=String(l.id),u=typeof l.joinDate==="number"?new Date(l.joinDate*1e3).toISOString():null,c=u!=null&&u>e,f=c?u:e,g=c?n:t,b=Number(bwoBalanceFixedDistributions[g]),p=Number(bwoBalanceTeamValueDistributions[g]);if(Number.isFinite(b)){o[s]={startDate:f,balance:b,squadSize:null,pricesApprox:0,unresolved:!1,distribution:g};continue}if(!Number.isFinite(p)){o[s]={startDate:f,balance:null,squadSize:null,pricesApprox:0,unresolved:!0,distribution:g};continue}let y;try{y=await qg(l.id)}catch{o[s]={startDate:f,balance:null,squadSize:null,pricesApprox:0,unresolved:!0,distribution:g};continue}let w=m=>m.player_id===s,m=(await z()).state.movements.filter(w),d=bwoBalanceSquadAt(y,m,f),v=await bwoBalanceMapLimit(d.squad,4,x=>bwoBalancePriceAt(x,f)),M=0,B=0,E=d.unresolved;for(let x of v)x?(M+=x.price,x.exact||B++):E=!0;o[s]={startDate:f,balance:p-M,teamValue:M,squadSize:d.squad.length,pricesApprox:B,unresolved:E||B>0,distribution:g}}let P=await z();P.state.bwoBalanceShadows=o,P.state.syncState.bwoBalanceShadowAnchor=e,P.state.syncState.bwoBalanceShadowDistribution=String(t??""),P.state.syncState.bwoBalanceShadowBuiltAt=String(Date.now()),await Cu();return o}

async function bwoBalancePrepareRebuild(){let e=await z(),t=e.state.syncState;if(t.bwoBalanceEngineV1721==="1"||t.bwoBalanceEngineV1721==="rebuilding")return;t.bwoBalanceEngineV1721="rebuilding",e.state.movements=[],e.movementIds.clear(),e.movementFingerprints.clear(),e.state.bwoBalanceShadows={},delete e.state.balanceCorrections;for(let n of new Set([...bwoBalanceEventTypes,"market","transfer","roundFinished","exchange","loan"]))t[`lastSeenDate:${n}`]="";t.seasonAnchorDate="",t.seasonStartingBalance="",t.seasonDistribution="",delete t.seasonStartingBalanceLockedV3,await Cu()}

var bwoOriginalZw=zw;
Ri=async function(){let{state:e}=await z(),{date:t,startingBalance:n}=await qo(),r=String(e.syncState.myManagerId??""),o=e.syncState.myBalance,a=o!=null&&o!==""?Number(o):null,i=e.bwoBalanceShadows??{},l=new Map;for(let u of Object.values(i))u?.squadSize!=null&&l.set(u.squadSize,(l.get(u.squadSize)??0)+1);let s=null,c=0;for(let[u,f]of l)f>c&&(c=f,s=u);return Object.values(e.players).filter(u=>!e.inactive[u.id]).map(u=>{let f=i[u.id],g=!!f&&f.balance!=null&&!f.unresolved&&!f.pricesApprox&&(f.squadSize==null||s==null||f.squadSize===s),b=f?.startDate??t,p=e.movements.filter(y=>y.player_id===u.id&&y.occurred_at>b).reduce((y,w)=>y+w.amount,0),m=u.id===r&&a!=null?a:(g?f.balance:n)+p,d=e.teamValue[u.id];return{id:u.id,name:u.name,icon:u.icon,team_value:d?.teamValue??null,squad_size:d?.squadSize??null,team_value_change_today:d?.changeToday??null,balance:Math.round(m),balance_exact:u.id===r&&a!=null||g}}).sort((u,f)=>f.balance-u.balance)};
zw=async function(){await bwoBalancePrepareRebuild();let e=await bwoOriginalZw(),t=await zg(),n=t?.data?.users??[],r=await Fg().catch(()=>({})),o=typeof t?.data?.created==="number"?new Date(t.data.created*1e3).toISOString():"1970-01-01T00:00:00.000Z",a=await bwoBalanceResolveAnchor(o),i=await Li({fresh:!0}),l=await rg(i.id,a.date),s=typeof r?.newSeason==="number"?r.newSeason:null,u=a.distribution??s,c=a.distribution==null&&s!=null,f=i.balance-l,g=bwoBalanceFixedDistributions[u],b=g!=null&&c&&Math.abs(g-f)>3e6,p=g!=null&&!b?g:f,h=b?null:u,y=await z(),w=y.state.syncState;w.seasonAnchorDate=a.date,w.seasonStartingBalance=String(Math.round(p)),w.seasonDistribution=u!=null?String(u):"",w.myManagerId=String(i.id),w.myBalance=String(Math.round(i.balance)),w.derivedStartingBalance=String(Math.round(f)),w.rejectedFlatCash=b?String(g):"",delete y.state.balanceCorrections;let m=y.state.bwoBalanceShadows??{},d=`${a.date}|${h??""}|${typeof r?.newUsers==="number"?r.newUsers:""}|${n.map(v=>v.id).join(",")}`,v=w.bwoBalanceShadowSignature!==d||n.some(M=>!m[String(M.id)])||w.bwoBalanceEngineV1721!=="1";return v&&(m=await bwoBalanceBuildShadows(a.date,h,typeof r?.newUsers==="number"?r.newUsers:null,n),w.bwoBalanceShadowSignature=d),w.bwoBalanceEngineV1721="1",await Cu(),e};
...
var bwoRuntimeToken=`v1.7.21-${Date.now()}-${Math.random().toString(36).slice(2)}`;
