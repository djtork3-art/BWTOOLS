...
.bwo-fab-menu-version::after {
  content: "v1.7.22 saldos oficiales reconciliados";
  font-size: 10px;
}
...
bwoDiagKpi=function(e,t,n,r=""){return bwoDiagKpiVersionBase(e,e==="Versión"?"v1.7.22":t,n,r)};
...
globalThis.__BIWTOOLS_BALANCE_FIX__="v1.7.22-shadow-starting-balances-oficial";
...
async function bwoBalancePriceAt(e,t){let n=String(e),r=bwoBalancePriceCache.get(n);if(!r){r=(async()=>{let o=await Mr(n).catch(()=>null);if(o?.length)return o.map(a=>({date:String(a.date).slice(0,10),price:Number(a.price)}));try{let a=await fetch(`https://cf.biwenger.com/api/v2/players/la-liga/${encodeURIComponent(n)}?lang=es&fields=prices`,{headers:{Accept:"application/json"}});if(!a.ok)return null;return((await a.json())?.data?.prices??[]).map(([i,l])=>{let s=String(i).padStart(6,"0");return{date:`20${s.slice(0,2)}-${s.slice(2,4)}-${s.slice(4,6)}`,price:Number(l)}})}catch{return null}})(),bwoBalancePriceCache.set(n,r)}let o=await r;if(!o?.length)return null;let a=String(t).slice(0,10),i=o.find(l=>l.date===a);if(i)return{price:i.price,exact:!0};let l=o.filter(s=>s.date<=a).sort((s,u)=>u.date.localeCompare(s.date))[0];return l?{price:l.price,exact:!1}:null}
async function bwoBalanceMapLimit(e,t,n){let r=new Array(e.length),o=0;async function a(){for(;;){let i=o++;if(i>=e.length)return;r[i]=await n(e[i],i)}}return await Promise.all(Array.from({length:Math.min(t,e.length||1)},a)),r}
...
var bwoRuntimeToken=`v1.7.22-${Date.now()}-${Math.random().toString(36).slice(2)}`;
