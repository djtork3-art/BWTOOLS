...
zw=async function(){let e=await z(),t=e.state.syncState,n=t.seasonAnchorDate??null,r=t.seasonStartingBalance??null,o=t.seasonStartingBalanceLockedV3==="1",a;try{a=await bwoOriginalZw()}catch(i){if(o&&n!=null&&r!=null){t.seasonAnchorDate=n,t.seasonStartingBalance=r,await Cu()}throw i}let l=await z(),s=l.state.syncState,u=await Li(),c=(await qo()).date,f=await rg(u.id,c);l.state.balanceCorrections??={};if(o&&n===c&&r!=null)s.seasonAnchorDate=n,s.seasonStartingBalance=r;else l.state.balanceCorrections={},s.seasonStartingBalanceLockedV3="1";let g=Number(s.seasonStartingBalance??0);return l.state.balanceCorrections[String(u.id)]=Math.round(u.balance-(g+f)),await Cu(),a};
...
var bwoBalanceCycleRunning=!1;
async function bwoRefreshBalances(){if(bwoBalanceCycleRunning)return;bwoBalanceCycleRunning=!0;try{Av(),Rv(),await Vg();let e=location.pathname,t=[];e.includes("/market")?(dv(),wv()):e.includes("/league")&&t.push(gv(),yv()),ov(),t.push(Cv(),_v(),kv(),Tv()),await Promise.allSettled(t)}catch(e){console.warn("[biwtools] No se pudo completar el refresco coordinado:",e)}finally{bwoBalanceCycleRunning=!1}}
