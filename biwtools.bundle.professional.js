...
.bwo-fab-menu-version::after {
  content: "v1.1.2 jornada integrada";
  font-size: 10px;
}
...
var bwoRoundNativeDataCache=null,bwoRoundNativeDataAt=0,bwoRoundNativeDataKey="",bwoRoundNativeBusy=!1;
...
function bwoRoundNativeCleanup(){document.querySelectorAll(".bwo-round-planner,.bwo-round-native-badge,.bwo-round-native-summary").forEach(e=>e.remove());document.querySelectorAll(".bwo-round-native-risk,.bwo-round-native-unavailable").forEach(e=>e.classList.remove("bwo-round-native-risk","bwo-round-native-unavailable"))}
...
function bwoRoundNativeTip(e,t){let n=[`Previsión ${e.projected==null?"sin datos":`${e.projected.toFixed(1)} pts`}`,e.average==null?null:`Media ${e.average.toFixed(1)} pts`,e.probability?.label,e.physical?.label,`${e.match?.home?"vs":"@"} ${e.match?.opponentName??"Rival por confirmar"}`,bwoRoundTime(e.match?.date),e.match?.stateLabel,t].filter(Boolean);return n.join(" · ")}
function bwoRoundNativeRender(e){bwoRoundNativeCleanup(),bwoRoundNativeEnsureStyles();let t=document.querySelector("main.container")??document.querySelector("main");if(!t)return;let n=new Map,r=new Map;for(let o of e.recommendations){o.out&&n.set(String(o.out.id),o),r.set(String(o.in.id),o)}for(let o of e.players){let a=n.get(String(o.id)),i=r.get(String(o.id)),l=o.physical.unavailable?"bad":o.risk?"warn":i?"recommended":"good",s=o.physical.unavailable?"!":o.risk?"⚠":i?"↑":"",u=o.projected==null?"—":o.projected.toFixed(1),c=a?`Cambio sugerido: ${a.out.name} → ${a.in.name}${a.gain!=null&&a.gain>0?` (+${a.gain.toFixed(1)} pts)`:""}. ${a.reason}`:i?`Entrada recomendada${i.out?` por ${i.out.name}`:""}. ${i.reason}`:null,f=bwoRoundNativeTip(o,c);for(let g of bwoRoundNativeNameNodes(t,o.name)){if(g.querySelector(":scope > .bwo-round-native-badge"))continue;let b=bwoBidsNode("span",`bwo-round-native-badge ${l}`,`${s}${s?" ":""}${u}`);b.title=f,b.setAttribute("aria-label",f),g.appendChild(b);let p=bwoRoundNativeHost(g);o.physical.unavailable?p.classList.add("bwo-round-native-unavailable"):o.risk&&p.classList.add("bwo-round-native-risk"),p.title||(p.title=f)}}let o=[...t.querySelectorAll("h1,h2,h3,h4,button,span,div")].find(a=>/^jornada\s+\d+$/i.test((a.textContent??"").replace(/\s+/g," ").trim())&&a.getBoundingClientRect().width>0);if(o&&!o.parentElement?.querySelector(":scope > .bwo-round-native-summary")){let a=bwoBidsNode("small","bwo-round-native-summary"),i=`${e.lineup.length}/11 alineados · `,l=document.createElement("strong");l.textContent=`${e.predicted.toFixed(1)} pts previstos`,a.append(document.createTextNode(i),l,document.createTextNode(` · ${e.risks} en riesgo · ${e.pendingMatches} partidos pendientes`)),o.insertAdjacentElement("afterend",a)}}
async function bwoEnhanceRoundPlanner(e=!1){if(!location.pathname.includes("/round")||bwoToolsPageWanted){bwoRoundNativeCleanup();return}let t=`${ke()}|${location.pathname}`;if(!e&&bwoRoundNativeDataCache&&t===bwoRoundNativeDataKey&&Date.now()-bwoRoundNativeDataAt<45e3){document.querySelector(".bwo-round-native-badge")||bwoRoundNativeRender(bwoRoundNativeDataCache);return}if(bwoRoundNativeBusy)return;bwoRoundNativeBusy=!0;try{let n=await bwoRoundData();bwoRoundNativeDataCache=n,bwoRoundNativeDataKey=t,bwoRoundNativeDataAt=Date.now(),bwoRoundNativeRender(n)}catch(n){bwoRoundNativeCleanup(),console.warn("[biwtools] No se pudo integrar la información de Jornada:",n)}finally{bwoRoundNativeBusy=!1}}

globalThis.__BIWTOOLS_BALANCE_FIX__="v1.1.2-round-native-integration";
...
bwoRefreshBalances=async function(){await bwoOriginalRefreshWithRoundPlanner(),await bwoEnhanceRoundPlanner().catch(e=>console.warn("[biwtools] No se pudo actualizar Jornada:",e))};
