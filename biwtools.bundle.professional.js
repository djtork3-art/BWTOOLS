...
.bwo-fab-menu-version::after {
            content: "v1.7.31 medias fiables por participación";
  font-size: 10px;
}
...
bwoRecRenderCard=function(e,t){bwoRecRenderCardParticipationBase(e,t);let n=[...zv.querySelectorAll(".bwo-comp")].find(a=>(a.querySelector(".bwo-comp-name")?.textContent??"").trim()==="Recomendaciones"),r=n?.querySelector(".bwo-rec-upgrade");if(!r)return;let o=bwoRecRows(e,t),a=o.slice(0,12),i=r.querySelector(".bwo-rec-upgrade-head p");i&&(i.textContent="Comparación por posición con media corregida por participación. Una puntuación alta aislada no se considera una mejora fiable.");let l=[...r.querySelectorAll(".bwo-rec-kpi strong")];l[1]&&(l[1].textContent=String(o.filter(s=>s.verdict==="Mejora clara").length));let s=r.querySelector("thead th:nth-child(4)");s&&(s.textContent="Media fiable");let u=[...r.querySelectorAll("tbody tr")];u.forEach((c,f)=>{let g=a[f];if(!g)return;let b=c.querySelector(".bwo-rec-verdict");b&&(b.classList.remove("clear","maybe","watch"),b.classList.add(g.verdict==="Mejora clara"?"clear":g.verdict==="Mejora posible"?"maybe":"watch"));let p=c.querySelector("td:nth-child(3)");if(p&&!p.querySelector(".bwo-rec-sample")){let y=bwoBidsNode("small","bwo-rec-cell-sub bwo-rec-sample",`${g.games}/${g.possibleGames} jugados · confianza ${g.sampleConfidence}`);p.appendChild(y)}let w=c.querySelector("td:nth-child(4)");if(w){w.textContent=g.games&&Number.isFinite(g.rawAverage)?`${g.rawAverage.toFixed(1)} pts`:"—";let m=bwoBidsNode("small","bwo-rec-cell-sub",g.games?`Ajustada: ${g.baseAverage.toFixed(1)} pts`:"Sin muestra válida");w.appendChild(m)}g.sampleReliable||(c.setAttribute("data-bwo-sample-low","1"),c.style.opacity=".76")});let c=r.querySelector(".bwo-rec-upgrade-foot");c&&(c.textContent="La media observada se ajusta hacia 3 puntos cuando hay pocos partidos. La previsión penaliza una participación inferior al 50% de las jornadas disponibles; con una sola aparición no se etiqueta como mejora. También pondera Media AS + SofaScore, titularidad, estado, calendario y rival.")};
...
bwoDiagKpi=function(e,t,n,r=""){return bwoDiagKpiVersionBase(e,e==="Versión"?"v1.7.31":t,n,r)};
...
globalThis.__BIWTOOLS_BALANCE_FIX__="v1.7.31-reliable-averages-idb-repair";
...
var bwoRuntimeToken=`v1.7.31-${Date.now()}-${Math.random().toString(36).slice(2)}`;
