...
.bwo-fab-menu-version::after {
            content: "v1.7.23 saldos oficiales reconciliados";
  font-size: 10px;
}
...
bwoDiagKpi=function(e,t,n,r=""){return bwoDiagKpiVersionBase(e,e==="Versión"?"v1.7.23":t,n,r)};
...
globalThis.__BIWTOOLS_BALANCE_FIX__="v1.7.23-precios-historicos-oficiales";
...
zw=async function(){await bwoBalancePrepareRebuild();let e=await bwoOriginalZw(),t=await zg(),n=t?.data?.users??[],r=await Fg().catch(()=>({})),o=typeof t?.data?.created==="number"?new Date(t.data.created*1e3).toISOString():"1970-01-01T00:00:00.000Z",a=await bwoBalanceResolveAnchor(o),i=await Li({fresh:!0}),l=await rg(i.id,a.date),s=typeof r?.newSeason==="number"?r.newSeason:null,u=a.distribution??s,c=a.distribution==null&&s!=null,f=i.balance-l,g=bwoBalanceFixedDistributions[u],b=g!=null&&c&&Math.abs(g-f)>3e6,p=g!=null&&!b?g:f,h=b?null:u,y=await z(),w=y.state.syncState;w.seasonAnchorDate=a.date,w.seasonStartingBalance=String(Math.round(p)),w.seasonDistribution=u!=null?String(u):"",w.myManagerId=String(i.id),w.myBalance=String(Math.round(i.balance)),w.derivedStartingBalance=String(Math.round(f)),w.rejectedFlatCash=b?String(g):"",delete y.state.balanceCorrections;let m=y.state.shadowStartingBalances??y.state.bwoBalanceShadows??{},d=`${a.date}|${h??""}|${typeof r?.newUsers==="number"?r.newUsers:""}|${n.map(v=>v.id).join(",")}`,v=w.bwoBalanceEngineV1722==="1"&&w.bwoBalanceShadowEngine!=="official-cf-prices-v1",M=!v&&w.shadowStartingBalancesAnchor===a.date&&n.every(B=>m[String(B.id)]),B=!M&&(w.bwoBalanceShadowSignature!==d||n.some(E=>!m[String(E.id)])||w.bwoBalanceEngineV1723!=="1"||w.bwoBalanceShadowEngine!=="official-cf-prices-v1");return B&&(m=await bwoBalanceBuildShadows(a.date,h,typeof r?.newUsers==="number"?r.newUsers:null,n),w.bwoBalanceShadowSignature=d),y.state.shadowStartingBalances=m,w.shadowStartingBalancesAnchor=a.date,w.bwoBalanceEngineV1721="1",w.bwoBalanceEngineV1722="1",w.bwoBalanceEngineV1723="1",await Cu(),e};
...
var bwoRuntimeToken=`v1.7.23-${Date.now()}-${Math.random().toString(36).slice(2)}`;
