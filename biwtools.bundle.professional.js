...
.bwo-fab-menu-version::after {
            content: "v1.8.1 Apertura de herramientas corregida";
  font-size: 10px;
}
...
bwoDiagKpi=function(e,t,n,r=""){return bwoDiagKpiVersionBase(e,e==="Versión"?"v1.8.1":t,n,r)};
...
globalThis.__BIWTOOLS_BALANCE_FIX__="v1.8.1-tools-open-fix";
...
/* BIWTOOLS PRO SUITE v1.8.1: módulos lógicos en un único bundle. */
var bwoSuiteVersion="1.8.1",bwoSuiteMetrics={startedAt:Date.now(),requests:0,errors:0,rateLimits:0,totalMs:0,cacheHits:0,cacheMisses:0};
...
function bwoSuiteEndpointList(){return["catalog.json","lineups.json","fixtures.json","injuries.json"]}
async function bwoSuiteEndpointTest(e){let t=[];for(let n of bwoSuiteEndpointList())try{let r=await Promise.race([fetch(`${e}${e.endsWith("/")?"":"/"}${n}`,{cache:"no-store"}),new Promise((e,t)=>setTimeout(()=>t(new Error("Tiempo agotado")),6500))]);if(!r.ok)throw new Error(`HTTP ${r.status}`);let o=await r.json();t.push({name:n,ok:o!=null,message:Array.isArray(o)?`${o.length} registros`:"JSON válido"})}catch(e){t.push({name:n,ok:!1,message:e?.message??String(e)})}return t}
...
var bwoRuntimeToken=`v1.8.1-${Date.now()}-${Math.random().toString(36).slice(2)}`;
