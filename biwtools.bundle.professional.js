...
.bwo-fab-menu-version::after {
            content: "v1.7.42 Once Jornada inteligente";
  font-size: 10px;
}
...
bwoDiagKpi=function(e,t,n,r=""){return bwoDiagKpiVersionBase(e,e==="Versión"?"v1.7.42":t,n,r)};
...
function bwoMatchdayXICompleteSelection(e,t,n=4){let r=new Set(t.players.map(g=>String(g.id))),o=(e.players??[]).filter(g=>!r.has(String(g.id))&&!g.physical?.unavailable).sort((g,b)=>bwoMatchdayXIPlayerRank(b)-bwoMatchdayXIPlayerRank(g)),a=[],i=Math.max(0,Number(n)||0),l=["1","2","3","4"];for(let g=0;g<Math.min(i,l.length);g++)a.push(o.find(b=>b.position===l[g])??null);for(let g of o)if(a.length<i&&!a.some(b=>b&&String(b.id)===String(g.id)))a.push(g);let s=[...t.players].filter(g=>!g.physical?.unavailable).sort((g,b)=>bwoMatchdayXIPlayerRank(b)-bwoMatchdayXIPlayerRank(g)),u=s[0]??t.players[0]??null,c=s.filter(g=>g.position==="4").sort((g,b)=>bwoMatchdayXIPlayerRank(b)-bwoMatchdayXIPlayerRank(g)),f=c.find(g=>!u||String(g.id)!==String(u.id))??c[0]??s.find(g=>!u||String(g.id)!==String(u.id))??u;return{...t,reserves:a.slice(0,i),captain:u,striker:f,_allPlayers:e.players??[]}}
...
async function bwoMatchdayXISave(e,t){if(bwoMatchdayXISaving)return;let n=`Se abrirá Equipo y se guardará la formación ${e.formation} con:\n\nTitulares: ${e.players.map(g=>g.name).join(", ")}\n\nSuplentes: ${(e.reserves??[]).map(g=>g?.name??"Vacío").join(", ")||"sin suplentes"}\nCapitán: ${e.captain?.name??"—"}\nAriete: ${e.striker?.name??"—"}\n\n¿Quieres aplicar la alineación completa?`;if(!confirm(n))return;bwoMatchdayXISaving=!0;let r=t.textContent;t.disabled=!0,t.textContent="Abriendo Equipo…";try{let o=await bwoMatchdayXIHeaders(),a=bwoMatchdayXICompleteSelection({players:e._allPlayers??[...e.players,...(e.reserves??[])]},e,e.reserves?.length||4),i=a.players.map(h=>Number.isFinite(Number(h.id))?Number(h.id):String(h.id)),l=a.reserves.map(h=>h==null?null:Number.isFinite(Number(h.id))?Number(h.id):String(h.id)),s=a.captain?{id:Number.isFinite(Number(a.captain.id))?Number(a.captain.id):String(a.captain.id)}:null,u=a.striker?{id:Number.isFinite(Number(a.striker.id))?Number(a.striker.id):String(a.striker.id)}:null,c=`${qi}/user?fields=*,lineup(type,playersID,reservesID,captain,striker)`,f=async(h,E)=>{t.textContent=E;let k=await bwoApiFetch(c,{method:"PUT",headers:o,body:JSON.stringify({lineup:h})});if(!k.ok)throw await bwoMatchdayXIResponseError(k,E.replace("…",""));return k.json().catch(()=>null)},g={type:a.formation,playersID:i};bwoMatchdayXIGoTeam(),await new Promise(h=>setTimeout(h,600));await f(g,"Guardando once titular…");await f({...g,reservesID:l},"Asignando suplentes…");s&&await f({...g,reservesID:l,captain:s},"Asignando capitán…");u&&await f({...g,reservesID:l,...s&&{captain:s},striker:u},"Asignando ariete…");t.textContent="Comprobando alineación…";let b=await bwoApiFetch(c,{headers:o,cache:"no-store"});if(!b.ok)throw new Error(`La alineación se guardó, pero no se pudo comprobar (${b.status})`);let p=bwoMatchdayXILineupFromResponse(await b.json()),y=bwoMatchdayXIIds(p?.players??p?.playersID).sort(),w=i.map(String).sort();if(y.length!==11||y.join("|")!==w.join("|"))throw new Error("Biwenger no confirmó los 11 titulares");let d=bwoMatchdayXIIds(p?.reserves??p?.reservesID).sort(),v=l.filter(h=>h!=null).map(String).sort();if(d.length!==v.length||d.join("|")!==v.join("|"))throw new Error("Los titulares están guardados, pero Biwenger no confirmó los suplentes");let S=bwoMatchdayXIIdValue(p?.captain),x=bwoMatchdayXIIdValue(p?.striker);if(s&&S!==String(s.id))throw new Error("Titulares y suplentes guardados, pero Biwenger no confirmó el capitán");if(u&&x!==String(u.id))throw new Error("Titulares, suplentes y capitán guardados, pero Biwenger no confirmó el ariete");await Uf(w),bwoMatchdayXICache=null,bwoMatchdayXICacheAt=0,bwoRoundNativeDataCache=null,bwoRoundNativeDataAt=0,t.textContent="✓ Alineación completa guardada",bwoRecommendationToast(`Once, ${l.filter(h=>h!=null).length} suplentes, capitán y ariete guardados correctamente.`,"ok"),setTimeout(()=>location.pathname==="/team"&&location.reload(),800)}catch(o){console.warn("[biwtools] No se pudo completar Once Jornada:",o),bwoRecommendationToast(o?.message??"No se pudo completar la alineación."),t.disabled=!1,t.textContent=r}finally{bwoMatchdayXISaving=!1}}
...
globalThis.__BIWTOOLS_BALANCE_FIX__="v1.7.42-matchday-xi-network-guard-rate-limit-reliable-averages-idb-repair";
...
var bwoRuntimeToken=`v1.7.42-${Date.now()}-${Math.random().toString(36).slice(2)}`;
