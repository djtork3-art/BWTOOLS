...
.bwo-fab-menu-version::after {
  content: "v1.6.1 móvil optimizado";
  font-size: 10px;
}
...
function bwoModernizeFabMenu(){let e=zv?.querySelector(".bwo-fab-menu");if(!e)return;let t=e.querySelector(".bwo-fab-menu-version");t&&(t.textContent="v1.6.1",t.setAttribute("aria-label","Biwtools versión 1.6.1"));let n=e.querySelector(".bwo-fab-menu-head");if(n&&!n.querySelector(".bwo-fab-menu-brand")){let p=bwoBidsNode("div","bwo-fab-menu-brand"),y=n.querySelector(".bwo-fab-menu-wordmark"),w=bwoBidsNode("small",null,"Centro de análisis");y&&p.appendChild(y),p.appendChild(w),n.insertBefore(p,t??n.firstChild)}if(!e.querySelector(".bwo-fab-menu-section")){let p=bwoBidsNode("div","bwo-fab-menu-section","Accesos rápidos");n?.insertAdjacentElement("afterend",p)}for(let p of e.querySelectorAll(".bwo-fab-item")){if(p.dataset.bwoFabModern==="1")continue;let y=(p.textContent??"").trim().toLocaleLowerCase("es"),w=bwoToolsCatalog.find(v=>y.includes(v.name.toLocaleLowerCase("es")));if(!w)continue;let m=p.querySelector(".bwo-fab-item-icon")??bwoBidsNode("span","bwo-fab-item-icon",w.icon),d=bwoBidsNode("span","bwo-fab-item-copy"),v=bwoBidsNode("strong",null,w.name),S=bwoBidsNode("small",null,`${w.category} · ${w.status}`),x=bwoBidsNode("span","bwo-fab-item-arrow","›");d.append(v,S),p.replaceChildren(m,d,x),p.dataset.bwoFabModern="1",p.dataset.bwoToolName=w.name,p.title=w.description,p.setAttribute("aria-label",`${w.name}. ${w.description}`)}if(!e.querySelector(".bwo-fab-center-open")){let p=bwoBidsNode("button","bwo-fab-center-open"),y=bwoBidsNode("span",null,"Abrir centro completo"),w=bwoBidsNode("small",null,"Tarjetas, métricas y buscador");p.type="button",p.append(y,w),p.addEventListener("click",()=>{zv.querySelector(".bwo-fab")?.click(),setTimeout(bwoOpenToolsPage,0)}),e.appendChild(p)}e.dataset.bwoModernVersion="1.6.1"}
var bwoModernizeFabMenuRender=bwoModernizeFabMenu;
bwoModernizeFabMenu=function(){let e=zv?.querySelector(".bwo-fab-menu");if(!e||e.dataset.bwoModernVersion==="1.6.1")return;return bwoModernizeFabMenuRender()};
...
bwoDiagKpi=function(e,t,n,r=""){return bwoDiagKpiVersionBase(e,e==="Versión"?"v1.6.1":t,n,r)};
...
function bwoClauseEnsureStyles(){let e=document.getElementById("bwo-clause-tool-styles");if(e?.dataset.bwoVersion==="1.6.1")return;e||(e=document.createElement("style"),e.id="bwo-clause-tool-styles"),e.dataset.bwoVersion="1.6.1",e.textContent=`
...
function bwoClauseOpenDetail(e){bwoCloseToolsPage(),setTimeout(()=>bwoRecommendationOpenDetail(e.catalog,e),80)}
...
function bwoMobileToolsNavEnsureStyles(){let e=document.getElementById("bwo-mobile-tools-nav-styles");if(e?.dataset.bwoVersion==="1.6.1")return;e||(e=document.createElement("style"),e.id="bwo-mobile-tools-nav-styles"),e.dataset.bwoVersion="1.6.1",e.textContent=`
 @media(max-width:768px){
...
globalThis.__BIWTOOLS_BALANCE_FIX__="v1.6.1-clausulas-oficiales";
...
var bwoRuntimeToken=`v1.6.1-${Date.now()}-${Math.random().toString(36).slice(2)}`;
