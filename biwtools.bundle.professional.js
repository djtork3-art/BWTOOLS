...
.bwo-fab-menu-version::after {
            content: "v1.7.47 Media fiable por jornada";
  font-size: 10px;
}
...
bwoRecRenderCard=function(e,t){bwoRecRenderCardParticipationBase(e,t);let n=[...zv.querySelectorAll(".bwo-comp")].find(a=>(a.querySelector(".bwo-comp-name")?.textContent??"").trim()==="Recomendaciones"),r=n?.querySelector(".bwo-rec-upgrade");if(!r)return;let o=bwoRecRows(e,t),a=o.slice(0,12),i=r.querySelector(".bwo-rec-upgrade-head p");i&&(i.textContent="Comparación por posición con media fiable por jornada, participación real y datos consolidados. Una puntuación alta aislada no se considera una mejora fiable.");let l=[...r.querySelectorAll(".bwo-rec-kpi strong")];l[1]&&(l[1].textContent=String(o.filter(s=>s.verdict==="Mejora clara").length));let s=r.querySelector("thead th:nth-child(4)");s&&(s.textContent="Media fiable / jornada");let u=[...r.querySelectorAll("tbody tr")];u.forEach((c,f)=>{let g=a[f];if(!g)return;let b=c.querySelector(".bwo-rec-verdict");b&&(b.classList.remove("clear","maybe","watch"),b.classList.add(g.verdict==="Mejora clara"?"clear":g.verdict==="Mejora posible"?"maybe":"watch"));let p=c.querySelector("td:nth-child(3)");if(p){g.recentPoints.length||(p.textContent=g.games?`${g.games} partidos contabilizados`:"Sin partidos actuales");if(!p.querySelector(".bwo-rec-sample")){let y=bwoBidsNode("small","bwo-rec-cell-sub bwo-rec-sample",`${g.games}/${g.possibleGames} jugados · confianza ${g.sampleConfidence}`);p.appendChild(y)}}let w=c.querySelector("td:nth-child(4)");if(w){w.textContent=g.games&&Number.isFinite(g.reliableRoundAverage)?`${g.reliableRoundAverage.toFixed(2)} pts/J`:"—";let m=bwoBidsNode("small","bwo-rec-cell-sub",g.games?`Cuando juega: ${g.rawAverage.toFixed(1)} pts · ${g.games}/${g.possibleGames} jornadas`:"Sin muestra válida");w.appendChild(m)}g.sampleReliable||(c.setAttribute("data-bwo-sample-low","1"),c.style.opacity=".76")});let c=r.querySelector(".bwo-rec-refresh");if(c){let f=Math.max(0,bwoRecommendationUpgradeCooldown-(Date.now()-bwoRecommendationUpgradeAt));c.title=f?`Los datos se reutilizan para proteger el límite de Biwenger. Próxima consulta en ${Math.ceil(f/1e3)} s.`:"Actualizar los datos consolidados"}let f=r.querySelector(".bwo-rec-upgrade-foot");f&&(f.textContent="Media fiable/jornada = media ajustada por tamaño de muestra × jornadas jugadas/posibles: las ausencias cuentan como 0. Con pocos partidos, la media observada se corrige de forma conservadora hacia 3 puntos; con una sola aparición no se etiqueta como mejora. Los datos ya consultados se reutilizan desde caché para proteger el límite de Biwenger.")};
...
bwoDiagKpi=function(e,t,n,r=""){return bwoDiagKpiVersionBase(e,e==="Versión"?"v1.7.47":t,n,r)};
...
globalThis.__BIWTOOLS_BALANCE_FIX__="v1.7.47-recommendations-reliable-round-average";
...
var bwoRuntimeToken=`v1.7.47-${Date.now()}-${Math.random().toString(36).slice(2)}`;
