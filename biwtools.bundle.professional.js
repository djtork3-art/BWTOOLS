...
.bwo-fab-menu-version::after {
            content: "v1.7.38 Once Jornada inteligente";
  font-size: 10px;
}
...
bwoDiagKpi=function(e,t,n,r=""){return bwoDiagKpiVersionBase(e,e==="Versión"?"v1.7.38":t,n,r)};
...
function bwoMatchdayXIIds(e){return(e??[]).map(t=>String(t?.id??t)).filter(Boolean)}
...
async function bwoMatchdayXISave(e,t){if(bwoMatchdayXISaving)return;let n=`Se guardará la formación ${e.formation} con:\n\nTitulares: ${e.players.map(g=>g.name).join(", ")}\n\nSuplentes: ${(e.reserves??[]).map(g=>g.name).join(", ")||"sin suplentes"}\nCapitán: ${e.captain?.name??"—"}\nAriete: ${e.striker?.name??"—"}\n\n¿Quieres aplicar y guardar toda la alineación?`;if(!confirm(n))return;bwoMatchdayXISaving=!0;let r=t.textContent;t.disabled=!0,t.textContent="Guardando titulares, suplentes y roles…";try{let o=await bwoMatchdayXIHeaders(),a=String(await pn()),i=await bwoApiFetch(`${qi}/rounds/league`,{headers:o,cache:"no-store"});if(!i.ok)throw new Error(`No se pudo consultar la jornada activa (${i.status})`);let l=bwoMatchdayXIRoundInfo(await i.json(),a),s=l.roundId??l.raw?.id;if(s==null)throw new Error("Biwenger no indicó la jornada activa");let u=bwoMatchdayXIIds(l.lineup?.reserves??l.lineup?.reservesID),c=u.length||e.reserves?.length||4,f=bwoMatchdayXICompleteSelection({players:e._allPlayers??[...e.players,...(e.reserves??[])]},e,c);bwoMatchdayXIGoTeam(),await new Promise(h=>setTimeout(h,550));let g={round:Number.isFinite(Number(s))?Number(s):s,lineup:bwoMatchdayXILineupPayload(f,!0)},b=`${qi}/user/${encodeURIComponent(a)}/roundLineup`,p=await bwoApiFetch(b,{method:"PUT",headers:o,body:JSON.stringify(g)});if(!p.ok&&[400,404,409,422,500].includes(p.status)){let h={round:g.round,lineup:bwoMatchdayXILineupPayload(f,!1)};p=await bwoApiFetch(b,{method:"PUT",headers:o,body:JSON.stringify(h)})}if(!p.ok){let h=await p.text().catch(()=>""),E=null;try{let k=JSON.parse(h);E=k?.data?.userMessage??k?.userMessage??k?.data?.message??k?.message}catch{}(!E||/^unexpected error$/i.test(String(E)))&&(E=`Biwenger no pudo guardar la alineación (HTTP ${p.status}). Actualiza la página y vuelve a intentarlo.`);throw new Error(E)}let y=await p.json().catch(()=>null),w=bwoMatchdayXILineupFromResponse(y,a);if(!w){let h=await bwoApiFetch(`${qi}/rounds/league`,{headers:o,cache:"no-store"});if(!h.ok)throw new Error(`No se pudo comprobar la alineación guardada (${h.status})`);w=bwoMatchdayXIRoundInfo(await h.json(),a).lineup}let d=bwoMatchdayXIIds(w?.players??w?.playersID).sort(),v=f.players.map(h=>String(h.id)).sort();if(d.length!==11||d.join("|")!==v.join("|"))throw new Error("Biwenger no confirmó los 11 titulares seleccionados");let S=bwoMatchdayXIIds(w?.reserves??w?.reservesID).sort(),x=f.reserves.map(h=>String(h.id)).sort();if(("reserves"in(w??{})||"reservesID"in(w??{}))&&(S.length!==x.length||S.join("|")!==x.join("|")))throw new Error("Biwenger no confirmó los suplentes seleccionados");let C=bwoMatchdayXIIdValue(w?.captain),N=bwoMatchdayXIIdValue(w?.striker);if(C&&C!==String(f.captain?.id))throw new Error("Biwenger no confirmó el capitán recomendado");if(N&&N!==String(f.striker?.id))throw new Error("Biwenger no confirmó el ariete recomendado");await Uf(v),bwoMatchdayXICache=null,bwoMatchdayXICacheAt=0,bwoRoundNativeDataCache=null,bwoRoundNativeDataAt=0,t.textContent="✓ Alineación completa guardada",bwoRecommendationToast(`Once ${f.formation}, ${f.reserves.length} suplentes, capitán y ariete guardados en Biwenger.`,"ok"),setTimeout(()=>location.pathname==="/team"&&location.reload(),700)}catch(o){console.warn("[biwtools] No se pudo guardar Once Jornada:",o),bwoRecommendationToast(o?.message??"No se pudo guardar la alineación. Revisa si la jornada está bloqueada."),t.disabled=!1,t.textContent=r}finally{bwoMatchdayXISaving=!1}}
...
globalThis.__BIWTOOLS_BALANCE_FIX__="v1.7.38-matchday-xi-network-guard-rate-limit-reliable-averages-idb-repair";
...
var bwoRuntimeToken=`v1.7.38-${Date.now()}-${Math.random().toString(36).slice(2)}`;
