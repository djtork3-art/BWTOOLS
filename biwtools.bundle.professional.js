...
.bwo-fab-menu-version::after {
            content: "v1.7.24 motor público y fuentes JSON";
  font-size: 10px;
}
...
bwoDiagKpi=function(e,t,n,r=""){return bwoDiagKpiVersionBase(e,e==="Versión"?"v1.7.24":t,n,r)};
...
globalThis.__BIWTOOLS_BALANCE_FIX__="v1.7.24-motor-publico-json";
...
$w=async function(){let e=0,t=await Fg().catch(()=>({}));for(let n of bwoBalanceEventTypes){try{let r=`lastSeenDate:${n}`,o=Number(await fn(r)??0),a=0,i=!1,l=o,s=!0;for(;!i;){s||await new Promise(c=>setTimeout(c,220)),s=!1;let u=await Ui({type:n,offset:a,limit:50}),f=u?.data??u??[];if(f.length===0)break;for(let c of f){if(c.date<=o){i=!0;break}await Ug(c,{splitRound:t?.splitRound}),e++}a===0&&f[0]&&(l=Math.max(l,f[0].date)),a+=50;if(f.length<50)break}await Le(r,String(l)),await Cu()}catch(r){console.warn(`[biwtools] Se omite el tipo de tablón ${n}:`,r)}}return e};
...
var bwoRuntimeToken=`v1.7.24-${Date.now()}-${Math.random().toString(36).slice(2)}`;
