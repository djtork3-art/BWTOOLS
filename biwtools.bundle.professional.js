...
.bwo-fab-menu-version::after {
            content: "v1.7.28 conciliación monetaria completa";
  font-size: 10px;
}
...
bwoDiagKpi=function(e,t,n,r=""){return bwoDiagKpiVersionBase(e,e==="Versión"?"v1.7.28":t,n,r)};
...
globalThis.__BIWTOOLS_BALANCE_FIX__="v1.7.28-round-bonus-and-cash-adjustments";
...
async function bwoBalanceMapLimit(e,t,n){let r=new Array(e.length),o=0;async function a(){for(;o<e.length;){let i=o++;r[i]=await n(e[i],i),await new Promise(l=>setTimeout(l,300))}}return await Promise.all(Array.from({length:Math.min(t,e.length||1)},a)),r}
...
async function bwoBalanceBuildShadows(e,t,n,r){let o={};for(let l of r){let s=String(l.id),u=typeof l.joinDate==="number"?new Date(l.joinDate*1e3).toISOString():null,c=u!=null&&u>e,f=c?u:e,g=c?n:t,b=Number(bwoBalanceFixedDistributions[g]),p=Number(bwoBalanceTeamValueDistributions[g]);if(Number.isFinite(b)){o[s]={startDate:f,joinedAfterAnchor:c,distribution:g,teamValue:null,squadSize:null,movementsUndone:null,pricesApprox:null,balance:b,unresolved:!1};continue}if(!Number.isFinite(p)){o[s]={startDate:f,joinedAfterAnchor:c,distribution:g,teamValue:null,squadSize:null,movementsUndone:null,pricesApprox:null,balance:null,unresolved:!0};continue}let y;try{y=await qg(l.id)}catch{o[s]={startDate:f,joinedAfterAnchor:c,distribution:g,teamValue:null,squadSize:null,movementsUndone:null,pricesApprox:null,balance:null,unresolved:!0};continue}let w=m=>m.player_id===s,m=(await z()).state.movements.filter(w),d=bwoBalanceSquadAt(y,m,f),v=await bwoBalanceMapLimit(d.squad,2,x=>bwoBalancePriceAt(x,f)),M=0,B=0,E=d.unresolved;for(let x of v)x?(M+=x.price,x.exact||B++):E=!0;o[s]={startDate:f,joinedAfterAnchor:c,distribution:g,teamValue:M,squadSize:d.squad.length,movementsUndone:m.filter(x=>x.occurred_at>f&&(bwoBalanceIncomingTypes.has(x.type)||bwoBalanceOutgoingTypes.has(x.type))).length,pricesApprox:B,balance:p-M,unresolved:E}}let P=await z();P.state.shadowStartingBalances=o,P.state.bwoBalanceShadows=o,P.state.syncState.shadowStartingBalancesAnchor=e,P.state.syncState.bwoBalanceShadowAnchor=e,P.state.syncState.bwoBalanceShadowDistribution=String(t??""),P.state.syncState.bwoBalanceShadowBuiltAt=String(Date.now()),P.state.syncState.bwoBalanceShadowEngine="public-v1720-json-v3",await Cu();return o}
...
var bwoRuntimeToken=`v1.7.28-${Date.now()}-${Math.random().toString(36).slice(2)}`;
