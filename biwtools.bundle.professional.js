...
function bwoEnsureBidsNav(){if(document.querySelector("[data-bwo-bids-nav]"))return;let e=[],t=document.createTreeWalker(document.body,NodeFilter.SHOW_TEXT);for(let n;n=t.nextNode();)if((n.textContent??"").replace(/\s+/g," ").trim().toLocaleLowerCase("es")==="jugadores"&&n.parentElement)e.push(n.parentElement);let n=e.find(r=>{let o=r.getBoundingClientRect();return r.getClientRects().length>0&&o.top>=0&&o.top<120})??e[0];if(!n)return;let r=bwoNavUnit(n);if(!r||r===document.body)return;let o=r.cloneNode(!0),a=o.matches("a, button, [role='menuitem']")?o:o.querySelector("a, button, [role='menuitem'], [tabindex]")??o;o.querySelectorAll("[id]").forEach(i=>i.removeAttribute("id")),o.querySelectorAll("[aria-current]").forEach(i=>i.removeAttribute("aria-current")),o.querySelectorAll(".active, .is-active, .selected, .current").forEach(i=>i.classList.remove("active","is-active","selected","current")),o.dataset.bwoBidsNav="1",o.classList.add("bwo-nav-bids"),a.dataset.bwoBidsNav="1",a.classList.add("bwo-nav-bids","bwo-nav-bids-trigger"),a.removeAttribute("routerlink"),a.removeAttribute("ng-reflect-router-link"),a.removeAttribute("aria-current"),a.tagName==="A"&&a.setAttribute("href","#"),a.tagName==="BUTTON"&&(a.type="button"),a.matches("a, button")||(a.setAttribute("role","button"),a.tabIndex=0),a.setAttribute("title","Ver las pujas reveladas de tus rivales"),a.setAttribute("aria-label","Ver pujas de rivales"),a.setAttribute("aria-haspopup","dialog"),a.setAttribute("aria-expanded","false");let i=bwoBidsNode("span","bwo-nav-bids-icon","◉"),l=bwoBidsNode("span","bwo-nav-bids-label","Ver pujas");i.setAttribute("aria-hidden","true"),a.replaceChildren(i,l);let s=u=>{u.preventDefault(),u.stopImmediatePropagation(),bwoOpenBidsPanel()};a.addEventListener("click",s),a.addEventListener("keydown",u=>{(u.key==="Enter"||u.key===" ")&&s(u)}),r.insertAdjacentElement("afterend",o)}
globalThis.__BIWTOOLS_BALANCE_FIX__="v1.0.53-navbar-bids";
