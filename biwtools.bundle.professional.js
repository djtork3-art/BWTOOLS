...
.bwo-fab-menu-version::after {
            content: "v1.7.40 Once Jornada inteligente";
  font-size: 10px;
}
...
bwoDiagKpi=function(e,t,n,r=""){return bwoDiagKpiVersionBase(e,e==="Versión"?"v1.7.40":t,n,r)};
...
function bwoMatchdayXIProbability(e){let t=Number(e.jp?.probability??e.probability?.prob);return Number.isFinite(t)?Math.max(0,Math.min(100,t)):50}
function bwoMatchdayXIRisk(e){let t=bwoRoundNativeText(`${e.jp?.status??""} ${e.jp?.pick??""} ${e.jp?.form??""}`);return!!(e.physical?.unavailable||e.physical?.key==="risk"||bwoMatchdayXIProbability(e)<50||/lesion|sancion|duda|riesgo|pufo|racha baja/.test(t))}
function bwoMatchdayXIValue(e){let t=e.projected==null?NaN:Number(e.projected),n=e.average==null?NaN:Number(e.average),r=bwoMatchdayXIProbability(e),o=Number.isFinite(t)?t:Number.isFinite(n)?n*Math.max(.15,r/100):0,a=e.jp?bwoRecJPBonus(e.jp):0;return e.physical?.unavailable?o-100:e.physical?.key==="risk"?o*.78+a:o+a}
function bwoMatchdayXIPlans(e){let t=e.players??[],n=t.filter(l=>l.lineup&&l.locked),r=[];for(let o of bwoMatchdayXIFormations){let a=[],i=!0;for(let l of["1","2","3","4"]){let s=n.filter(g=>g.position===l),u=o.slots[l];if(s.length>u){i=!1;break}let c=t.filter(g=>g.position===l&&!g.locked&&!s.some(b=>b.id===g.id)).sort((g,b)=>bwoMatchdayXIValue(b)-bwoMatchdayXIValue(g)),f=s.concat(c.slice(0,u-s.length));if(f.length<u){i=!1;break}a.push(...f)}if(i&&a.length===11){let l=a.reduce((s,u)=>s+(Number(u.projected)||0),0),s=a.reduce((u,c)=>u+bwoMatchdayXIValue(c),0),u=a.filter(bwoMatchdayXIRisk).length,c=a.filter(g=>!g.physical?.unavailable&&bwoMatchdayXIProbability(g)>=50).length;r.push({formation:o.name,players:a,projected:l,score:s,risks:u,reliable:c})}}return r.sort((o,a)=>a.score-o.score||a.projected-o.projected)}
...
function bwoMatchdayXIIds(e){return(e??[]).map(t=>t?.id??t).filter(t=>t!=null&&t!=="").map(String)}
...
function bwoMatchdayXIPlayerRank(e){let t=bwoMatchdayXIValue(e),n=bwoMatchdayXIProbability(e);return t+Math.min(100,Math.max(0,n))*.035-(e.physical?.unavailable?100:0)-(e.locked?2:0)}
function bwoMatchdayXICompleteSelection(e,t,n=4){let r=new Set(t.players.map(g=>String(g.id))),o=(e.players??[]).filter(g=>!r.has(String(g.id))&&!g.physical?.unavailable).sort((g,b)=>bwoMatchdayXIPlayerRank(b)-bwoMatchdayXIPlayerRank(g)),a=[],i=Math.max(0,Number(n)||0);for(let g of["1","2","3","4"]){if(a.length>=i)break;let b=o.find(p=>p.position===g&&!a.includes(p));b&&a.push(b)}for(let g of o)if(a.length<i&&!a.includes(g))a.push(g);let l=[...t.players].filter(g=>!g.physical?.unavailable).sort((g,b)=>bwoMatchdayXIPlayerRank(b)-bwoMatchdayXIPlayerRank(g)),s=l[0]??t.players[0]??null,u=l.filter(g=>g.position==="4").sort((g,b)=>bwoMatchdayXIPlayerRank(b)-bwoMatchdayXIPlayerRank(g)),c=u.find(g=>!s||String(g.id)!==String(s.id))??u[0]??l.find(g=>!s||String(g.id)!==String(s.id))??s;return{...t,reserves:a.slice(0,i),captain:s,striker:c,_allPlayers:e.players??[]}}
...
async function bwoMatchdayXIUpdateSlot(e,t,n,r,o,a,i,l=null){let s={round:Number.isFinite(Number(n))?Number(n):n,zone:r,lineupPosition:o,in:Number.isFinite(Number(i))?Number(i):i};a!=null&&a!==""&&(s.out=Number.isFinite(Number(a))?Number(a):a),l!=null&&l!==""&&(s.playingAs=Number.isFinite(Number(l))?Number(l):l);let u=await bwoApiFetch(`${qi}/user/${encodeURIComponent(t)}/roundLineup`,{method:"PUT",headers:e,body:JSON.stringify(s)});if(!u.ok)throw await bwoMatchdayXIResponseError(u,`No se pudo asignar ${r==="reserves"?"un suplente":r==="captain"?"el capitán":"el ariete"}`);return u.json().catch(()=>null)}
async function bwoMatchdayXISave(e,t){if(bwoMatchdayXISaving)return;let n=`Se abrirá Equipo y se guardará la formación ${e.formation} con:\n\nTitulares: ${e.players.map(g=>g.name).join(", ")}\n\nSuplentes: ${(e.reserves??[]).map(g=>g.name).join(", ")||"sin suplentes"}\nCapitán: ${e.captain?.name??"—"}\nAriete: ${e.striker?.name??"—"}\n\n¿Quieres aplicar la alineación completa?`;if(!confirm(n))return;bwoMatchdayXISaving=!0;let r=t.textContent;t.disabled=!0,t.textContent="Abriendo Equipo…";try{let o=await bwoMatchdayXIHeaders(),a=String(await pn()),i=await bwoApiFetch(`${qi}/rounds/league`,{headers:o,cache:"no-store"});if(!i.ok)throw new Error(`No se pudo consultar la jornada activa (${i.status})`);let l=bwoMatchdayXIRoundInfo(await i.json(),a),s=l.roundId??l.raw?.id;if(s==null)throw new Error("Biwenger no indicó la jornada activa");let u=l.lineup?.reserves??l.lineup?.reservesID??[],c=Array.isArray(u)&&u.length?u.length:e.reserves?.length||4,f=bwoMatchdayXICompleteSelection({players:e._allPlayers??[...e.players,...(e.reserves??[])]},e,c),g=f.players.map(h=>Number.isFinite(Number(h.id))?Number(h.id):String(h.id));bwoMatchdayXIGoTeam(),await new Promise(h=>setTimeout(h,600));t.textContent="Guardando once titular…";let b=await bwoApiFetch(`${qi}/user?fields=*,lineup(type,playersID)`,{method:"PUT",headers:o,body:JSON.stringify({lineup:{type:f.formation,playersID:g}})});if(!b.ok)throw await bwoMatchdayXIResponseError(b,"No se pudo guardar el once titular");await new Promise(h=>setTimeout(h,250));let p=await bwoApiFetch(`${qi}/rounds/league`,{headers:o,cache:"no-store"});if(!p.ok)throw new Error(`El once se guardó, pero no se pudieron consultar los suplentes (${p.status})`);let y=bwoMatchdayXIRoundInfo(await p.json(),a),w=bwoMatchdayXISlotIds(y.lineup?.reserves??y.lineup?.reservesID),d=f.reserves.map(h=>String(h.id));t.textContent="Asignando suplentes…";for(let h=0;h<d.length;h++){if(w[h]===d[h])continue;let E=w.indexOf(d[h]),k=w[h];await bwoMatchdayXIUpdateSlot(o,a,s,"reserves",h,k,d[h],f.reserves[h]?.position??h+1),E>=0&&(w[E]=k),w[h]=d[h]}let v=bwoMatchdayXIIdValue(y.lineup?.captain),S=bwoMatchdayXIIdValue(y.lineup?.striker),x=String(f.captain?.id??""),C=String(f.striker?.id??"");if(x&&v!==x)t.textContent="Asignando capitán…",await bwoMatchdayXIUpdateSlot(o,a,s,"captain",0,v,x);if(C&&S!==C)t.textContent="Asignando ariete…",await bwoMatchdayXIUpdateSlot(o,a,s,"striker",0,S,C);t.textContent="Comprobando alineación…";let N=await bwoApiFetch(`${qi}/rounds/league`,{headers:o,cache:"no-store"});if(!N.ok)throw new Error(`La alineación se guardó, pero no se pudo comprobar (${N.status})`);let _=bwoMatchdayXIRoundInfo(await N.json(),a).lineup,L=bwoMatchdayXIIds(_?.players??_?.playersID).sort(),A=g.map(String).sort();if(L.length!==11||L.join("|")!==A.join("|"))throw new Error("Los suplentes se procesaron, pero Biwenger no confirmó los 11 titulares");let I=bwoMatchdayXIIds(_?.reserves??_?.reservesID).sort(),D=d.slice().sort();if(I.length!==D.length||I.join("|")!==D.join("|"))throw new Error("El once está guardado, pero Biwenger no confirmó todos los suplentes");let M=bwoMatchdayXIIdValue(_?.captain),R=bwoMatchdayXIIdValue(_?.striker);if(M&&M!==x)throw new Error("El once está guardado, pero Biwenger no confirmó el capitán");if(R&&R!==C)throw new Error("El once está guardado, pero Biwenger no confirmó el ariete");await Uf(A),bwoMatchdayXICache=null,bwoMatchdayXICacheAt=0,bwoRoundNativeDataCache=null,bwoRoundNativeDataAt=0,t.textContent="✓ Alineación completa guardada",bwoRecommendationToast(`Once, ${d.length} suplentes, capitán y ariete guardados correctamente.`,"ok"),setTimeout(()=>location.pathname==="/team"&&location.reload(),800)}catch(o){console.warn("[biwtools] No se pudo completar Once Jornada:",o),bwoRecommendationToast(o?.message??"No se pudo completar la alineación."),t.disabled=!1,t.textContent=r}finally{bwoMatchdayXISaving=!1}}
function bwoMatchdayXIRender(e,t,n=null){let r=t.querySelector(".bwo-xi-content");if(!r)return;let o=bwoMatchdayXIPlans(e);if(!o.length){r.replaceChildren(bwoBidsNode("div","bwo-xi-empty","No se puede construir un once reglamentario con los jugadores y partidos desbloqueados disponibles. Revisa que tu plantilla tenga al menos un portero, tres defensas, tres centrocampistas y un delantero."));return}let a=bwoMatchdayXICompleteSelection(e,o.find(m=>m.formation===n)??o[0]),i=bwoMatchdayXIChanges(e,a),l=(Number(a.projected)||0)-(Number(e.predicted)||0),s=bwoBidsNode("div","bwo-xi-toolbar"),u=bwoBidsNode("div","bwo-xi-toolbar-copy");u.append(bwoBidsNode("strong",null,"Configuración recomendada"),bwoBidsNode("span",null,"La formación automática maximiza previsión, disponibilidad y probabilidad de jugar."));let c=bwoBidsNode("select","bwo-xi-select");c.setAttribute("aria-label","Elegir formación");for(let m of o){let d=bwoBidsNode("option",null,`${m.formation}${m===o[0]?" · recomendada":""}`);d.value=m.formation,d.selected=m.formation===a.formation,c.appendChild(d)}c.addEventListener("change",()=>bwoMatchdayXIRender(e,t,c.value)),s.append(u,c);let f=bwoBidsNode("div","bwo-xi-kpis");f.append(bwoMatchdayXIKpi("Formación",a.formation,"good"),bwoMatchdayXIKpi("Previsión",`${a.projected.toFixed(1)} pts`,"good"),bwoMatchdayXIKpi("Frente a tu once",`${l>=0?"+":""}${l.toFixed(1)} pts`,l>=.1?"good":l<-.1?"bad":""),bwoMatchdayXIKpi("Titulares fiables",`${a.reliable}/11`,a.reliable>=10?"good":"warn"),bwoMatchdayXIKpi("En riesgo",String(a.risks),a.risks?"warn":"good"));let g=a.players.filter(m=>m.locked).length,b=bwoBidsNode("p",`bwo-xi-notice ${a.risks?"warn":"good"}`,g?`${e.stateLabel}: se han conservado ${g} jugador${g===1?"":"es"} cuyo partido ya está bloqueado. ${a.risks?"Revisa las alertas antes de confirmar el resto del once.":"El resto del once está disponible."}`:a.risks?`Hay ${a.risks} jugador${a.risks===1?"":"es"} con lesión, duda o baja probabilidad de titularidad. La propuesta los evita cuando existe una alternativa válida.`:"Once completo, sin bajas conocidas y con buena probabilidad de participación.");let p=bwoBidsNode("div","bwo-xi-layout"),y=bwoBidsNode("section","bwo-xi-panel"),w=bwoBidsNode("header","bwo-xi-panel-head");w.append(bwoBidsNode("h2",null,"Once recomendado"),bwoBidsNode("span",null,`${a.formation} · ordenado por posición`));let d=bwoBidsNode("table","bwo-xi-table"),v=bwoBidsNode("thead"),S=bwoBidsNode("tr");for(let m of["Jugador","Pos.","Partido","Estado","Titular","Previsión"])S.appendChild(bwoBidsNode("th",null,m));v.appendChild(S);let x=bwoBidsNode("tbody");for(let m of["1","2","3","4"])a.players.filter(d=>d.position===m).sort((d,v)=>bwoMatchdayXIValue(v)-bwoMatchdayXIValue(d)).forEach(d=>x.appendChild(bwoMatchdayXIPlayerRow(d)));d.append(v,x),y.append(w,d);let C=bwoBidsNode("aside","bwo-xi-side"),N=bwoBidsNode("section","bwo-xi-panel"),_=bwoBidsNode("header","bwo-xi-panel-head");_.append(bwoBidsNode("h2",null,"Cambios sugeridos"),bwoBidsNode("span",null,`${i.length} cambio${i.length===1?"":"s"}`));let L=bwoBidsNode("div","bwo-xi-change-list");if(!i.length)L.appendChild(bwoBidsNode("div","bwo-xi-empty","Tu once actual ya coincide con esta recomendación."));else for(let m of i){let d=bwoBidsNode("article","bwo-xi-change"),v=bwoBidsNode("div","bwo-xi-change-line");v.append(bwoBidsNode("strong",null,`${m.out?`${m.out.name} → `:"Añadir "}${m.in.name}`),bwoBidsNode("span","bwo-xi-gain",`${m.gain>=0?"+":""}${m.gain.toFixed(1)} pts`)),d.append(v,bwoBidsNode("p",null,m.reason)),L.appendChild(d)}N.append(_,L);let A=bwoBidsNode("section","bwo-xi-panel"),I=bwoBidsNode("header","bwo-xi-panel-head");I.append(bwoBidsNode("h2",null,"Roles y suplentes"),bwoBidsNode("span",null,"También se guardarán"));let D=bwoBidsNode("div","bwo-xi-alt-list");for(let[m,d,v]of[["Capitán",a.captain,"Mayor combinación de previsión, forma y fiabilidad"],["Ariete",a.striker,"Delantero con mejor expectativa de la jornada"]])if(d){let S=bwoBidsNode("article","bwo-xi-alt bwo-xi-role");S.append(bwoBidsNode("strong",null,`${m}: ${d.name}`),bwoBidsNode("span",null,d.projected==null?"—":`${Number(d.projected).toFixed(1)} pts`),bwoBidsNode("small",null,`${v} · ${Math.round(bwoMatchdayXIProbability(d))}% titular`)),D.appendChild(S)}if(!a.reserves.length)D.appendChild(bwoBidsNode("div","bwo-xi-empty","No hay suplentes disponibles."));else for(let[m,d]of a.reserves.entries()){let v=bwoBidsNode("article","bwo-xi-alt");v.append(bwoBidsNode("strong",null,`${m+1}. ${d.name}`),bwoBidsNode("span",null,d.projected==null?"—":`${Number(d.projected).toFixed(1)} pts`),bwoBidsNode("small",null,`Suplente · ${bwoMatchdayXIPosition(d.position)} · ${Math.round(bwoMatchdayXIProbability(d))}% titular`)),D.appendChild(v)}A.append(I,D),C.append(N,A),p.append(y,C);let P=bwoBidsNode("footer","bwo-xi-footer"),T=bwoBidsNode("p",null,"La propuesta usa Media AS + SofaScore, forma, disponibilidad, titularidad probable, rival y estado de la jornada. Al confirmar, guardará en Biwenger los 11 titulares, los suplentes, el capitán y el ariete."),O=bwoBidsNode("button","bwo-xi-round","Alinear y guardar");O.type="button",O.addEventListener("click",()=>bwoMatchdayXISave(a,O)),P.append(T,O),r.replaceChildren(s,f,g,p,P)}
async function bwoMatchdayXILoad(e=!1){if(!e&&bwoMatchdayXICache&&Date.now()-bwoMatchdayXICacheAt<9e4)return bwoMatchdayXICache;let t=!e&&bwoRoundNativeDataCache&&Date.now()-bwoRoundNativeDataAt<9e4?bwoRoundNativeDataCache:await bwoRoundData();if(!(t.jp instanceof Map))try{t.jp=await bwoRoundJpData(t,e)}catch{}for(let n of t.players??[])n.jp=t.jp instanceof Map?t.jp.get(String(n.id))??null:null;return bwoMatchdayXICache=t,bwoMatchdayXICacheAt=Date.now(),t}
async function bwoOpenMatchdayXI(e=!1){bwoMatchdayXIEnsureStyles();let t=document.querySelector(".bwo-tools-page-host");if(!t)return;let n=bwoBidsNode("section","bwo-xi-tool"),r=bwoBidsNode("header","bwo-xi-head"),o=bwoBidsNode("div","bwo-xi-head-main"),a=bwoBidsNode("button","bwo-xi-back","← Herramientas"),i=bwoBidsNode("div"),l=bwoBidsNode("span","bwo-xi-eyebrow","BIWTOOLS · ASISTENTE DE JORNADA"),s=bwoBidsNode("h1","bwo-xi-title","Once Jornada"),u=bwoBidsNode("p","bwo-xi-subtitle","Construye el mejor once posible para esta jornada y explica cada decisión según previsión, titularidad, estado físico, rival y bloqueos ya activos."),c=bwoBidsNode("button","bwo-xi-refresh",e?"Actualizando…":"Actualizar análisis");a.type=c.type="button",c.disabled=e,a.addEventListener("click",bwoClauseReturnTools),c.addEventListener("click",()=>bwoOpenMatchdayXI(!0)),i.append(l,s,u),o.append(a,i),r.append(o,c);let f=bwoBidsNode("main","bwo-xi-content"),g=bwoBidsNode("div","bwo-xi-empty",e?"Actualizando plantilla, jornada y probabilidades…":"Analizando tu plantilla y calculando todas las formaciones válidas…");f.appendChild(g),n.append(r,f),t.replaceChildren(n);try{let b=await bwoMatchdayXILoad(e);document.contains(n)&&bwoMatchdayXIRender(b,n)}catch(b){console.warn("[biwtools] No se pudo construir Once Jornada:",b),f.replaceChildren(bwoBidsNode("div","bwo-xi-empty","No se pudo preparar el once con la sesión actual. Espera unos segundos y pulsa Actualizar análisis; la herramienta respetará automáticamente el límite de peticiones de Biwenger."))}}
if(!bwoToolsCatalog.some(e=>e.name==="Once Jornada")){let e={icon:"🧠",name:"Once Jornada",category:"Jornada",status:"Alineación recomendada",tone:"green",featured:!0,description:"Calcula el mejor once de cada jornada con formación válida, previsión de puntos, titularidad, bajas, rival y partidos ya bloqueados.",features:["Mejor formación automática","Cambios explicados","Riesgos y alternativas"],source:"Plantilla + jornada + Media AS/SofaScore"},t=bwoToolsCatalog.findIndex(n=>n.name==="Onces");bwoToolsCatalog.splice(t<0?0:t,0,e)}
...
globalThis.__BIWTOOLS_BALANCE_FIX__="v1.7.40-matchday-xi-network-guard-rate-limit-reliable-averages-idb-repair";
...
var bwoRuntimeToken=`v1.7.40-${Date.now()}-${Math.random().toString(36).slice(2)}`;
