...
.bwo-fab-menu-version::after {
            content: "v1.7.51 Onces de liga integrados";
  font-size: 10px;
}
...
bwoDiagKpi=function(e,t,n,r=""){return bwoDiagKpiVersionBase(e,e==="Versión"?"v1.7.51":t,n,r)};
...
async function bwoMatchdayXILeagueData(){let[e,t,n,r]=await Promise.all([H(),Oo(),wf(),Do()]),o=new Map,a=new Map;for(let n of e.values()){a.set(String(n.id),n.name);let e=t.get(String(n.id))??t.get(n.id);if(!e?.titular||!n.team_id||!n.team_name||null==e.x||null==e.y)continue;let r=String(n.team_id),a=o.get(r);a||(a={teamId:r,teamName:n.team_name,players:[],alternatives:[]},o.set(r,a)),a.players.push({id:String(n.id),name:n.name,injured:Number(e.lesion)>=0,prob:e.prob,x:Number(e.x),y:Number(e.y)})}for(let e of n??[]){let t=String(e.team_id??"");if(!t)continue;let n=o.get(t);n&&n.players.push({id:null,name:e.name||"Jugador",injured:Number(e.lesion)>=0,prob:e.prob,x:Number(e.x),y:Number(e.y)})}for(let[e,t]of r??new Map){let n=o.get(String(e));n&&(n.alternatives=(t??[]).map(e=>({id:e.id?String(e.id):null,name:e.id?a.get(String(e.id))??e.name:e.name,injured:Number(e.lesion)>=0,prob:e.prob,position:e.posicion})))}return[...o.values()].sort((e,t)=>e.teamName.localeCompare(t.teamName,"es"))}
...
globalThis.__BIWTOOLS_BALANCE_FIX__="v1.7.51-native-league-lineups";
...
var bwoRuntimeToken=`v1.7.51-${Date.now()}-${Math.random().toString(36).slice(2)}`;
