...
.bwo-fab-menu-version::after {
            content: "v1.7.46 Once Jornada con consenso";
  font-size: 10px;
}
...
function bwoRecScore(e,t,n=1){let r=bwoRecConsensus(e.probability,t),o=e.recentAverage??e.seasonAverage??0,a=e.seasonAverage??e.recentAverage??0,i=e.recentAverage==null?a:e.seasonAverage==null?o:o*.62+a*.38,l=Math.max(0,Number(e.games??e.recentPoints?.length??0)),s=Math.max(1,l,Number(n)||1),u=Math.max(0,Math.min(1,l/s)),c=l===0?0:l/(l+2),f=l===0?0:i*c+3*(1-c),g=e.match?.difficulty==null?1:e.match.difficulty<=40?1.09:e.match.difficulty<=60?1:e.match.difficulty<=75?.92:.85,b=e.match?.home?1.035:.985,p=e.physical.unavailable?0:e.physical.key==="risk"?.82:1,y=(.32+.68*Math.max(0,Math.min(100,r))/100)*g*b*p,w=s<=1?1:.62+.38*u,m=l===0?0:Math.max(0,(f*y+bwoRecJPBonus(t))*w),d=e.recentPoints.length>=2?bwoRecStd(e.recentPoints):null,v=s<=2?1:Math.min(3,Math.ceil(s*.5)),S=l>=v&&u>=.5,x=l===0?"sin datos":S?u>=.8?"alta":"media":"baja";return{...e,jp:t??null,probabilityConsensus:r,rawAverage:i,baseAverage:f,possibleGames:s,appearanceRate:u,sampleWeight:c,sampleRequired:v,sampleReliable:S,sampleConfidence:x,projected:m,consistency:d==null?null:Math.max(0,10-d),rating:m+(e.priceChange>0?.12:e.priceChange<0?-.08:0)-(1-u)*.45}}
...
bwoRecRenderCard=function(e,t){bwoRecRenderCardParticipationBase(e,t);let n=[...zv.querySelectorAll(".bwo-comp")].find(a=>(a.querySelector(".bwo-comp-name")?.textContent??"").trim()==="Recomendaciones"),r=n?.querySelector(".bwo-rec-upgrade");if(!r)return;let o=bwoRecRows(e,t),a=o.slice(0,12),i=r.querySelector(".bwo-rec-upgrade-head p");i&&(i.textContent="Comparación por posición con media corregida por participación y datos consolidados. Una puntuación alta aislada no se considera una mejora fiable.");let l=[...r.querySelectorAll(".bwo-rec-kpi strong")];l[1]&&(l[1].textContent=String(o.filter(s=>s.verdict==="Mejora clara").length));let s=r.querySelector("thead th:nth-child(4)");s&&(s.textContent="Media fiable");let u=[...r.querySelectorAll("tbody tr")];u.forEach((c,f)=>{let g=a[f];if(!g)return;let b=c.querySelector(".bwo-rec-verdict");b&&(b.classList.remove("clear","maybe","watch"),b.classList.add(g.verdict==="Mejora clara"?"clear":g.verdict==="Mejora posible"?"maybe":"watch"));let p=c.querySelector("td:nth-child(3)");if(p){g.recentPoints.length||(p.textContent=g.games?`${g.games} partidos contabilizados`:"Sin partidos actuales");if(!p.querySelector(".bwo-rec-sample")){let y=bwoBidsNode("small","bwo-rec-cell-sub bwo-rec-sample",`${g.games}/${g.possibleGames} jugados · confianza ${g.sampleConfidence}`);p.appendChild(y)}}let w=c.querySelector("td:nth-child(4)");if(w){w.textContent=g.games&&Number.isFinite(g.rawAverage)?`${g.rawAverage.toFixed(1)} pts`:"—";let m=bwoBidsNode("small","bwo-rec-cell-sub",g.games?`Ajustada: ${g.baseAverage.toFixed(1)} pts`:"Sin muestra válida");w.appendChild(m)}g.sampleReliable||(c.setAttribute("data-bwo-sample-low","1"),c.style.opacity=".76")});let c=r.querySelector(".bwo-rec-refresh");if(c){let f=Math.max(0,bwoRecommendationUpgradeCooldown-(Date.now()-bwoRecommendationUpgradeAt));c.title=f?`Los datos se reutilizan para proteger el límite de Biwenger. Próxima consulta en ${Math.ceil(f/1e3)} s.`:"Actualizar los datos consolidados"}let f=r.querySelector(".bwo-rec-upgrade-foot");f&&(f.textContent="La media observada se ajusta hacia 3 puntos cuando hay pocos partidos. La previsión penaliza una participación inferior al 50% de las jornadas disponibles; con una sola aparición no se etiqueta como mejora. Los informes ya consultados se reutilizan desde caché y la actualización se limita a una cada 2 minutos para no saturar Biwenger.")};
...
bwoDiagKpi=function(e,t,n,r=""){return bwoDiagKpiVersionBase(e,e==="Versión"?"v1.7.46":t,n,r)};
...
globalThis.__BIWTOOLS_BALANCE_FIX__="v1.7.46-matchday-xi-four-source-consensus-role-ranking-cache";
...
var bwoRuntimeToken=`v1.7.46-${Date.now()}-${Math.random().toString(36).slice(2)}`;
