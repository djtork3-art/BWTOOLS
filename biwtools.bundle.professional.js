...
.bwo-fab-menu-version::after {
            content: "v1.9.0 Suite de decisiones y estabilidad";
  font-size: 10px;
}
...
var bwoToolsPageWanted=!1,bwoToolsPagePath=null,bwoToolsExitBound=!1,bwoToolsActiveTool=null,bwoToolsEmbedBusy=!1,bwoToolsRows=[
  ...
  {icon:"🧠",name:"Once",description:"Optimiza tu alineación, suplentes y roles, y consulta los onces utilizados por los equipos de la liga."},
...
function bwoRecommendationPrefillNativeBid(e){let t=Number(e.recommendedBid??e.suggestedBid??0),n=Date.now(),r=()=>{let o=document.querySelector('modal-dialog[aria-label^="Pujar por"]')??[...document.querySelectorAll("modal-dialog")].find(l=>/pujar|oferta/i.test(l.getAttribute("aria-label")??l.textContent??"")),a=o?.querySelector('input-int[name="bid"] input, input[name="bid"], input[type="number"], input[inputmode="numeric"]');if(a){if(t>0){Q0(a,Math.round(t));let i=o.querySelector(".bwo-rec-native-bid-note");i||(i=bwoBidsNode("div","bwo-rec-native-bid-note"),Object.assign(i.style,{margin:"8px 0",padding:"8px 10px",border:"1px solid rgba(95,203,114,.35)",borderRadius:"8px",color:"#baf3c5",background:"rgba(95,203,114,.1)",font:"700 11px/1.4 Open Sans,system-ui,sans-serif"}),a.closest("input-int")?.insertAdjacentElement("afterend",i)),i&&(i.textContent=`Biwtools recomienda ${bwoBidsMoney(t)}. Importe rellenado; revisa y confirma manualmente.`),a.focus(),a.select?.(),bwoRecommendationToast(`Puja de ${e.name} abierta con ${bwoBidsMoney(t)}. Solo falta que la revises y confirmes.`,"ok")}else bwoRecommendationToast(`Abierta la puja de ${e.name}. Introduce el importe y confirma manualmente.`,"ok");return}Date.now()-n<5e3?setTimeout(r,100):bwoRecommendationToast(`Se abrió ${e.name}, pero no se pudo rellenar automáticamente el importe.`)};setTimeout(r,80)}
function bwoRecommendationOpenNativeBid(e){bwoRecommendationMarketTarget=e,bwoCloseToolsPage();let t=Date.now(),n=()=>{if(Date.now()-t>12e3){bwoRecommendationToast(`${e.name} figura en el mercado, pero su tarjeta no se ha podido localizar. Restablece los filtros del Mercado y vuelve a intentarlo.`);return}if(!location.pathname.includes("/market")){setTimeout(n,180);return}let r=bwoRecommendationListingMatch(S0(),e);if(!r){setTimeout(n,180);return}let o=E0(r)??r.markEl;o&&(o.classList.add("bwo-recommendation-target"),Object.assign(o.style,{outline:"3px solid rgba(95,203,114,.85)",outlineOffset:"3px",boxShadow:"0 0 0 7px rgba(95,203,114,.14)"}),o.scrollIntoView({behavior:"smooth",block:"center"})),setTimeout(()=>{let a=r.pujarEl instanceof Element?r.pujarEl:null;a?(a.click(),bwoRecommendationPrefillNativeBid(e)):bwoRecommendationToast(`Se encontró ${e.name}, pero Biwenger no muestra ahora el botón Pujar.`)},450),bwoRecommendationMarketTarget=null};setTimeout(()=>{let r=document.querySelector("app-nav a[href='/market']")??document.querySelector("a[href='/market']");r?r.click():bwoRecommendationToast("No se encontró el acceso nativo a Mercado; ábrelo desde la barra superior."),setTimeout(n,180)},120)}
...
var bwoRecommendationUpgradeBusy=!1,bwoRecommendationUpgradeCache=null,bwoRecommendationUpgradeAt=0,bwoRecommendationUpgradeJPAt=0,bwoRecommendationUpgradeCooldown=12e4;
...
function bwoRecScore(e,t,n=1){let r=bwoRecConsensus(e.probability,t),o=e.recentAverage??e.seasonAverage??0,a=e.seasonAverage??e.recentAverage??0,i=e.recentAverage==null?a:e.seasonAverage==null?o:o*.62+a*.38,l=Math.max(0,Number(e.games??e.recentPoints?.length??0)),s=Math.max(1,l,Number(n)||1),u=Math.max(0,Math.min(1,l/s)),c=l===0?0:l/(l+2),f=l===0?0:i*c+3*(1-c),h=l===0?0:f*u,g=e.match?.difficulty==null?1:e.match.difficulty<=40?1.09:e.match.difficulty<=60?1:e.match.difficulty<=75?.92:.85,b=e.match?.home?1.035:.985,p=e.physical.unavailable?0:e.physical.key==="risk"?.82:1,y=(.32+.68*Math.max(0,Math.min(100,r))/100)*g*b*p,w=s<=1?1:.62+.38*u,m=l===0?0:Math.max(0,(f*y+bwoRecJPBonus(t))*w),d=e.recentPoints.length>=2?bwoRecStd(e.recentPoints):null,v=s<=2?1:Math.min(3,Math.ceil(s*.5)),S=l>=v&&u>=.5,x=l===0?"sin datos":S?u>=.8?"alta":"media":"baja";return{...e,jp:t??null,probabilityConsensus:r,rawAverage:i,baseAverage:f,reliableRoundAverage:h,possibleGames:s,appearanceRate:u,sampleWeight:c,sampleRequired:v,sampleReliable:S,sampleConfidence:x,projected:m,consistency:d==null?null:Math.max(0,10-d),rating:m*.82+h*.18+(e.priceChange>0?.12:e.priceChange<0?-.08:0)-(1-u)*.25}}
async function bwoRecProfile(e,t,n,r,o,a){let i=bwoCatalogPlayer(t,e);if(!i)return null;let[l,s]=await Promise.all([pe(e).catch(()=>null),Et(e).catch(()=>null)]),u=Pi instanceof Map&&Pi.has(String(e))?Pi.get(String(e)):[],c=Array.isArray(u)?[...u].sort((w,m)=>Date.parse(m.date)-Date.parse(w.date)):[],f=c.map(w=>bwoRecReportPoints(w,n,r)).filter(Number.isFinite),g=f.slice(0,5),b=bwoRecMean(g),p=bn(i,n,c,r),y=bwoRoundPhysical(i,o.get(String(e)),l),v=i.last_season_is_current===!1?0:Math.max(0,Number(i.last_season_games??0)),S=Math.max(f.length,v);return{id:String(e),catalog:i,name:i.name,position:String(i.position),teamId:String(i.team_id??""),teamName:i.team_name??"",price:Number(i.price??0),priceChange:Number(i.price_increment??0),physical:y,probability:bwoRoundProbability(l,s),match:a.get(String(i.team_id))??{opponentName:"Rival por confirmar",home:!1,date:null,state:"upcoming",stateLabel:"Pendiente",difficulty:null},recentPoints:g,recentAverage:b,seasonAverage:p,games:S,reportSource:f.length?"cache":"catalog"}}
async function bwoRecBuildData(e=!1){if(bwoRecommendationUpgradeCache&&Date.now()-bwoRecommendationUpgradeAt<12e4)return bwoRecommendationUpgradeCache;let[t,n,r,o,a,i,l,s,u,c]=await Promise.all([H(),bwoFetchLiveMarket(e).catch(()=>null),gn().catch(()=>[]),vn().catch(()=>[]),Lr().catch(()=>[]),hn().catch(()=>[]),pn().catch(()=>null),Oe().catch(()=>null),yn().catch(()=>null),xf().catch(()=>[])]),f=bwoMarketListings(n,t,[]),g=new Set((a??[]).map(String)),b=f.filter(I=>!g.has(String(I.id))&&bwoCatalogPlayer(t,String(I.id))),p=((r?.length?r:o)??[]).map(String),y=new Set([...p,...g,...b.map(I=>String(I.id))]),w=bwoRoundInjuries(c);await Mi().catch(()=>null);let m=[...new Set([...y].map(I=>bwoCatalogPlayer(t,I)?.team_id).filter(Boolean).map(String))],d=await bwoRoundMatches(t,m).catch(()=>new Map),v=await bwoRoundMapLimit([...y],4,I=>bwoRecProfile(String(I),t,s,u,w,d)),S=new Map(v.filter(Boolean).map(I=>[I.id,I])),x=p.map(I=>S.get(I)).filter(Boolean),C=[...g].map(I=>S.get(I)).filter(Boolean),N=new Set(p),_=C.filter(I=>!N.has(String(I.id))),L=b.map(I=>{let M=S.get(String(I.id));return M?{...M,price:Number(I.price??M.price)}:null}).filter(Boolean),A=(i??[]).find(I=>String(I.id)===String(l))??{balance:0,maxBid:0},D={catalog:t,scoreId:s,formula:u,balance:Number(A.balance??0),maxBid:Number(A.maxBid??0),own:x,squad:C,bench:_,candidates:L,listings:b,jp:new Map};bwoRecommendationUpgradeCache=D,bwoRecommendationUpgradeAt=Date.now();return D}
function bwoRecRoundBid(e,t=1e4,n="nearest"){let r=Math.max(0,Number(e)||0),o=Math.max(1,Number(t)||1);return n==="down"?Math.floor(r/o)*o:n==="up"?Math.ceil(r/o)*o:Math.round(r/o)*o}
function bwoRecBidPlan(e,t){let n=Math.max(0,Number(e.price)||0),r=Math.max(0,Number(t.maxBid)||0),o=Math.max(-.05,Math.min(.14,(Number(e.gain)||0)*.018)),a=Math.max(-.025,Math.min(.075,(Number(e.probabilityConsensus??50)-55)/600)),i=Math.max(-.025,Math.min(.045,(Number(e.appearanceRate??0)-.55)*.09)),l=n>0?Math.max(-.025,Math.min(.04,Number(e.priceChange||0)/n*.5)):0,s=e.sampleReliable?.025:-.025,u=Math.max(0,Math.min(.13,.015+o*.45+a*.35+i*.35+l+s)),c=Math.max(.02,Math.min(.22,.035+o+a+i+l+s)),f=bwoRecRoundBid(n*(1+u)),g=bwoRecRoundBid(n*(1+c));r>0&&(g=Math.min(g,bwoRecRoundBid(r,1e4,"down")),f=Math.min(f,g));let b=r>=n&&g>=n;return{suggestedBid:Math.max(n,f),recommendedBid:b?Math.max(n,g):null,bidPremiumSuggested:n?Math.max(0,(Math.max(n,f)-n)/n):0,bidPremiumRecommended:n&&b?Math.max(0,(Math.max(n,g)-n)/n):null,bidReason:!b?"Fuera de tu puja máxima":e.sampleReliable?"Rendimiento, titularidad, regularidad y tendencia":"Importe conservador por muestra limitada"}}
function bwoRecRows(e,t){let n=Math.max(1,...[...e.own,...e.candidates].map(a=>Number(a.games??0))),r=e.own.map(a=>bwoRecScore(a,t.get(a.id),n)),o=e.candidates.map(a=>bwoRecScore(a,t.get(a.id),n)),a=o.map(i=>{let l=r.filter(s=>s.position===i.position).sort((s,u)=>s.rating-u.rating)[0]??null,c=l?i.rating-l.rating:null,f=i.price<=e.maxBid,g=f?e.balance<0?"Pujable por puja máx.":"Pujable ahora":`Faltan ${bwoRecMoneyShort(i.price-e.maxBid)}`,b=!i.sampleReliable?"Muestra insuficiente":c==null?"Refuerzo":c>=1.5?"Mejora clara":c>=.35?"Mejora posible":"Seguimiento",p=bwoRecBidPlan({...i,gain:c},e);return{...i,...p,out:l,gain:c,affordable:f,economy:g,verdict:b}});return a.sort((i,l)=>Number(l.sampleReliable)-Number(i.sampleReliable)||(l.gain??-99)-(i.gain??-99)||l.rating-i.rating||i.price-l.price)}
...
function bwoRecEnsureUpgradeStyles(){if(zv.querySelector("#bwo-rec-upgrade-styles"))return;let e=document.createElement("style");e.id="bwo-rec-upgrade-styles",e.textContent=`
 ...
 .bwo-rec-player{display:flex;align-items:center;gap:10px;min-width:0;cursor:pointer}.bwo-rec-player img{flex:0 0 auto;width:38px;height:38px;border-radius:50%;object-fit:cover;background:#202227}.bwo-rec-player strong{display:block;color:#fff;font-size:13px;line-height:1.25}.bwo-rec-player small,.bwo-rec-cell-sub{display:block;margin-top:3px;color:#aeb5c2;font-size:9px;line-height:1.35}.bwo-rec-verdict,.bwo-rec-economy{display:inline-flex;padding:4px 7px;border-radius:999px;font-size:9px;line-height:1.25;font-weight:900;white-space:nowrap}.bwo-rec-verdict.clear,.bwo-rec-economy.ok{color:#9beaab;background:rgba(95,203,114,.13)}.bwo-rec-verdict.maybe{color:#f0cf82;background:rgba(216,166,70,.14)}.bwo-rec-verdict.watch,.bwo-rec-economy.no{color:#ff9b8b;background:rgba(226,102,75,.13)}.bwo-rec-gain{color:var(--bwo-green);font-weight:900}.bwo-rec-bid-plan{display:grid;gap:2px;margin-top:7px;padding-top:6px;border-top:1px solid rgba(255,255,255,.08);font-size:9px;line-height:1.3;white-space:nowrap}.bwo-rec-bid-plan span{color:#aeb5c2}.bwo-rec-bid-plan strong{color:#9beaab;font-size:10px}.bwo-rec-bid-plan small{color:#858e9b;font-size:8px;white-space:normal}.bwo-rec-form{font-variant-numeric:tabular-nums;white-space:nowrap}.bwo-rec-form b{display:inline-block;min-width:22px;margin-right:3px;padding:3px 4px;border-radius:5px;background:#292e38;color:#fff;text-align:center}.bwo-rec-upgrade-action{padding:8px 10px;border:1px solid rgba(95,203,114,.45);border-radius:8px;color:#9beaab;background:rgba(95,203,114,.12);cursor:pointer;font:900 10px/1.2 var(--bwo-font-body);white-space:nowrap}.bwo-rec-upgrade-foot{margin:12px 2px 0;color:var(--bwo-muted);font-size:10px;line-height:1.55}
 .bwo-rec-bench{margin-top:16px;padding:14px;border:1px solid rgba(93,153,255,.2);border-radius:12px;background:linear-gradient(145deg,rgba(26,32,43,.94),rgba(15,18,24,.96))}.bwo-rec-bench-head{display:flex;align-items:flex-start;justify-content:space-between;gap:12px;margin-bottom:11px}.bwo-rec-bench-head h3{margin:0;color:#fff;font-size:16px}.bwo-rec-bench-head p{max-width:850px;margin:5px 0 0;color:#aeb5c2;font-size:10px;line-height:1.5}.bwo-rec-bench-count{padding:5px 8px;border-radius:999px;color:#a9c7ff;background:rgba(93,153,255,.12);font-size:9px;font-weight:900;white-space:nowrap}.bwo-rec-bench-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:9px}.bwo-rec-bench-card{display:flex;min-width:0;flex-direction:column;gap:8px;padding:11px;border:1px solid rgba(255,255,255,.075);border-radius:10px;background:rgba(0,0,0,.2)}.bwo-rec-bench-top{display:flex;align-items:center;gap:8px}.bwo-rec-bench-top img{width:36px;height:36px;flex:0 0 auto;border-radius:50%;object-fit:cover;background:#242832}.bwo-rec-bench-top strong{display:block;overflow:hidden;color:#fff;font-size:12px;text-overflow:ellipsis;white-space:nowrap}.bwo-rec-bench-top small{display:block;margin-top:2px;color:#8f98a7;font-size:8px}.bwo-rec-bench-badge{margin-left:auto;padding:4px 6px;border-radius:999px;color:#a9efb6;background:rgba(101,212,122,.12);font-size:8px;font-weight:900;white-space:nowrap}.bwo-rec-bench-badge.watch{color:#f0cf82;background:rgba(216,166,70,.14)}.bwo-rec-bench-metrics{display:grid;grid-template-columns:repeat(3,1fr);gap:5px}.bwo-rec-bench-metrics div{padding:6px;border-radius:7px;background:rgba(255,255,255,.035)}.bwo-rec-bench-metrics span{display:block;color:#7f8998;font-size:7px;text-transform:uppercase}.bwo-rec-bench-metrics strong{display:block;margin-top:2px;color:#fff;font-size:10px}.bwo-rec-bench-reason{margin:0;color:#aeb5c2;font-size:9px;line-height:1.45}.bwo-rec-bench-foot{display:flex;align-items:center;justify-content:space-between;gap:8px;margin-top:auto}.bwo-rec-bench-foot span{color:#9beaab;font-size:9px;font-weight:900}.bwo-rec-bench-foot button{padding:6px 8px;border:1px solid rgba(95,203,114,.4);border-radius:7px;color:#9beaab;background:rgba(95,203,114,.1);cursor:pointer;font:900 8px var(--bwo-font-body)}
 @media(max-width:900px){.bwo-rec-upgrade{padding:12px!important}.bwo-rec-kpis{grid-template-columns:repeat(2,minmax(0,1fr))}.bwo-rec-upgrade-head{flex-direction:column}.bwo-rec-upgrade-table{min-width:1180px}.bwo-rec-upgrade-table td{font-size:11px}.bwo-rec-bench-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}
 @media(max-width:620px){.bwo-rec-bench-grid{grid-template-columns:1fr}.bwo-rec-bench-head{flex-direction:column}}
`;zv.appendChild(e)}
...
function bwoRecBenchRows(e,t){let n=Math.max(1,...[...(e.squad??e.own??[]),...(e.candidates??[])].map(l=>Number(l.games??0))),r=(e.bench??[]).map(l=>bwoRecScore(l,t.get(String(l.id)),n)),o=(e.candidates??[]).map(l=>bwoRecScore(l,t.get(String(l.id)),n)),a=o.map(l=>{let s=r.filter(u=>u.position===l.position).sort((u,c)=>u.rating-c.rating)[0]??null,f=s?l.reliableRoundAverage-s.reliableRoundAverage:l.reliableRoundAverage,g=l.price>0?l.priceChange/l.price:0,b=Math.max(-.4,Math.min(.65,g*8)),p=l.reliableRoundAverage*.5+l.projected*.25+l.probabilityConsensus/100*1.4+b+(s?0:.35),y=l.price<=e.maxBid&&!l.physical?.unavailable,w=y&&l.sampleReliable&&l.appearanceRate>=.6&&l.probabilityConsensus>=55&&f>=.35?"Merece invertir":y&&l.appearanceRate>=.45&&f>=0?"Opción de futuro":"Solo seguimiento",m=bwoRecBidPlan({...l,gain:f},e),d=s?`Mejora a ${s.name} en ${f>=0?"+":""}${f.toFixed(2)} pts/J fiables.`:`Añade cobertura ${({1:"de portería",2:"defensiva",3:"de medio",4:"de ataque"})[l.position]??"posicional"} al banquillo.`;return{...l,...m,benchReference:s,benchGain:f,futureScore:p,benchVerdict:w,benchReason:`${d} ${l.appearanceRate>=.75?"Participación sólida.":l.appearanceRate>=.5?"Participación aceptable.":"Participación todavía limitada."}`}});return a.sort((l,s)=>Number(s.benchVerdict==="Merece invertir")-Number(l.benchVerdict==="Merece invertir")||s.futureScore-l.futureScore||s.benchGain-l.benchGain).slice(0,6)}
function bwoRecRenderBench(e,t,n){n.querySelector(".bwo-rec-bench")?.remove();let r=bwoRecBenchRows(e,t);if(!r.length)return;let o=bwoBidsNode("section","bwo-rec-bench"),a=bwoBidsNode("header","bwo-rec-bench-head"),i=bwoBidsNode("div");i.append(bwoBidsNode("h3",null,"Mejoras de banquillo y futuro"),bwoBidsNode("p",null,"Opciones del mercado que pueden reforzar tus suplentes o ganar valor deportivo con el tiempo, separadas de las mejoras inmediatas del once."));let l=r.filter(g=>g.benchVerdict==="Merece invertir").length;a.append(i,bwoBidsNode("span","bwo-rec-bench-count",`${l} inversión${l===1?"":"es"} recomendable${l===1?"":"s"}`));let s=bwoBidsNode("div","bwo-rec-bench-grid");for(let g of r){let b=bwoBidsNode("article","bwo-rec-bench-card"),p=bwoBidsNode("div","bwo-rec-bench-top"),y=document.createElement("img");y.src=`https://cdn.biwenger.com/i/p/${g.id}.png`,y.alt="",y.addEventListener("error",()=>y.style.visibility="hidden");let w=bwoBidsNode("div"),m=bwoBidsNode("strong",null,g.name),d=bwoBidsNode("small",null,`${({1:"PT",2:"DF",3:"MC",4:"DL"})[g.position]??g.position} · ${g.teamName||"Equipo sin dato"}`),v=bwoBidsNode("span",`bwo-rec-bench-badge${g.benchVerdict==="Solo seguimiento"?" watch":""}`,g.benchVerdict);w.append(m,d),p.append(y,w,v);let S=bwoBidsNode("div","bwo-rec-bench-metrics");for(let[C,N]of[["Media fiable",`${g.reliableRoundAverage.toFixed(2)} pts/J`],["Participación",`${g.games}/${g.possibleGames}`],["Tendencia",`${g.priceChange>=0?"▲":"▼"}${bwoRecMoneyShort(Math.abs(g.priceChange))}`]]){let _=bwoBidsNode("div");_.append(bwoBidsNode("span",null,C),bwoBidsNode("strong",null,N)),S.appendChild(_)}let x=bwoBidsNode("div","bwo-rec-bench-foot"),C=bwoBidsNode("span",null,g.recommendedBid?`Puja: ${bwoBidsMoney(g.recommendedBid)}`:`Valor: ${bwoBidsMoney(g.price)}`),N=bwoBidsNode("button",null,g.recommendedBid?"Pujar":"Ver puja");N.type="button",N.addEventListener("click",()=>bwoRecommendationOpenNativeBid({id:g.id,name:g.name,price:g.price,position:g.position,suggestedBid:g.suggestedBid,recommendedBid:g.recommendedBid})),x.append(C,N),b.append(p,S,bwoBidsNode("p","bwo-rec-bench-reason",g.benchReason),x),s.appendChild(b)}o.append(a,s);let u=n.querySelector(".bwo-rec-upgrade-foot");u?n.insertBefore(o,u):n.appendChild(o)}
function bwoRecFinite(e){let t=Number(e);return Number.isFinite(t)?t:0}
function bwoRecEnsureInvestmentStyles(){if(zv.querySelector("#bwo-rec-investment-styles"))return;let e=document.createElement("style");e.id="bwo-rec-investment-styles",e.textContent=`
 .bwo-rec-upgrade-wrap{scrollbar-gutter:stable;padding-bottom:4px}.bwo-rec-upgrade-table th:nth-child(9),.bwo-rec-upgrade-table td:nth-child(9){position:sticky;right:0;z-index:3;min-width:138px;box-shadow:-8px 0 14px rgba(0,0,0,.26)}.bwo-rec-upgrade-table th:nth-child(9){z-index:5;background:#1d2025}.bwo-rec-upgrade-table td:nth-child(9){background:#14171c}.bwo-rec-upgrade-table tr:hover td:nth-child(9){background:#1c2026}.bwo-rec-upgrade-action{display:block;width:100%;box-sizing:border-box;white-space:normal}.bwo-rec-bench-foot{flex-wrap:wrap}.bwo-rec-bench-foot button{min-width:92px;flex:0 0 auto}.bwo-rec-bench-badge.profit{color:#83f1a2;background:rgba(52,190,91,.16)}.bwo-rec-bench-badge.mixed{color:#91c5ff;background:rgba(72,137,225,.16)}.bwo-rec-investment{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:5px;padding-top:8px;border-top:1px solid rgba(255,255,255,.07)}.bwo-rec-investment div{min-width:0;padding:7px;border-radius:7px;background:rgba(95,203,114,.055)}.bwo-rec-investment span{display:block;color:#7f8998;font-size:7px;text-transform:uppercase}.bwo-rec-investment strong{display:block;margin-top:2px;color:#9beaab;font-size:10px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.bwo-rec-invest-note{margin:10px 1px 0;color:#7f8998;font-size:8px;line-height:1.45}
 @media(max-width:620px){.bwo-rec-upgrade-action{min-height:38px}.bwo-rec-investment{grid-template-columns:1fr 1fr}.bwo-rec-bench-foot{align-items:stretch;flex-direction:column}.bwo-rec-bench-foot button{width:100%}}
 `,zv.appendChild(e)}
function bwoRecBenchInvestmentRows(e,t){let n=Math.max(1,...[...(e.squad??e.own??[]),...(e.candidates??[])].map(g=>Number(g.games??0))),r=(e.bench??[]).map(g=>bwoRecScore(g,t.get(String(g.id)),n)),o=(e.candidates??[]).map(g=>bwoRecScore(g,t.get(String(g.id)),n));return o.map(g=>{let h=r.filter(v=>v.position===g.position).sort((v,S)=>v.rating-S.rating)[0]??null,E=h?g.reliableRoundAverage-h.reliableRoundAverage:g.reliableRoundAverage,k=Number(g.priceChange)||0,b=g.price>0?k/g.price:0,p=Math.max(-.4,Math.min(.65,b*8)),y=g.reliableRoundAverage*.5+g.projected*.25+g.probabilityConsensus/100*1.4+p+(h?0:.35),w=g.price<=e.maxBid&&!g.physical?.unavailable,m=w&&g.sampleReliable&&g.appearanceRate>=.6&&g.probabilityConsensus>=55&&E>=.35?"Merece invertir":w&&g.appearanceRate>=.45&&E>=0?"Opción de futuro":"Solo seguimiento",d=bwoRecBidPlan({...g,gain:E},e),v=Number(d.recommendedBid||d.suggestedBid||g.price)||0,S=k>=5e4?3:k>=15e3?5:7,x=Math.max(.35,Math.min(1,.42+g.appearanceRate*.2+g.probabilityConsensus/100*.18+Math.min(8,Math.max(0,g.reliableRoundAverage))/8*.2)),C=k>0?Math.round(Math.min(g.price*.18,k*S*x)):0,N=Math.max(0,C-Math.max(0,v-g.price)),_=v>0?N/v:0,L=w&&k>0&&N>=5e4&&_>=.015&&g.appearanceRate>=.35?"Compra para reventa":w&&m!=="Solo seguimiento"&&k>=0?"Inversión mixta":m==="Merece invertir"?"Refuerzo de banquillo":"Solo seguimiento",A=h?`Mejora a ${h.name} en ${E>=0?"+":""}${E.toFixed(2)} pts/J fiables.`:`Añade cobertura ${({1:"de portería",2:"defensiva",3:"de medio",4:"de ataque"})[g.position]??"posicional"} al banquillo.`,I=k>0?`Sube ${bwoBidsMoney(k)} por actualización; si mantiene la tendencia, el margen neto estimado es ${bwoBidsMoney(N)}.`:k<0?`Pierde ${bwoBidsMoney(Math.abs(k))} por actualización: no es una compra de reventa ahora mismo.`:"Valor estable: prima el uso deportivo sobre la plusvalía.";return{...g,...d,benchReference:h,benchGain:E,futureScore:y,benchVerdict:m,benchReason:`${A} ${g.appearanceRate>=.75?"Participación sólida.":g.appearanceRate>=.5?"Participación aceptable.":"Participación todavía limitada."}`,investmentVerdict:L,investmentEntry:v,investmentTarget:g.price+C,investmentProfit:N,investmentRoi:_,investmentHorizon:`${S}–${S+3} días`,investmentReason:I,investmentScore:(L==="Compra para reventa"?8:L==="Inversión mixta"?4:0)+N/1e5+_*10+y}}).sort((g,h)=>h.investmentScore-g.investmentScore||h.futureScore-g.futureScore).slice(0,8)}
bwoRecBenchRows=bwoRecBenchInvestmentRows;
bwoRecRenderBench=function(e,t,n){bwoRecEnsureInvestmentStyles(),n.querySelector(".bwo-rec-bench")?.remove();let r=bwoRecBenchRows(e,t);if(!r.length)return;let o=bwoBidsNode("section","bwo-rec-bench"),a=bwoBidsNode("header","bwo-rec-bench-head"),i=bwoBidsNode("div");i.append(bwoBidsNode("h3",null,"Banquillo, futuro e inversiones"),bwoBidsNode("p",null,"Separa los refuerzos deportivos de las compras para reventa. La estimación de venta usa tendencia actual, coste de entrada, participación, titularidad y fiabilidad."));let l=r.filter(g=>g.investmentVerdict==="Compra para reventa").length;a.append(i,bwoBidsNode("span","bwo-rec-bench-count",`${l} oportunidad${l===1?"":"es"} de reventa`));let s=bwoBidsNode("div","bwo-rec-bench-grid");for(let g of r){let h=bwoBidsNode("article","bwo-rec-bench-card"),E=bwoBidsNode("div","bwo-rec-bench-top"),k=document.createElement("img");k.src=`https://cdn.biwenger.com/i/p/${g.id}.png`,k.alt="",k.addEventListener("error",()=>k.style.visibility="hidden");let b=bwoBidsNode("div"),p=bwoBidsNode("strong",null,g.name),y=bwoBidsNode("small",null,`${({1:"PT",2:"DF",3:"MC",4:"DL"})[g.position]??g.position} · ${g.teamName||"Equipo sin dato"}`),w=g.investmentVerdict==="Compra para reventa"?" profit":g.investmentVerdict==="Inversión mixta"?" mixed":g.investmentVerdict==="Solo seguimiento"?" watch":"",m=bwoBidsNode("span",`bwo-rec-bench-badge${w}`,g.investmentVerdict);b.append(p,y),E.append(k,b,m);let d=bwoBidsNode("div","bwo-rec-bench-metrics");for(let[v,S]of[["Media fiable",`${g.reliableRoundAverage.toFixed(2)} pts/J`],["Participación",`${g.games}/${g.possibleGames}`],["Tendencia",`${g.priceChange>=0?"▲":"▼"}${bwoRecMoneyShort(Math.abs(g.priceChange))}`]]){let x=bwoBidsNode("div");x.append(bwoBidsNode("span",null,v),bwoBidsNode("strong",null,S)),d.appendChild(x)}let v=bwoBidsNode("div","bwo-rec-investment");for(let[S,x]of[["Compra estimada",g.investmentEntry?bwoBidsMoney(g.investmentEntry):"No viable"],["Venta objetivo",g.investmentTarget>bwoRecFinite(g.price)?bwoBidsMoney(g.investmentTarget):"Sin subida"],["Beneficio estimado",g.investmentProfit?`+${bwoBidsMoney(g.investmentProfit)}`:"0 €"],["Rentabilidad",g.investmentProfit?`${(g.investmentRoi*100).toFixed(1)}%`:"0%"],["Horizonte",g.investmentHorizon],["Uso deportivo",g.benchVerdict]]){let C=bwoBidsNode("div");C.append(bwoBidsNode("span",null,S),bwoBidsNode("strong",null,x)),v.appendChild(C)}let S=bwoBidsNode("div","bwo-rec-bench-foot"),x=bwoBidsNode("span",null,g.recommendedBid?`Puja: ${bwoBidsMoney(g.recommendedBid)}`:`Valor: ${bwoBidsMoney(g.price)}`),C=bwoBidsNode("button",null,g.recommendedBid?"Pujar ahora":"Abrir puja");C.type="button",C.addEventListener("click",()=>bwoRecommendationOpenNativeBid({id:g.id,name:g.name,price:g.price,position:g.position,suggestedBid:g.suggestedBid,recommendedBid:g.recommendedBid})),S.append(x,C),h.append(E,d,v,bwoBidsNode("p","bwo-rec-bench-reason",`${g.benchReason} ${g.investmentReason}`),S),s.appendChild(h)}o.append(a,s,bwoBidsNode("p","bwo-rec-invest-note","La venta objetivo y el beneficio son proyecciones conservadoras si continúa la tendencia actual; no garantizan una subida ni una venta concreta. Revisa el mercado cada día."));let u=n.querySelector(".bwo-rec-upgrade-foot");u?n.insertBefore(o,u):n.appendChild(o)};
var bwoRecRenderCardParticipationBase=bwoRecRenderCard;
bwoRecRenderCard=function(e,t){bwoRecRenderCardParticipationBase(e,t);let n=[...zv.querySelectorAll(".bwo-comp")].find(a=>(a.querySelector(".bwo-comp-name")?.textContent??"").trim()==="Recomendaciones"),r=n?.querySelector(".bwo-rec-upgrade");if(!r)return;let o=bwoRecRows(e,t),a=o.slice(0,12),i=r.querySelector(".bwo-rec-upgrade-head p");i&&(i.textContent="Comparación por posición con media fiable por jornada, participación real y una puja calculada según rendimiento, titularidad, regularidad, tendencia y presupuesto.");let l=[...r.querySelectorAll(".bwo-rec-kpi strong")];l[1]&&(l[1].textContent=String(o.filter(s=>s.verdict==="Mejora clara").length));let s=r.querySelector("thead th:nth-child(4)");s&&(s.textContent="Media fiable / jornada");let u=[...r.querySelectorAll("tbody tr")];u.forEach((c,f)=>{let g=a[f];if(!g)return;let b=c.querySelector(".bwo-rec-verdict");b&&(b.classList.remove("clear","maybe","watch"),b.classList.add(g.verdict==="Mejora clara"?"clear":g.verdict==="Mejora posible"?"maybe":"watch"));let p=c.querySelector("td:nth-child(3)");if(p){g.recentPoints.length||(p.textContent=g.games?`${g.games} partidos contabilizados`:"Sin partidos actuales");if(!p.querySelector(".bwo-rec-sample")){let y=bwoBidsNode("small","bwo-rec-cell-sub bwo-rec-sample",`${g.games}/${g.possibleGames} jugados · confianza ${g.sampleConfidence}`);p.appendChild(y)}}let w=c.querySelector("td:nth-child(4)");if(w){w.textContent=g.games&&Number.isFinite(g.reliableRoundAverage)?`${g.reliableRoundAverage.toFixed(2)} pts/J`:"—";let m=bwoBidsNode("small","bwo-rec-cell-sub",g.games?`Cuando juega: ${g.rawAverage.toFixed(1)} pts · ${g.games}/${g.possibleGames} jornadas`:"Sin muestra válida");w.appendChild(m)}let d=c.querySelector("td:nth-child(8)");if(d&&!d.querySelector(".bwo-rec-bid-plan")){let v=bwoBidsNode("div","bwo-rec-bid-plan"),S=bwoBidsNode("span",null,`Sugerida: ${bwoBidsMoney(g.suggestedBid)}`),x=bwoBidsNode("strong",null,g.recommendedBid?`Recomendada: ${bwoBidsMoney(g.recommendedBid)}`:"Sin puja viable"),C=bwoBidsNode("small",null,g.bidReason);v.append(S,x,C),d.appendChild(v)}let v=c.querySelector("td:nth-child(9) .bwo-rec-upgrade-action");if(v){let S=v.cloneNode(!0);S.textContent=g.recommendedBid?"Pujar recomendado":"Ver puja",S.title=g.recommendedBid?`Abrir la puja y rellenar ${bwoBidsMoney(g.recommendedBid)}. No se confirmará automáticamente.`:"Abrir la ventana nativa de puja",S.addEventListener("click",()=>bwoRecommendationOpenNativeBid({id:g.id,name:g.name,price:g.price,position:g.position,suggestedBid:g.suggestedBid,recommendedBid:g.recommendedBid})),v.replaceWith(S)}g.sampleReliable||(c.setAttribute("data-bwo-sample-low","1"),c.style.opacity=".76")});bwoRecRenderBench(e,t,r);let c=r.querySelector(".bwo-rec-refresh");if(c){let f=Math.max(0,bwoRecommendationUpgradeCooldown-(Date.now()-bwoRecommendationUpgradeAt));c.title=f?`Los datos se reutilizan para proteger el límite de Biwenger. Próxima consulta en ${Math.ceil(f/1e3)} s.`:"Actualizar los datos consolidados"}let f=r.querySelector(".bwo-rec-upgrade-foot");f&&(f.textContent="Media fiable/jornada: las ausencias cuentan como 0 y las muestras pequeñas se corrigen de forma conservadora. La puja sugerida limita el sobreprecio; la recomendada pondera mejora frente a tu plantilla, titularidad, participación, tendencia y puja máxima. El bloque de futuro compara el mercado con tus suplentes reales. El botón abre Biwenger con el importe rellenado, pero nunca confirma la oferta automáticamente.")};
async function bwoRecommendationRenderUpgrade(e=!1){let t=[...zv.querySelectorAll(".bwo-comp")].find(r=>(r.querySelector(".bwo-comp-name")?.textContent??"").trim()==="Recomendaciones");if(!t||bwoRecommendationUpgradeBusy)return;bwoRecommendationUpgradeBusy=!0;let n=t.querySelector(".bwo-rec-upgrade");n||(n=bwoBidsNode("section","bwo-card bwo-card-wide bwo-rec-upgrade"),n.appendChild(bwoBidsNode("div","bwo-no-movements","Analizando tu once y el mercado actual…")),t.querySelector(".bwo-cards")?.appendChild(n));try{let r=await bwoRecBuildData(e),o={players:[...r.own,...r.candidates]},a=bwoRoundJpData(o,bwoRecommendationUpgradeJPAt===0||e&&Date.now()-bwoRecommendationUpgradeJPAt>=bwoRecommendationUpgradeCooldown).then(i=>(bwoRecommendationUpgradeJPAt=Date.now(),i)).catch(()=>new Map),i=await Promise.race([a,new Promise(l=>setTimeout(()=>l(null),1200))]);i&&(r.jp=i),bwoRecRenderCard(r,r.jp),i||a.then(l=>{r.jp=l,bwoRecRenderCard(r,l)})}catch(r){n.replaceChildren(bwoBidsNode("div","bwo-no-movements","No se pudo completar el análisis del mercado. Espera a que termine el límite temporal de Biwenger y vuelve a intentarlo.")),console.warn("[biwtools] Error en recomendaciones avanzadas:",r)}finally{bwoRecommendationUpgradeBusy=!1}}
...
function bwoRoundJpProxy(e){return`https://r.jina.ai/https://${String(e).replace(/^https?:\/\//,"")}`}
async function bwoRoundJpText(e){let t=new AbortController,n=setTimeout(()=>t.abort(),16e3);try{let r=await fetch(bwoRoundJpProxy(e),{headers:{Accept:"text/plain","Cache-Control":"no-cache"},credentials:"omit",cache:"no-store",signal:t.signal});if(!r.ok)throw new Error(`Fuente externa ${r.status}`);return await r.text()}finally{clearTimeout(n)}}
...
async function bwoRoundJpData(e,t=!1){let n=Date.now(),r="bwo-round-jp-v1";if(!t&&bwoRoundJpMemory&&n-bwoRoundJpAt<9e5)return bwoRoundJpMemory;if(!t)try{let o=JSON.parse(localStorage.getItem(r)??"null");if(o&&n-o.at<9e5&&o.players){bwoRoundJpMemory=new Map(Object.entries(o.players)),bwoRoundJpAt=o.at;return bwoRoundJpMemory}}catch{}let o=await bwoRoundJpText("https://www.jornadaperfecta.com/onces-posibles/"),a=bwoXIConsensusRelevantUrls(o,/https?:\/\/(?:www\.)?jornadaperfecta\.com\/partido\/\d+\/[a-z0-9-]+/gi,e,10),i=new Set(e.players.map(l=>String(l.id))),l=await bwoRoundMapLimit(a,4,async s=>{try{return{url:s,text:await bwoRoundJpText(s)}}catch{return null}}),s=new Map;for(let u of l.filter(Boolean))for(let[c,f]of bwoRoundJpParseMatch(u.text,i,u.url)){let g=s.get(c);(!g||f.status||f.expected&&!g.expected)&&s.set(c,f)}bwoRoundJpMemory=s,bwoRoundJpAt=n;try{localStorage.setItem(r,JSON.stringify({at:n,players:Object.fromEntries(s)}))}catch{}return s}

/* Consenso de alineaciones: FútbolFantasy, Comuniate, Jornada Perfecta y Analítica Fantasy. */
var bwoXIConsensusMemory=null,bwoXIConsensusAt=0;
function bwoXIConsensusNorm(e){return String(e??"").normalize("NFD").replace(/[\u0300-\u036f]/g,"").replace(/[^a-z0-9]+/gi," ").trim().toLocaleLowerCase("es")}
function bwoXIConsensusTeamTokens(e){let t=new Set(["real","club","de","del","la","fc","cf","rcd","cd","ud"]);return bwoXIConsensusNorm(e).split(" ").filter(n=>n.length>=4&&!t.has(n))}
function bwoXIConsensusCandidates(e,t){let n=bwoXIConsensusNorm(e),r=(t.players??[]).filter(o=>{let a=bwoXIConsensusTeamTokens(o.teamName);return a.length&&a.some(i=>n.includes(i))});return r.length?r:t.players??[]}
function bwoXIConsensusRelevantUrls(e,t,n,r=10){let o=[...new Set([...String(e).matchAll(t)].map(a=>a[0].replace(/^http:/,"https:")))],a=(n?.players??[]).flatMap(i=>bwoXIConsensusTeamTokens(i.teamName));return o.filter(i=>{let l=bwoXIConsensusNorm(i);return a.some(s=>l.includes(s))}).slice(0,Math.max(1,r))}
function bwoXIConsensusNameScore(e,t){let n=bwoXIConsensusNorm(e),r=bwoXIConsensusNorm(t);if(!n||!r)return 0;if(n===r)return 1;if(n.length>=5&&r.length>=5&&(n.includes(r)||r.includes(n)))return .9;let o=n.split(" ").filter(Boolean),a=r.split(" ").filter(Boolean),i=o.filter(l=>a.includes(l)).length,s=new Set([...o,...a]).size,u=s?i/s:0,c=o.at(-1)===a.at(-1)?.22:0;return Math.min(1,u+c)}
function bwoXIConsensusFind(e,t){let n=null,r=0;for(let o of t){let a=bwoXIConsensusNameScore(e,o.name);a>r&&(n=o,r=a)}return r>=.72?n:null}
function bwoXIConsensusPut(e,t,n,r={}){let o=bwoXIConsensusFind(t,n);if(!o)return null;let a={source:r.source,label:r.label,url:r.url,starter:!!r.starter,probability:Math.max(1,Math.min(98,Number(r.probability)||50)),confirmed:!!r.confirmed,captain:!!r.captain,striker:!!r.striker};return e.set(String(o.id),a),o}
function bwoXIConsensusMissing(e,t,n,r){for(let o of n)e.has(String(o.id))||e.set(String(o.id),{source:r.source,label:r.label,url:r.url,starter:!1,probability:28,confirmed:!!r.confirmed,captain:!1,striker:!1})}
function bwoXIFFParse(e,t,n){let r=[...e.matchAll(/Alineaciones\s+(?:confirmadas|probables)[^\n]*/gi)].map(i=>{let l=e.indexOf("/jugadores/ficha/",i.index),s=l<0?1/0:l-i.index;return{index:i.index,heading:i[0],distance:s}}).filter(i=>i.distance<12e3).sort((i,l)=>i.distance-l.distance)[0];if(!r)return new Map;let o=e.slice(r.index),a=["Lesionados del","Convocatoria del","Estadísticas enfrentadas"].map(i=>o.indexOf(i)).filter(i=>i>0).sort((i,l)=>i-l)[0]??Math.min(o.length,45e3);o=o.slice(0,a);let i=new Map,l=[],s=/!\[Image [^:\]\n]*:\s*([^\]\n]+)\]\([^\)\n]*\/jugadores\/ficha\/[^\)\n]+\)\]\(https?:\/\/(?:www\.)?futbolfantasy\.com\/jugadores\/[^\)\n]+\)/gi;for(let u of o.matchAll(s)){let c=bwoXIConsensusNorm(u[1]);c&&!l.some(f=>bwoXIConsensusNorm(f)===c)&&l.push(u[1]);if(l.length>=22)break}let u=/confirmadas/i.test(r.heading),c={source:"ff",label:"FútbolFantasy",url:n,starter:!0,probability:u?98:78,confirmed:u};for(let f of l)bwoXIConsensusPut(i,f,t,c);l.length>=18&&bwoXIConsensusMissing(i,n,t,c);return i}
function bwoXIComuniateParse(e,t,n){let r=e.indexOf("### BANQUILLO"),o=r>0?e.slice(0,r):e,a=new Map,i=[],l=/\[!\[Image [^:\]]*:\s*([^\]]+)\]\([^)]*\/caras3\/[^)]+\)\]\([^)]*\)/gi;for(let s of o.matchAll(l)){let u=bwoXIConsensusNorm(s[1]);u&&!i.some(c=>bwoXIConsensusNorm(c)===u)&&i.push(s[1]);if(i.length>=22)break}let s=/ALINEACI[ÓO]N CONFIRMADA/i.test(o),u={source:"comuniate",label:"Comuniate",url:n,starter:!0,probability:s?98:76,confirmed:s};for(let c of i)bwoXIConsensusPut(a,c,t,u);i.length>=18&&bwoXIConsensusMissing(a,n,t,u);return a}
function bwoXIAnaliticaParse(e,t,n){let r=new Map,o="",a=e.split(/\r?\n/);for(let i of a){/###\s+Alineaci[oó]n probable/i.test(i)&&(o="starter"),/^Suplentes/i.test(i.trim())&&(o="bench");let l=[],s=i.match(/Foto de ([^\]]+)\]\([^)]*\/jugadores\/[^)]+\)(\d{1,3})%/i);s&&l.push([s[1],s[2]]);for(let u of i.matchAll(/\[([^\]\n*]+?)(?:\*\*)?\s*(?:—\s*)?(\d{1,3})%[^\]]*\]\(https?:\/\/(?:www\.)?analiticafantasy\.com\/jugadores\/[^)]+\)/gi))l.push([u[1].trim(),u[2]]);for(let[u,c]of l){let f=Number(c),g=o!=="bench",b=/%\s*C\b|%C\b/.test(i),p=i.includes("🎯");bwoXIConsensusPut(r,u,t,{source:"analitica",label:"Analítica Fantasy",url:n,starter:g,probability:f,confirmed:!1,captain:b,striker:p})}}return r}
function bwoXIConsensusMerge(e,t){for(let[n,r]of t){let o=e.get(n)??[],a=Array.isArray(r)?r:[r];o.push(...a),e.set(n,o)}}
async function bwoXIFFData(e){let t="https://www.futbolfantasy.com/laliga/posibles-alineaciones",n=await bwoRoundJpText(t),r=bwoXIConsensusRelevantUrls(n,/https?:\/\/(?:www\.)?futbolfantasy\.com\/partidos\/\d+-[a-z0-9-]+/gi,e,10),o=await bwoRoundMapLimit(r,4,async a=>{try{return{url:a,text:await bwoRoundJpText(a)}}catch{return null}}),a=new Map;for(let i of o.filter(Boolean))bwoXIConsensusMerge(a,bwoXIFFParse(i.text,bwoXIConsensusCandidates(i.url,e),i.url));return{key:"ff",label:"FútbolFantasy",url:t,map:a,available:r.length>0&&o.some(Boolean)}}
async function bwoXIComuniateData(e){let t="https://www.comuniate.com/alineaciones",n=await bwoRoundJpText(t),r=bwoXIConsensusRelevantUrls(n,/https?:\/\/(?:www\.)?comuniate\.com\/partido\/\d+\/\d+\/\d+\/[a-z0-9-]+/gi,e,10),o=await bwoRoundMapLimit(r,4,async a=>{try{return{url:a,text:await bwoRoundJpText(a)}}catch{return null}}),a=new Map;for(let i of o.filter(Boolean))bwoXIConsensusMerge(a,bwoXIComuniateParse(i.text,bwoXIConsensusCandidates(i.url,e),i.url));return{key:"comuniate",label:"Comuniate",url:t,map:a,available:r.length>0&&o.some(Boolean)}}
async function bwoXIAnaliticaData(e){let t="https://www.analiticafantasy.com/la-liga/alineaciones-probables",n=await bwoRoundJpText(t),r=[...n.matchAll(/https?:\/\/(?:www\.)?analiticafantasy\.com\/alineaciones-probables\/la-liga\/temporada-\d+\/jornada-\d+/gi)].map(o=>o[0].replace(/^http:/,"https:")),o=n.match(/Title:.*?Jornada\s+(\d+)/i)?.[1],a=r.find(i=>new RegExp(`/jornada-${o}(?:$|\\b)`).test(i))??r[0]??t,i=a===t?n:await bwoRoundJpText(a),l=bwoXIAnaliticaParse(i,e.players??[],a);return{key:"analitica",label:"Analítica Fantasy",url:a,map:l,available:l.size>0}}
async function bwoXIExternalData(e,t=!1){let n=Date.now(),r="bwo-xi-consensus-v2";if(!t&&bwoXIConsensusMemory&&n-bwoXIConsensusAt<12e5)return bwoXIConsensusMemory;if(!t)try{let o=JSON.parse(localStorage.getItem(r)??"null");if(o&&n-o.at<12e5){let a={players:new Map(Object.entries(o.players??{})),sources:o.sources??[]};return bwoXIConsensusMemory=a,bwoXIConsensusAt=o.at,a}}catch{}let o=[{key:"ff",label:"FútbolFantasy",url:"https://www.futbolfantasy.com/laliga/posibles-alineaciones",task:bwoXIFFData(e)},{key:"comuniate",label:"Comuniate",url:"https://www.comuniate.com/alineaciones",task:bwoXIComuniateData(e)},{key:"analitica",label:"Analítica Fantasy",url:"https://www.analiticafantasy.com/la-liga/alineaciones-probables",task:bwoXIAnaliticaData(e)}],a=await Promise.allSettled(o.map(l=>l.task)),i=new Map,l=[];for(let s=0;s<a.length;s++){let u=a[s],c=o[s];if(u.status==="fulfilled"){let f=u.value;l.push({key:f.key,label:f.label,url:f.url,available:!!f.available,count:f.map.size}),bwoXIConsensusMerge(i,f.map)}else l.push({key:c.key,label:c.label,url:c.url,available:!1,count:0})}let s={players:i,sources:l};bwoXIConsensusMemory=s,bwoXIConsensusAt=n;try{localStorage.setItem(r,JSON.stringify({at:n,players:Object.fromEntries(i),sources:l}))}catch{}return s}
function bwoXIConsensusBuild(e,t,n=[]){let r=[...n];if(t){let o=bwoRoundNativeText(t.status),a=t.probability!=null?Number(t.probability):t.expected?76:/lesion|sancion/.test(o)?3:50;r.push({source:"jp",label:"Jornada Perfecta",url:t.source??"https://www.jornadaperfecta.com/onces-posibles/",starter:!!t.expected||a>=60,probability:a,confirmed:!1,captain:t.pick==="capitan",striker:t.pick==="ariete"})}let o=Math.max(1,Math.min(98,Number(e.probability?.prob)||50));if(!r.length)return{probability:o,confidence:0,agreement:0,starterVotes:0,benchVotes:0,signals:[],available:0,summary:"Solo Biwenger",captainVotes:0,strikerVotes:0};let a=r.reduce((f,g)=>f+Number(g.probability||50)*(g.confirmed?1.25:1),0),i=r.reduce((f,g)=>f+(g.confirmed?1.25:1),0),l=a/Math.max(1,i),s=r.filter(f=>f.starter).length,u=r.length-s,c=Math.max(s,u),f=c/r.length,g=r.length>=4&&f===1?98:r.length>=3&&f===1?95:r.length>=2&&f===1?90:r.length===1?78:Math.min(88,Math.round(52+f*42)),b=r.length>=3?.2:r.length===2?.27:.38,p=Math.round(o*b+l*(1-b)),y=[...new Set(r.map(d=>d.label))],w=s>u;return{probability:Math.max(1,Math.min(98,p)),confidence:g,agreement:f,starterVotes:s,benchVotes:u,signals:r,available:r.length,summary:`${c}/${r.length} fuentes coinciden · ${g}% confianza`,captainVotes:r.filter(d=>d.captain).length,strikerVotes:r.filter(d=>d.striker).length,majorityStarter:w,labels:y}}
...
function bwoModernizeFabMenu(){let e=zv?.querySelector(".bwo-fab-menu");if(!e)return;let t=e.querySelector(".bwo-fab-menu-version");t&&(t.textContent="v1.6.2",t.setAttribute("aria-label","Biwtools versión 1.6.2"));let n=e.querySelector(".bwo-fab-menu-head");if(n&&!n.querySelector(".bwo-fab-menu-brand")){let p=bwoBidsNode("div","bwo-fab-menu-brand"),y=n.querySelector(".bwo-fab-menu-wordmark"),w=bwoBidsNode("small",null,"Centro de análisis");y&&p.appendChild(y),p.appendChild(w),n.insertBefore(p,t??n.firstChild)}if(!e.querySelector(".bwo-fab-menu-section")){let p=bwoBidsNode("div","bwo-fab-menu-section","Accesos rápidos");n?.insertAdjacentElement("afterend",p)}for(let p of e.querySelectorAll(".bwo-fab-item")){if(p.dataset.bwoFabModern==="1")continue;let y=(p.textContent??"").trim().toLocaleLowerCase("es"),w=bwoToolsCatalog.find(v=>y.includes(v.name.toLocaleLowerCase("es")));if(!w)continue;let m=p.querySelector(".bwo-fab-item-icon")??bwoBidsNode("span","bwo-fab-item-icon",w.icon),d=bwoBidsNode("span","bwo-fab-item-copy"),v=bwoBidsNode("strong",null,w.name),S=bwoBidsNode("small",null,`${w.category} · ${w.status}`),x=bwoBidsNode("span","bwo-fab-item-arrow","›");d.append(v,S),p.replaceChildren(m,d,x),p.dataset.bwoFabModern="1",p.dataset.bwoToolName=w.name,p.title=w.description,p.setAttribute("aria-label",`${w.name}. ${w.description}`)}if(!e.querySelector(".bwo-fab-center-open")){let p=bwoBidsNode("button","bwo-fab-center-open"),y=bwoBidsNode("span",null,"Abrir centro completo"),w=bwoBidsNode("small",null,"Tarjetas, métricas y buscador");p.type="button",p.append(y,w),p.addEventListener("click",()=>{zv.querySelector(".bwo-fab")?.click(),setTimeout(bwoOpenToolsPage,0)}),e.appendChild(p)}e.dataset.bwoModernVersion="1.6.2"}
var bwoModernizeFabMenuRender=bwoModernizeFabMenu;
bwoModernizeFabMenu=function(){let e=zv?.querySelector(".bwo-fab-menu");if(!e||e.dataset.bwoModernVersion==="1.6.2")return;return bwoModernizeFabMenuRender()};
...
bwoDiagKpi=function(e,t,n,r=""){return bwoDiagKpiVersionBase(e,e==="Versión"?"v1.9.0":t,n,r)};
...
/* Centro de clausulazos: plantillas rivales, cláusulas expuestas y recomendación económica. */
var bwoClauseCache=null,bwoClauseCacheAt=0;
function bwoClauseEnsureStyles(){let e=document.getElementById("bwo-clause-tool-styles");if(e?.dataset.bwoVersion==="1.6.3")return;e||(e=document.createElement("style"),e.id="bwo-clause-tool-styles"),e.dataset.bwoVersion="1.6.3",e.textContent=`
 .bwo-clause-tool{display:flex;min-height:520px;flex-direction:column;color:#f5f7fa;background:#111318;font-family:"Open Sans",system-ui,sans-serif}
 .bwo-clause-head{display:flex;align-items:flex-start;justify-content:space-between;gap:18px;padding:22px 24px;border-bottom:1px solid rgba(255,255,255,.09);background:radial-gradient(circle at 88% 0,rgba(225,24,52,.19),transparent 32%),linear-gradient(135deg,#1e222a,#15171c)}
 .bwo-clause-head-main{display:flex;align-items:flex-start;gap:14px}.bwo-clause-back,.bwo-clause-refresh,.bwo-clause-action{border:1px solid rgba(255,255,255,.14);border-radius:9px;color:#fff;background:#242832;cursor:pointer;font:800 10px/1.2 inherit}
 .bwo-clause-back,.bwo-clause-refresh{padding:9px 12px;white-space:nowrap}.bwo-clause-refresh{border-color:rgba(225,24,52,.42);background:rgba(225,24,52,.13)}.bwo-clause-refresh:disabled{opacity:.55;cursor:wait}
 .bwo-clause-eyebrow{display:block;margin-bottom:5px;color:#ff3853;font-size:8px;font-weight:950;letter-spacing:.12em;text-transform:uppercase}.bwo-clause-title{margin:0;color:#fff;font-size:23px;line-height:1.15}.bwo-clause-subtitle{max-width:800px;margin:7px 0 0;color:#aeb6c3;font-size:10px;line-height:1.55}
 .bwo-clause-body{padding:18px 20px 24px}.bwo-clause-kpis{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:9px;margin-bottom:14px}.bwo-clause-kpi{padding:12px 13px;border:1px solid rgba(255,255,255,.085);border-radius:10px;background:#191d24}.bwo-clause-kpi span{display:block;color:#8892a2;font-size:7.5px;font-weight:900;letter-spacing:.07em;text-transform:uppercase}.bwo-clause-kpi strong{display:block;margin-top:5px;color:#fff;font-size:16px;font-variant-numeric:tabular-nums}.bwo-clause-kpi.good strong{color:#69dd83}.bwo-clause-kpi.warn strong{color:#efbd58}.bwo-clause-kpi.bad strong{color:#ff715f}
 .bwo-clause-note{margin:0 0 14px;padding:10px 12px;border-left:3px solid #efbd58;border-radius:4px;color:#c5bdab;background:rgba(239,189,88,.065);font-size:9px;line-height:1.55}
 .bwo-clause-filters{display:grid;grid-template-columns:minmax(180px,1.5fr) repeat(4,minmax(130px,.75fr));gap:8px;margin-bottom:13px}.bwo-clause-control{width:100%;height:38px;box-sizing:border-box;padding:0 10px;border:1px solid rgba(255,255,255,.12);border-radius:9px;color:#f6f7f9;background:#191c22;font:700 9px inherit;outline:none}.bwo-clause-control:focus{border-color:rgba(225,24,52,.7);box-shadow:0 0 0 3px rgba(225,24,52,.1)}
 .bwo-clause-summary{display:flex;align-items:center;justify-content:space-between;gap:12px;margin:0 2px 10px;color:#8f98a6;font-size:9px}.bwo-clause-summary strong{color:#fff}.bwo-clause-grid{display:grid;grid-template-columns:minmax(0,1fr);gap:6px}.bwo-clause-card{position:relative;display:grid;grid-template-columns:30px minmax(190px,1.15fr) minmax(150px,.85fr) minmax(300px,1.45fr) minmax(190px,1fr) 150px;align-items:center;gap:10px;overflow:hidden;padding:9px 11px;border:1px solid rgba(255,255,255,.085);border-radius:10px;background:linear-gradient(105deg,#1d2128,#15171c);box-shadow:0 4px 12px rgba(0,0,0,.13)}.bwo-clause-card.affordable{border-color:rgba(105,221,131,.28)}.bwo-clause-card::before{position:absolute;top:0;bottom:0;left:0;width:3px;background:#586171;content:""}.bwo-clause-card.affordable::before{background:#69dd83}.bwo-clause-card.near::before{background:#efbd58}
 .bwo-clause-rank{display:grid;width:26px;height:26px;place-items:center;border-radius:7px;color:#fff;background:#2a2f39;font-size:8px;font-weight:950}.bwo-clause-player{display:flex;min-width:0;align-items:center;gap:9px}.bwo-clause-player>div{min-width:0}.bwo-clause-player img{width:42px;height:42px;flex:0 0 42px;border-radius:50%;object-fit:cover;background:#2a2e36}.bwo-clause-player h3{overflow:hidden;margin:0;color:#fff;font-size:12px;line-height:1.2;text-overflow:ellipsis;white-space:nowrap}.bwo-clause-player p{overflow:hidden;margin:3px 0 0;color:#8f98a6;font-size:7.5px;text-overflow:ellipsis;white-space:nowrap}.bwo-clause-owner{display:inline-flex;max-width:100%;overflow:hidden;margin-top:4px;padding:2px 6px;border-radius:999px;color:#c6cdd7;background:rgba(255,255,255,.07);font-size:7px;font-weight:800;text-overflow:ellipsis;white-space:nowrap}
 .bwo-clause-verdict{display:flex;min-width:0;align-items:flex-start;flex-direction:column;gap:5px;margin:0;padding:7px 8px;border-radius:8px;background:rgba(0,0,0,.18)}.bwo-clause-verdict strong{overflow:hidden;width:100%;color:#fff;font-size:8px;text-overflow:ellipsis;white-space:nowrap}.bwo-clause-badge{padding:3px 6px;border-radius:999px;font-size:7px;font-weight:950;white-space:nowrap}.bwo-clause-badge.top,.bwo-clause-badge.good{color:#aef2bc;background:rgba(105,221,131,.13)}.bwo-clause-badge.watch{color:#f5d995;background:rgba(239,189,88,.14)}.bwo-clause-badge.out{color:#ffaea3;background:rgba(255,113,95,.13)}
 .bwo-clause-metrics{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:5px}.bwo-clause-metric{min-width:0;padding:6px 7px;border-radius:7px;background:rgba(255,255,255,.045)}.bwo-clause-metric span{display:block;overflow:hidden;color:#818b9a;font-size:6.5px;font-weight:850;text-overflow:ellipsis;text-transform:uppercase;white-space:nowrap}.bwo-clause-metric strong{display:block;overflow:hidden;margin-top:3px;color:#fff;font-size:9px;font-variant-numeric:tabular-nums;text-overflow:ellipsis;white-space:nowrap}.bwo-clause-metric.good strong{color:#69dd83}.bwo-clause-metric.bad strong{color:#ff806e}
 .bwo-clause-progress{position:absolute;right:0;bottom:0;left:3px;height:2px;overflow:hidden;background:#292e37}.bwo-clause-progress span{display:block;height:100%;background:#69dd83}.bwo-clause-card.near .bwo-clause-progress span{background:#efbd58}.bwo-clause-card:not(.affordable):not(.near) .bwo-clause-progress span{background:#ff715f}.bwo-clause-reason{display:-webkit-box;overflow:hidden;margin:0;color:#9da6b3;font-size:7.5px;line-height:1.35;-webkit-box-orient:vertical;-webkit-line-clamp:2}
 .bwo-clause-actions{display:grid;grid-template-columns:1fr;gap:5px;margin:0}.bwo-clause-action{width:100%;padding:7px 8px}.bwo-clause-action.primary{border-color:rgba(105,221,131,.35);color:#b6f3c2;background:rgba(105,221,131,.1)}.bwo-clause-action:disabled{opacity:.45;cursor:wait}.bwo-clause-empty,.bwo-clause-loading{padding:36px 18px;border:1px dashed rgba(255,255,255,.13);border-radius:12px;color:#9ca5b2;background:#171a20;text-align:center;font-size:10px;line-height:1.6}
 @media(max-width:1180px){.bwo-clause-card{grid-template-columns:30px minmax(190px,1fr) minmax(155px,.8fr) minmax(270px,1.3fr) 145px}.bwo-clause-reason{display:none}}
 @media(max-width:950px){.bwo-clause-kpis{grid-template-columns:repeat(2,minmax(0,1fr))}.bwo-clause-filters{grid-template-columns:1fr 1fr}.bwo-clause-filters .bwo-clause-search{grid-column:1/-1}.bwo-clause-card{grid-template-columns:30px minmax(180px,1fr) minmax(260px,1.25fr) 140px}.bwo-clause-verdict{display:none}}
 @media(max-width:700px){.bwo-clause-card{grid-template-columns:26px minmax(0,1fr) 112px}.bwo-clause-metrics{grid-column:2/-1;grid-template-columns:repeat(4,minmax(0,1fr))}.bwo-clause-actions{grid-column:2/-1;grid-template-columns:1fr 1fr}.bwo-clause-player img{width:38px;height:38px;flex-basis:38px}}
 @media(max-width:600px){.bwo-clause-head{align-items:stretch;flex-direction:column;padding:14px}.bwo-clause-head-main{gap:9px}.bwo-clause-title{font-size:19px}.bwo-clause-subtitle{font-size:9px}.bwo-clause-body{padding:12px}.bwo-clause-kpis{gap:7px}.bwo-clause-kpi{padding:10px}.bwo-clause-kpi strong{font-size:13px}.bwo-clause-filters{grid-template-columns:1fr}.bwo-clause-filters .bwo-clause-search{grid-column:auto}.bwo-clause-summary{align-items:flex-start;flex-direction:column}.bwo-clause-card{gap:8px;padding:9px}.bwo-clause-metric{padding:6px 5px}.bwo-clause-metric span{font-size:6px}.bwo-clause-metric strong{font-size:8px}}
 `,e.isConnected||document.head.appendChild(e)}
function bwoClauseNumber(e){if(typeof e==="number")return Number.isFinite(e)&&e>=1e3&&e<1e9?Math.round(e):null;if(typeof e!=="string")return null;let t=e.trim();if(!t)return null;let n=Number(t.replace(/[^\d,.-]/g,"").replace(/\.(?=\d{3}(?:\D|$))/g,"").replace(",","."));return Number.isFinite(n)&&n>=1e3&&n<1e9?Math.round(n):null}
function bwoClauseObjectMoney(e){if(e==null)return null;let t=bwoClauseNumber(e);if(t)return t;if(typeof e!=="object")return null;for(let n of["amount","price","value","cost","current","total"]){let r=bwoClauseNumber(e[n]);if(r)return r}return null}
function bwoClauseAmount(e){let t=[e?.clausePrice,e?.clause_price,e?.clausulaPrice,e?.clausula_price,e?.releaseClause,e?.release_clause,e?.buyoutPrice,e?.buyout_price,e?.clause,e?.clausula,e?.buyout,e?.contract?.clause,e?.contract?.releaseClause,e?.owner?.clause,e?.owner?.clausePrice,e?.user?.clause];for(let n of t){let r=bwoClauseObjectMoney(n);if(r)return r}if(e&&typeof e==="object")for(let[n,r]of Object.entries(e))if(/claus|release.?clause|buyout/i.test(n)){let o=bwoClauseObjectMoney(r);if(o)return o}return null}
function bwoClauseAvailability(e){for(let t of["canBeClausulated","can_be_clausulated","canClause","can_clause","clauseAvailable","clause_available"])if(e?.[t]===!1)return{ok:!1,reason:"Cláusula no disponible"};for(let t of["clauseLocked","clause_locked","clausulaLocked","clausula_locked","buyoutLocked","buyout_locked"])if(e?.[t]===!0)return{ok:!1,reason:"Cláusula bloqueada"};return{ok:!0,reason:"Disponible"}}
async function bwoClauseMapLimit(e,t,n){let r=new Array(e.length),o=0;async function a(){for(;;){let i=o++;if(i>=e.length)return;r[i]=await n(e[i],i)}}return await Promise.all(Array.from({length:Math.min(t,e.length||1)},a)),r}
async function bwoClauseUserSquad(e,t){try{let n=await fetch(`${qi}/user/${e.id}?fields=*,players`,{headers:t});if(!n.ok)throw new Error(String(n.status));let r=(await n.json())?.data??{};return{user:{...e,...r},players:Array.isArray(r.players)?r.players:[]}}catch(n){return{user:e,players:Array.isArray(e.players)?e.players:[],error:n}}}
function bwoClauseAverage(e,t){let n=bn(e,t);return Number.isFinite(Number(n))?Number(n):null}
function bwoClauseProbability(e){let t=Number.parseFloat(String(e?.prob??e?.probability??"").replace(",","."));return Number.isFinite(t)?Math.max(0,Math.min(100,t)):null}
function bwoClauseRecommendation(e){if(!e.affordable){if(e.gap<=Math.max(5e5,e.maxBid*.15))return{label:"Muy cerca",tone:"watch",reason:`Te faltan ${bwoProMoney(e.gap)}. Una venta pequeña puede habilitar el clausulazo.`};return{label:"Fuera de presupuesto",tone:"out",reason:`Supera tu puja máxima en ${bwoProMoney(e.gap)}; se mantiene visible para planificar ventas.`}}if(e.improvement!=null&&e.improvement>=1.5&&e.average!=null&&e.average>=5)return{label:"Muy recomendado",tone:"top",reason:`Mejora en ${e.improvement.toFixed(1)} pts al jugador más débil de tu misma posición y entra en tu presupuesto.`};if(e.average!=null&&e.average>=5)return{label:"Recomendado",tone:"good",reason:`Buen rendimiento para su coste de cláusula y operación asumible con tu puja máxima actual.`};return{label:"Asequible",tone:"watch",reason:"Puedes ejecutar la cláusula, aunque su rendimiento aconseja revisar forma y titularidad antes de confirmar."}}
async function bwoClauseLoad(e=!1){if(!e&&bwoClauseCache&&Date.now()-bwoClauseCacheAt<6e4)return bwoClauseCache;let[t,n,r,o,a,i,l]=await Promise.all([zg(),H(),Li({fresh:e}).catch(()=>null),hn().catch(()=>[]),gn().catch(()=>[]),Oe().catch(()=>"1"),Oo().catch(()=>new Map)]),s=String(r?.id??await pn()),u=(t?.data?.users??[]).filter(E=>E?.id!=null&&String(E.id)!==s),c=await Hi(),f=await bwoClauseMapLimit(u,4,E=>bwoClauseUserSquad(E,c)),g=new Set((a??[]).map(String));if(g.size===0)for(let E of await vn().catch(()=>[]))g.add(String(E));let b=new Map;for(let E of g){let P=n.get(String(E));if(!P)continue;let T=String(P.position),O=b.get(T),R=bwoClauseAverage(P,i);R!=null&&(O==null||R<O)&&b.set(T,R)}let p=(o??[]).find(E=>String(E.id)===s),y=Number(p?.maxBid??r?.balance??0),w=Number(p?.balance??r?.balance??0),m=[],d=0,v=0;for(let E of f){for(let P of E.players){let T=typeof P?.player==="object"?P.player:P,O=String(T?.id??P?.playerId??P?.player_id??"");if(!O)continue;let R=n.get(O)??T,F=bwoClauseAmount(P)??bwoClauseAmount(T),U=bwoClauseAvailability(P);if(!F){d++;continue}if(!U.ok){v++;continue}let k=String(R?.position??T?.position??P?.position??""),G=bwoClauseAverage(R,i),Q=bwoClauseProbability(l.get(O)),Y=b.get(k),J=G!=null&&Y!=null?G-Y:null,ee=F<=y,te=Math.max(0,F-y),ne=Number(R?.price??T?.price??0),re=Number(R?.price_increment??R?.priceIncrement??T?.priceIncrement??0),oe=G==null?0:G*10,ae=Q==null?0:Q*.12,ie=J==null?0:J*7,le=G==null?0:Math.min(20,G/Math.max(F/1e6,.25)*2),se=ee?55:-Math.min(45,te/Math.max(y,1)*16),ue=oe+ae+ie+le+se+Math.max(-8,Math.min(8,re/25000));let ce={id:O,name:R?.name??T?.name??`Jugador ${O}`,teamName:R?.team_name??T?.team?.name??"Equipo sin dato",position:k,ownerId:String(E.user?.id??""),ownerName:E.user?.name??"Rival",catalog:R,clause:F,marketValue:ne,priceChange:re,average:G,probability:Q,improvement:J,affordable:ee,gap:te,maxBid:y,rating:ue};ce.recommendation=bwoClauseRecommendation(ce),m.push(ce)}}m.sort((E,P)=>Number(P.affordable)-Number(E.affordable)||P.rating-E.rating||E.clause-P.clause);return bwoClauseCache={rows:m,rivals:u.length,maxBid:y,balance:w,unknown:d,blocked:v,loadedAt:Date.now()},bwoClauseCacheAt=Date.now(),bwoClauseCache}
async function bwoClauseFetchOfficial(){let e=await fetch(`${qi}/owners/league/clause`,{headers:await Hi()});if(!e.ok)throw new Error(`getLeagueClauses failed: ${e.status}`);let t=await e.json(),n=t?.data??t;return Array.isArray(n)?n:Array.isArray(n?.players)?n.players:Array.isArray(n?.items)?n.items:[]}
function bwoClauseOwnerId(e){let t=e?.owner?.user??e?.user,n=typeof t==="object"?t?.id:t;return String(n??e?.owner?.userID??e?.owner?.user_id??"")}
var bwoClauseAvailabilityBase=bwoClauseAvailability;
bwoClauseAvailability=function(e){let t=bwoClauseAvailabilityBase(e);if(!t.ok)return t;let n=Number(e?.clauseLockedUntil??e?.clause_locked_until??0);return n>0&&(n>1e12?n:n*1e3)>Date.now()?{ok:!1,reason:"Cláusula temporalmente bloqueada"}:t};
var bwoClauseLoadUserFallback=bwoClauseLoad;
bwoClauseLoad=async function(e=!1){if(!e&&bwoClauseCache&&Date.now()-bwoClauseCacheAt<6e4)return bwoClauseCache;try{let[t,n,r,o,a,i,l,s]=await Promise.all([bwoClauseFetchOfficial(),H(),Li({fresh:e}).catch(()=>null),hn().catch(()=>[]),gn().catch(()=>[]),Oe().catch(()=>"1"),Oo().catch(()=>new Map),zg()]),u=String(r?.id??await pn()),c=new Map((s?.data?.users??[]).filter(M=>M?.id!=null).map(M=>[String(M.id),M])),f=new Set((a??[]).map(String));if(f.size===0)for(let M of await vn().catch(()=>[]))f.add(String(M));let g=new Map;for(let M of f){let B=n.get(String(M));if(!B)continue;let E=String(B.position),P=bwoClauseAverage(B,i),T=g.get(E);P!=null&&(T==null||P<T)&&g.set(E,P)}let b=(o??[]).find(M=>String(M.id)===u),p=Number(b?.maxBid??r?.balance??0),y=Number(b?.balance??r?.balance??0),w=[],m=0,d=0;for(let M of t){let B=String(M?.id??M?.player?.id??M?.playerID??M?.player_id??"");if(!B)continue;let E=M?.owner??M?.player?.owner??{},P=bwoClauseOwnerId({owner:E});if(P&&P===u)continue;let T=n.get(B)??M?.player??M,R=bwoClauseAmount({owner:E})??bwoClauseAmount(M),F=bwoClauseAvailability(E);if(!R){m++;continue}if(!F.ok){d++;continue}let U=String(T?.position?.id??T?.position??M?.position??""),k=bwoClauseAverage(T,i),G=bwoClauseProbability(l.get(B)),Q=g.get(U),Y=k!=null&&Q!=null?k-Q:null,J=R<=p,ee=Math.max(0,R-p),te=Number(T?.price??M?.price??0),ne=Number(T?.price_increment??T?.priceIncrement??M?.priceIncrement??0),re=k==null?0:k*10,oe=G==null?0:G*.12,ae=Y==null?0:Y*7,ie=k==null?0:Math.min(20,k/Math.max(R/1e6,.25)*2),le=J?55:-Math.min(45,ee/Math.max(p,1)*16),se=re+oe+ae+ie+le+Math.max(-8,Math.min(8,ne/25000)),ue=c.get(P)??(typeof E.user==="object"?E.user:null),ce={id:B,name:T?.name??M?.name??`Jugador ${B}`,teamName:T?.team_name??T?.team?.name??M?.team?.name??"Equipo sin dato",position:U,ownerId:P,ownerName:ue?.name??"Rival",catalog:T,clause:R,marketValue:te,priceChange:ne,average:k,probability:G,improvement:Y,affordable:J,gap:ee,maxBid:p,rating:se};ce.recommendation=bwoClauseRecommendation(ce),w.push(ce)}w.sort((M,B)=>Number(B.affordable)-Number(M.affordable)||B.rating-M.rating||M.clause-B.clause);let v=Math.max(0,c.size-(c.has(u)?1:0));return bwoClauseCache={rows:w,rivals:v||new Set(w.map(M=>M.ownerId).filter(Boolean)).size,maxBid:p,balance:y,unknown:m,blocked:d,loadedAt:Date.now(),source:"owners/league/clause"},bwoClauseCacheAt=Date.now(),bwoClauseCache}catch(t){console.warn("[biwtools] La colección oficial de cláusulas no estuvo disponible; usando respaldo por plantillas:",t);return bwoClauseLoadUserFallback(e)}};
function bwoClausePosition(e){return({1:"Portero",2:"Defensa",3:"Centrocampista",4:"Delantero"})[String(e)]??"Sin posición"}
function bwoClauseKpi(e,t,n=""){let r=bwoBidsNode("article",`bwo-clause-kpi ${n}`),o=bwoBidsNode("span",null,e),a=bwoBidsNode("strong",null,t);return r.append(o,a),r}
function bwoClauseMetric(e,t,n=""){let r=bwoBidsNode("div",`bwo-clause-metric ${n}`);return r.append(bwoBidsNode("span",null,e),bwoBidsNode("strong",null,t)),r}
var bwoClauseNativeMetaCache=new Map;
async function bwoClauseNativePlayerMeta(e){let t=String(e?.id??e?.catalog?.id??""),n=e?.catalog?.competition?.slug??e?.catalog?.competition_slug??e?.catalog?.competitionSlug??"la-liga";n==="primera-division"&&(n="la-liga");let r=e?.catalog?.slug??bwoClauseSlug(e?.name),o=e?.catalog?.url??e?.url;if(typeof o==="string"&&o.includes("/players/")){let a=o.startsWith("http")?new URL(o).pathname:o,i=a.match(/^\/([^/]+)\/players\/([^/?#]+)/);if(i)return{competition:i[1],slug:i[2],path:a}}if(!t)return{competition:n,slug:r,path:`/${n}/players/${r}`};if(bwoClauseNativeMetaCache.has(t))return bwoClauseNativeMetaCache.get(t);let a=(async()=>{try{let i=await fetch(`https://cf.biwenger.com/api/v2/players/${encodeURIComponent(n)}/${encodeURIComponent(t)}?lang=es`,{headers:{Accept:"application/json"}});if(!i.ok)throw new Error(`${i.status}`);let l=(await i.json())?.data??{},s=String(l.slug??"").trim();return s&&(r=s),{competition:n,slug:r,path:`/${n}/players/${r}`}}catch(i){return console.warn(`[biwtools] No se pudo obtener el slug oficial de ${e?.name??t}:`,i),{competition:n,slug:r,path:`/${n}/players/${r}`}}})();return bwoClauseNativeMetaCache.set(t,a),a}
async function bwoClauseNativePlayerUrl(e){return(await bwoClauseNativePlayerMeta(e)).path}
async function bwoClauseOpenDetail(e,t){let n=t?.textContent;t&&(t.disabled=!0,t.textContent="Abriendo…");try{let r=await bwoClauseNativePlayerMeta(e),o=r.path,a=[...document.querySelectorAll("a[href*='/players/']")].find(i=>{let l=i.getAttribute("href")??"";return l===o||l.endsWith(`/players/${r.slug}`)||i.closest("player-card")?.querySelector(`img[src*="/i/p/${e.id}.png"]`)});bwoCloseToolsPage(),setTimeout(()=>{if(a){a.click();return}location.assign(o)},80)}catch(r){console.warn("[biwtools] No se pudo abrir la ficha oficial:",r),bwoRecommendationToast(`No se pudo abrir la ficha de ${e.name}. Inténtalo de nuevo.`),t&&(t.disabled=!1,t.textContent=n??"Ver ficha")}}
function bwoClauseSlug(e){return String(e??"usuario").normalize("NFD").replace(/[\u0300-\u036f]/g,"").toLowerCase().replace(/[^a-z0-9]+/g,"-").replace(/^-|-$/g,"")||"usuario"}
function bwoClauseOpenNative(e){bwoCloseToolsPage();let t=Date.now(),n=!1,r=!1,o=()=>{if(Date.now()-t>14e3){bwoRecommendationToast(`No se pudo abrir automáticamente la cláusula de ${e.name}. Entra en la plantilla de ${e.ownerName} y selecciónalo.`);return}let a=U0(location.pathname),i=a&&String(a)===String(e.ownerId);if(i){let l=[...document.querySelectorAll(`img[src*="/i/p/${e.id}.png"]`)].find(S=>S.getClientRects().length);if(!l){setTimeout(o,220);return}let s=l.closest("player-card,article,li,.player")??l.parentElement;s?.scrollIntoView({behavior:"smooth",block:"center"}),Object.assign((s??l).style,{outline:"3px solid rgba(105,221,131,.9)",outlineOffset:"3px"});let u=[...(s??document).querySelectorAll("button,a")].find(S=>/cl[aá]usul/i.test((S.textContent??"").trim()));if(u){u.click(),bwoRecommendationToast(`Abierta la cláusula de ${e.name}. Comprueba el importe antes de confirmar.`,"ok")}else bwoRecommendationToast(`${e.name} está localizado. Biwenger no muestra ahora un botón de cláusula disponible.`);return}if(location.pathname.includes("/league")){let l=[...document.querySelectorAll("a[href*='/user/']")].find(s=>String(U0(s.getAttribute("href")))===String(e.ownerId));if(l&&!r){r=!0,l.click()}else setTimeout(o,220);return}if(!n){n=!0;let l=document.querySelector("app-nav a[href='/league']")??document.querySelector("a[href='/league']");l?l.click():history.pushState(history.state,"",`/user/${bwoClauseSlug(e.ownerName)}-${e.ownerId}`)}setTimeout(o,220)};setTimeout(o,180)}
function bwoClauseReturnTools(){let e=document.querySelector(".bwo-tools-page-host"),t=bwoBuildToolsPage();e?.replaceWith(t),bwoEnsureToolsOverview(t),bwoSyncToolsNavState()}
function bwoClauseCard(e,t){let n=bwoBidsNode("article",`bwo-clause-card ${e.affordable?"affordable":e.recommendation.tone==="watch"?"near":""}`),r=bwoBidsNode("span","bwo-clause-rank",`${t}º`),o=bwoBidsNode("div","bwo-clause-player"),a=document.createElement("img");a.src=`https://cdn.biwenger.com/i/p/${e.id}.png`,a.alt="",a.addEventListener("error",()=>a.style.visibility="hidden");let i=bwoBidsNode("div"),l=bwoBidsNode("h3",null,e.name),s=bwoBidsNode("p",null,`${bwoClausePosition(e.position)} · ${e.teamName}`),u=bwoBidsNode("span","bwo-clause-owner",e.ownerName);i.append(l,s,u),o.append(a,i);let c=bwoBidsNode("div","bwo-clause-verdict"),f=bwoBidsNode("strong",null,e.affordable?"Operación dentro de presupuesto":e.recommendation.label),g=bwoBidsNode("span",`bwo-clause-badge ${e.recommendation.tone}`,e.recommendation.label);c.append(f,g);let b=bwoBidsNode("div","bwo-clause-metrics");b.append(bwoClauseMetric("Cláusula",bwoProMoney(e.clause),e.affordable?"good":"bad"),bwoClauseMetric("Puja máxima",bwoProMoney(e.maxBid)),bwoClauseMetric("Media",e.average==null?"Sin datos":`${e.average.toFixed(1)} pts`),bwoClauseMetric("Titularidad",e.probability==null?"Sin dato":`${Math.round(e.probability)}%`));let p=bwoBidsNode("div","bwo-clause-progress"),y=bwoBidsNode("span");y.style.width=`${Math.max(3,Math.min(100,e.maxBid/Math.max(e.clause,1)*100))}%`,p.appendChild(y);let w=bwoBidsNode("p","bwo-clause-reason",e.recommendation.reason),m=bwoBidsNode("div","bwo-clause-actions"),d=bwoBidsNode("button","bwo-clause-action","Ver ficha"),v=bwoBidsNode("button","bwo-clause-action primary",e.affordable?"Ver y clausular":"Ver rival");d.type=v.type="button",d.addEventListener("click",()=>bwoClauseOpenDetail(e,d)),v.addEventListener("click",()=>bwoClauseOpenNative(e)),m.append(d,v),n.append(r,o,c,b,p,w,m);return n}
function bwoClauseRender(e,t){let n=t.querySelector(".bwo-clause-content");if(!n)return;n.replaceChildren();let r=bwoBidsNode("div","bwo-clause-kpis"),o=e.rows.filter(x=>x.affordable).length,a=e.rows.filter(x=>x.recommendation.tone==="top").length;r.append(bwoClauseKpi("Rivales analizados",String(e.rivals)),bwoClauseKpi("Cláusulas visibles",String(e.rows.length),"good"),bwoClauseKpi("Dentro de presupuesto",String(o),o?"good":"bad"),bwoClauseKpi("Muy recomendados",String(a),a?"good":"warn"),bwoClauseKpi("Tu puja máxima",bwoProMoney(e.maxBid),e.maxBid>0?"good":"bad")),n.appendChild(r);if(e.unknown||e.blocked)n.appendChild(bwoBidsNode("p","bwo-clause-note",`${e.unknown?`${e.unknown} jugadores no publican un importe de cláusula en la respuesta actual. `:""}${e.blocked?`${e.blocked} aparecen bloqueados o no clausulables. `:""}Solo se muestran costes que Biwenger expone de forma verificable para tu cuenta.`));let i=bwoBidsNode("div","bwo-clause-filters"),l=bwoBidsNode("input","bwo-clause-control bwo-clause-search");l.type="search",l.placeholder="Buscar jugador, equipo o rival…";let s=bwoBidsNode("select","bwo-clause-control");for(let[x,C]of[["all","Todas las posiciones"],["1","Porteros"],["2","Defensas"],["3","Centrocampistas"],["4","Delanteros"]]){let N=bwoBidsNode("option",null,C);N.value=x,s.appendChild(N)}let u=bwoBidsNode("select","bwo-clause-control");for(let[x,C]of[["all","Cualquier presupuesto"],["yes","Puedo pagarlo"],["near","Me falta hasta 15%"],["no","Fuera de presupuesto"]]){let N=bwoBidsNode("option",null,C);N.value=x,u.appendChild(N)}let c=bwoBidsNode("select","bwo-clause-control");for(let[x,C]of[["all","Todos los rivales"],...[...new Map(e.rows.map(N=>[N.ownerId,N.ownerName])).entries()].sort((N,_)=>N[1].localeCompare(_[1],"es"))]){let N=bwoBidsNode("option",null,C);N.value=x,c.appendChild(N)}let f=bwoBidsNode("select","bwo-clause-control");for(let[x,C]of[["recommended","Orden: recomendados"],["clause","Cláusula más barata"],["average","Mayor media"],["probability","Mayor titularidad"],["improvement","Mayor mejora"]]){let N=bwoBidsNode("option",null,C);N.value=x,f.appendChild(N)}i.append(l,s,u,c,f);let g=bwoBidsNode("div","bwo-clause-summary"),b=bwoBidsNode("strong"),p=bwoBidsNode("span",null,"Media calculada con el sistema activo de la liga · importes procedentes de Biwenger"),y=bwoBidsNode("div","bwo-clause-grid");g.append(b,p),n.append(i,g,y);function w(){let x=l.value.trim().toLocaleLowerCase("es"),C=e.rows.filter(N=>(!x||`${N.name} ${N.teamName} ${N.ownerName}`.toLocaleLowerCase("es").includes(x))&&(s.value==="all"||N.position===s.value)&&(c.value==="all"||N.ownerId===c.value)&&(u.value==="all"||u.value==="yes"&&N.affordable||u.value==="near"&&!N.affordable&&N.gap<=Math.max(5e5,N.maxBid*.15)||u.value==="no"&&!N.affordable));f.value==="clause"?C.sort((N,_)=>N.clause-_.clause):f.value==="average"?C.sort((N,_)=>(_.average??-1)-(N.average??-1)):f.value==="probability"?C.sort((N,_)=>(_.probability??-1)-(N.probability??-1)):f.value==="improvement"?C.sort((N,_)=>(_.improvement??-99)-(N.improvement??-99)):C.sort((N,_)=>Number(_.affordable)-Number(N.affordable)||_.rating-N.rating||N.clause-_.clause),b.textContent=`${C.length} jugador${C.length===1?"":"es"} encontrado${C.length===1?"":"s"}`,y.replaceChildren();if(!C.length)y.appendChild(bwoBidsNode("div","bwo-clause-empty","No hay jugadores que coincidan con estos filtros. Amplía el presupuesto o selecciona todas las posiciones."));else C.forEach((N,_)=>y.appendChild(bwoClauseCard(N,_+1)))}for(let x of[l,s,u,c,f])x.addEventListener(x===l?"input":"change",w);w()}
async function bwoOpenClausesTool(e=!1){bwoClauseEnsureStyles();let t=document.querySelector(".bwo-tools-page-host");if(!t)return;let n=bwoBidsNode("section","bwo-clause-tool"),r=bwoBidsNode("div","bwo-tools-overview");r.hidden=!0;let o=bwoBidsNode("header","bwo-clause-head"),a=bwoBidsNode("div","bwo-clause-head-main"),i=bwoBidsNode("button","bwo-clause-back","← Herramientas"),l=bwoBidsNode("div"),s=bwoBidsNode("span","bwo-clause-eyebrow","BIWTOOLS · MERCADO DE CLÁUSULAS"),u=bwoBidsNode("h1","bwo-clause-title","Clausulazos"),c=bwoBidsNode("p","bwo-clause-subtitle","Explora jugadores de tus rivales cuya cláusula está expuesta, ordénalos por rendimiento y descubre cuáles encajan realmente en tu puja máxima."),f=bwoBidsNode("button","bwo-clause-refresh",e?"Actualizando…":"Actualizar datos");i.type=f.type="button",f.disabled=e,i.addEventListener("click",bwoClauseReturnTools),f.addEventListener("click",()=>bwoOpenClausesTool(!0)),l.append(s,u,c),a.append(i,l),o.append(a,f);let g=bwoBidsNode("main","bwo-clause-body bwo-clause-content"),b=bwoBidsNode("div","bwo-clause-loading",e?"Actualizando plantillas, cláusulas y capacidad económica…":"Consultando las plantillas rivales y preparando recomendaciones…");g.appendChild(b),n.append(r,o,g),t.replaceChildren(n);try{let p=await bwoClauseLoad(e);document.contains(n)&&bwoClauseRender(p,n)}catch(p){console.warn("[biwtools] No se pudo construir Clausulazos:",p),g.replaceChildren(bwoBidsNode("div","bwo-clause-empty","No se pudieron consultar las cláusulas con la sesión actual. Recarga Biwenger, ejecuta el bookmark y pulsa Actualizar datos."))}}
if(!bwoToolsCatalog.some(e=>e.name==="Clausulazos")){let e={icon:"⚡",name:"Clausulazos",category:"Mercado",status:"Oportunidades rivales",tone:"orange",featured:!0,description:"Ordena las cláusulas visibles de tus rivales por rendimiento, posición y capacidad real de pago.",features:["Plantillas rivales","Filtros por posición","Recomendación según tu puja máxima"],source:"Biwenger · liga y plantillas"},t=bwoToolsCatalog.findIndex(n=>n.name==="Recomendaciones");bwoToolsCatalog.splice(t<0?bwoToolsCatalog.length:t+1,0,e)}
var bwoOpenToolFromPageBeforeClauses=bwoOpenToolFromPage;
bwoOpenToolFromPage=function(e){return e==="Clausulazos"?bwoOpenClausesTool(!1):bwoOpenToolFromPageBeforeClauses(e)};

/* Once Jornada: optimizador integrado del once propio por formación, previsión y riesgo. */
var bwoMatchdayXIFormations=[
 {name:"3-4-3",slots:{"1":1,"2":3,"3":4,"4":3}},
 {name:"3-5-2",slots:{"1":1,"2":3,"3":5,"4":2}},
 {name:"4-3-3",slots:{"1":1,"2":4,"3":3,"4":3}},
 {name:"4-4-2",slots:{"1":1,"2":4,"3":4,"4":2}},
 {name:"4-5-1",slots:{"1":1,"2":4,"3":5,"4":1}},
 {name:"5-3-2",slots:{"1":1,"2":5,"3":3,"4":2}},
 {name:"5-4-1",slots:{"1":1,"2":5,"3":4,"4":1}}
],bwoMatchdayXICache=null,bwoMatchdayXICacheAt=0;
function bwoMatchdayXIEnsureStyles(){let e=document.getElementById("bwo-matchday-xi-styles");if(e)return;e=document.createElement("style"),e.id="bwo-matchday-xi-styles",e.textContent=`
 .bwo-xi-tool{min-height:620px;background:#101216;color:#f7f8fb;border:1px solid #2b2f38;border-radius:18px;overflow:hidden}
 .bwo-xi-head{display:flex;align-items:flex-start;justify-content:space-between;gap:20px;padding:25px 28px;background:linear-gradient(112deg,#1c222d 0%,#191c23 55%,#281117 100%);border-bottom:1px solid #343945}
 .bwo-xi-head-main{display:flex;align-items:flex-start;gap:17px}.bwo-xi-head-actions{display:flex;align-items:center;gap:8px}.bwo-xi-back,.bwo-xi-refresh,.bwo-xi-league,.bwo-xi-round,.bwo-xi-detail{border:1px solid #454b58;border-radius:10px;background:#20242c;color:#fff;font:700 13px inherit;cursor:pointer;padding:10px 14px}
 .bwo-xi-back:hover,.bwo-xi-refresh:hover,.bwo-xi-league:hover,.bwo-xi-detail:hover{border-color:#ff3046;background:#2b2025}.bwo-xi-refresh{border-color:#9e2432;background:#49131b;white-space:nowrap}.bwo-xi-refresh:disabled{opacity:.55;cursor:wait}.bwo-xi-league{border-color:#36547c;background:#17263a;color:#b8d5ff;white-space:nowrap}
 .bwo-xi-eyebrow{display:block;color:#ff3046;font-size:11px;font-weight:900;letter-spacing:.11em;text-transform:uppercase}.bwo-xi-title{margin:5px 0 3px;font-size:28px;line-height:1.1}.bwo-xi-subtitle{max-width:820px;margin:0;color:#b9c0ce;font-size:13px;line-height:1.55}
 .bwo-xi-content{padding:20px}.bwo-xi-toolbar{display:flex;align-items:center;justify-content:space-between;gap:14px;margin-bottom:14px}.bwo-xi-toolbar-copy strong{display:block;font-size:15px}.bwo-xi-toolbar-copy span{color:#969eae;font-size:12px}.bwo-xi-formations{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:8px;margin:0 0 14px}.bwo-xi-formation{padding:10px 11px;border:1px solid #303640;border-radius:10px;color:#cbd2de;background:#171b21;cursor:pointer;text-align:left}.bwo-xi-formation:hover,.bwo-xi-formation.active{border-color:#42c96b;background:#16251c}.bwo-xi-formation strong{display:block;color:#fff;font-size:12px}.bwo-xi-formation span{display:block;margin-top:3px;color:#9ca5b4;font-size:9px}.bwo-xi-formation b{color:#67e88d}.bwo-xi-team-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(125px,1fr));gap:10px}.bwo-xi-team-card{display:flex;min-height:105px;align-items:center;justify-content:center;flex-direction:column;gap:8px;padding:12px;border:1px solid #303640;border-radius:12px;color:#fff;background:#171b21;cursor:pointer;font:800 11px/1.3 inherit;text-align:center}.bwo-xi-team-card:hover{border-color:#4b78b7;background:#192536;transform:translateY(-1px)}.bwo-xi-team-card img{width:50px;height:50px;object-fit:contain}.bwo-xi-league-toolbar{display:flex;align-items:center;justify-content:space-between;gap:10px;margin-bottom:14px;padding:11px 13px;border:1px solid #303640;border-radius:11px;background:#171b21}.bwo-xi-league-toolbar strong{font-size:14px}.bwo-xi-league-toolbar span{color:#9da5b4;font-size:10px}.bwo-xi-league-pitch{position:relative;width:min(760px,100%);height:560px;margin:0 auto;border-radius:14px;overflow:hidden;background:#17291c}.bwo-xi-league-pitch>img{width:100%;height:100%;object-fit:cover}.bwo-xi-league-player{position:absolute;z-index:2;display:flex;width:92px;align-items:center;flex-direction:column;transform:translate(-50%,-50%);text-align:center}.bwo-xi-league-player img,.bwo-xi-league-initial{width:46px;height:46px;border:2px solid rgba(255,255,255,.82);border-radius:50%;object-fit:cover;background:#252a32}.bwo-xi-league-initial{display:grid;place-items:center;font-weight:900}.bwo-xi-league-player strong{max-width:92px;margin-top:3px;padding:2px 5px;border-radius:6px;color:#fff;background:rgba(0,0,0,.76);font-size:9px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.bwo-xi-league-prob{position:absolute;top:-3px;right:16px;padding:2px 4px;border-radius:999px;color:#fff;background:#217a3a;font-size:7px;font-weight:900}.bwo-xi-league-alts{display:grid;grid-template-columns:repeat(auto-fill,minmax(165px,1fr));gap:7px;margin-top:12px}.bwo-xi-league-alt{display:flex;align-items:center;gap:8px;padding:8px;border:1px solid #303640;border-radius:9px;background:#171b21}.bwo-xi-league-alt img,.bwo-xi-league-alt .bwo-xi-league-initial{width:32px;height:32px;flex:0 0 32px}.bwo-xi-league-alt strong{font-size:10px}.bwo-xi-league-alt span{margin-left:auto;color:#67e88d;font-size:9px;font-weight:900}
 .bwo-xi-select{min-width:210px;border:1px solid #3a404b;border-radius:10px;background:#191d24;color:#fff;padding:10px 12px;font:700 13px inherit}
 .bwo-xi-kpis{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:10px;margin-bottom:14px}.bwo-xi-kpi{padding:14px 15px;border:1px solid #303640;border-radius:12px;background:#181c22}.bwo-xi-kpi span{display:block;color:#929bab;font-size:10px;font-weight:900;letter-spacing:.07em;text-transform:uppercase}.bwo-xi-kpi strong{display:block;margin-top:5px;font-size:20px}.bwo-xi-kpi.good strong{color:#5ee787}.bwo-xi-kpi.warn strong{color:#ffc85a}.bwo-xi-kpi.bad strong{color:#ff6675}
 .bwo-xi-notice{margin:0 0 14px;padding:11px 13px;border-left:3px solid #ff3046;border-radius:7px;background:#1b1e24;color:#cbd1dc;font-size:12px;line-height:1.5}.bwo-xi-notice.good{border-color:#43d879;background:#14231b}.bwo-xi-notice.warn{border-color:#f0b63c;background:#252116}
 .bwo-xi-layout{display:grid;grid-template-columns:minmax(0,1.8fr) minmax(280px,.8fr);gap:14px;align-items:start}.bwo-xi-panel{border:1px solid #2e343e;border-radius:14px;background:#15181d;overflow:hidden}.bwo-xi-panel-head{display:flex;align-items:center;justify-content:space-between;gap:10px;padding:14px 15px;border-bottom:1px solid #2c313a;background:#1c2026}.bwo-xi-panel-head h2{margin:0;font-size:15px}.bwo-xi-panel-head span{color:#9da5b4;font-size:11px}
 .bwo-xi-plan-wrap{padding:14px}.bwo-xi-plan-pitch{position:relative;width:min(590px,100%);height:520px;margin:0 auto;overflow:hidden;border-radius:13px;background:#183c21;box-shadow:0 12px 28px rgba(0,0,0,.3)}.bwo-xi-plan-pitch>.bwo-xi-pitch-ground{position:absolute;inset:0;width:100%;height:100%;object-fit:fill}.bwo-xi-plan-player{position:absolute;z-index:2;display:flex;width:108px;align-items:center;flex-direction:column;transform:translate(-50%,-50%);border:0;background:transparent;color:#fff;cursor:pointer;text-align:center}.bwo-xi-plan-photo{position:relative;width:58px;height:58px}.bwo-xi-plan-photo>img{width:58px;height:58px;border-radius:50%;object-fit:cover;object-position:top center;filter:drop-shadow(0 2px 2px rgba(0,0,0,.5))}.bwo-xi-plan-name{display:block;max-width:108px;margin-top:-3px;padding:3px 7px;border-radius:8px;background:rgba(22,63,15,.9);font-size:10px;font-weight:900;line-height:1.15;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.bwo-xi-plan-meta{display:flex;align-items:center;justify-content:center;gap:3px;margin-top:2px}.bwo-xi-plan-meta span{padding:2px 4px;border-radius:5px;background:rgba(0,0,0,.74);font-size:8px;font-weight:800}.bwo-xi-plan-meta .good{color:#83f1a2}.bwo-xi-plan-meta .warn{color:#ffd56e}.bwo-xi-plan-meta .bad{color:#ff7b86}.bwo-xi-plan-role{position:absolute;z-index:3;display:grid;width:21px;height:21px;place-items:center;border:2px solid rgba(255,255,255,.9);border-radius:50%;background:#30343c;color:#fff;font-size:10px;font-weight:950}.bwo-xi-plan-role.captain{right:-7px;bottom:0}.bwo-xi-plan-role.striker{left:-7px;bottom:0;background:#253e59}.bwo-xi-plan-legend{display:flex;flex-wrap:wrap;justify-content:center;gap:8px;margin:11px auto 0;color:#aeb6c3;font-size:9px}.bwo-xi-plan-legend span{padding:4px 7px;border:1px solid #303640;border-radius:999px;background:#171b21}.bwo-xi-table{width:100%;border-collapse:collapse;table-layout:fixed}.bwo-xi-table th{padding:9px 8px;background:#191d22;color:#929baa;text-align:left;font-size:9px;letter-spacing:.07em;text-transform:uppercase}.bwo-xi-table td{padding:10px 8px;border-top:1px solid #282d35;font-size:12px;vertical-align:middle}.bwo-xi-table tbody tr:hover{background:#1c2026}.bwo-xi-player{display:flex;align-items:center;min-width:0;gap:9px}.bwo-xi-player img{width:36px;height:36px;flex:0 0 36px;border-radius:50%;background:#222831;object-fit:cover}.bwo-xi-player div{min-width:0}.bwo-xi-player strong,.bwo-xi-player small{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.bwo-xi-player small{margin-top:2px;color:#959ead;font-size:10px}.bwo-xi-pos{display:inline-flex;min-width:25px;height:25px;align-items:center;justify-content:center;border-radius:8px;background:#252b34;color:#fff;font-size:10px;font-weight:900}.bwo-xi-state{display:inline-flex;padding:4px 7px;border-radius:999px;background:#183121;color:#66e48b;font-size:10px;font-weight:800}.bwo-xi-state.warn{background:#362a15;color:#ffd067}.bwo-xi-state.bad{background:#3b1b21;color:#ff7884}.bwo-xi-main-score{color:#66e48b;font-size:14px}.bwo-xi-detail{padding:7px 9px;font-size:10px}
 .bwo-xi-prob{display:grid;gap:2px}.bwo-xi-prob strong{color:#fff;font-size:12px}.bwo-xi-prob small{color:#8f98a8;font-size:9px;line-height:1.25}.bwo-xi-prob small.high{color:#67e88d}.bwo-xi-side{display:grid;gap:14px}.bwo-xi-change-list,.bwo-xi-alt-list{display:grid}.bwo-xi-change,.bwo-xi-alt{padding:12px 14px;border-top:1px solid #292e36}.bwo-xi-change:first-child,.bwo-xi-alt:first-child{border-top:0}.bwo-xi-change-line{display:flex;align-items:center;justify-content:space-between;gap:8px}.bwo-xi-change-line strong{font-size:12px}.bwo-xi-gain{color:#62e68a;font-size:11px;font-weight:900}.bwo-xi-change p{margin:5px 0 0;color:#a7afbc;font-size:11px;line-height:1.45}.bwo-xi-alt{display:grid;grid-template-columns:1fr auto;gap:3px 10px}.bwo-xi-alt strong{font-size:12px}.bwo-xi-alt span{color:#64e48b;font-weight:900;font-size:12px}.bwo-xi-alt small{color:#959ead;font-size:10px}.bwo-xi-empty{padding:22px;color:#aeb5c1;text-align:center;font-size:12px}.bwo-xi-footer{display:flex;align-items:center;justify-content:space-between;gap:12px;margin-top:14px;padding:13px 15px;border:1px solid #303640;border-radius:12px;background:#171b21}.bwo-xi-footer p{margin:0;color:#aeb6c3;font-size:11px;line-height:1.45}.bwo-xi-round{border-color:#268b4a;background:#153d25;color:#77ef99;white-space:nowrap}
 .bwo-xi-plan-photo{height:62px}.bwo-xi-plan-photo>img{height:62px;border-radius:0;object-fit:contain;object-position:center bottom}
 .bwo-xi-plan-bench{margin:12px auto 0;padding:13px 14px 14px;border:1px solid #272c34;border-radius:13px;background:#08090b}.bwo-xi-plan-bench>h3{margin:0 0 11px;color:#fff;font-size:14px;text-align:center}.bwo-xi-plan-bench-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:10px}.bwo-xi-bench-player{display:flex;min-width:0;align-items:center;flex-direction:column;padding:8px 5px;border:1px solid transparent;border-radius:10px;background:transparent;color:#fff;cursor:pointer}.bwo-xi-bench-player:not(.empty):hover{border-color:#3d4652;background:#15191f}.bwo-xi-bench-photo{position:relative;display:grid;width:54px;height:54px;place-items:center;border-radius:50%;background:#242932;color:#aeb5c1;font-size:29px}.bwo-xi-bench-photo img{width:54px;height:54px;border-radius:50%;object-fit:cover;object-position:top center}.bwo-xi-bench-pos{position:absolute;right:-5px;bottom:-2px;display:grid;min-width:21px;height:21px;padding:0 3px;place-items:center;border:2px solid #08090b;border-radius:999px;color:#101318;background:#ffd51f;font-size:8px;font-weight:950}.bwo-xi-bench-pos[data-pos="2"]{background:#20b9f2}.bwo-xi-bench-pos[data-pos="3"]{background:#2ada58}.bwo-xi-bench-pos[data-pos="4"]{background:#ff2738;color:#fff}.bwo-xi-bench-player strong{display:block;max-width:100%;margin-top:7px;font-size:10px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.bwo-xi-bench-player small{margin-top:3px;color:#91e7a8;font-size:8px}.bwo-xi-bench-player.empty{cursor:default}.bwo-xi-bench-player.empty strong{color:#929aa7}
 @media(max-width:980px){.bwo-xi-kpis{grid-template-columns:repeat(3,1fr)}.bwo-xi-layout{grid-template-columns:1fr}.bwo-xi-table th:nth-child(4),.bwo-xi-table td:nth-child(4){display:none}}
 @media(max-width:680px){.bwo-xi-tool{border-radius:0;border-left:0;border-right:0}.bwo-xi-head{padding:15px 13px;display:block}.bwo-xi-head-main{display:block}.bwo-xi-back{margin-bottom:12px}.bwo-xi-head-actions{display:grid;grid-template-columns:1fr 1fr;margin-top:13px}.bwo-xi-refresh,.bwo-xi-league{width:100%;margin:0}.bwo-xi-title{font-size:23px}.bwo-xi-content{padding:12px}.bwo-xi-toolbar{align-items:stretch;flex-direction:column}.bwo-xi-select{width:100%}.bwo-xi-formations{grid-template-columns:1fr}.bwo-xi-kpis{grid-template-columns:repeat(2,1fr)}.bwo-xi-kpi{padding:11px}.bwo-xi-kpi strong{font-size:17px}.bwo-xi-table,.bwo-xi-table tbody{display:block}.bwo-xi-table thead{display:none}.bwo-xi-table tr{display:grid;grid-template-columns:minmax(0,1fr) auto auto;gap:8px;align-items:center;padding:11px 10px;border-top:1px solid #292e36}.bwo-xi-table td{padding:0;border:0}.bwo-xi-table td:nth-child(2){display:none}.bwo-xi-table td:nth-child(3){grid-column:1/2;color:#aeb6c3}.bwo-xi-table td:nth-child(4){display:block;grid-column:2}.bwo-xi-table td:nth-child(5){grid-column:3;grid-row:1}.bwo-xi-table td:nth-child(6){grid-column:2/4;grid-row:2;text-align:right}.bwo-xi-player img{width:42px;height:42px;flex-basis:42px}.bwo-xi-footer{align-items:stretch;flex-direction:column}.bwo-xi-round{width:100%}.bwo-xi-plan-wrap{padding:8px 3px}.bwo-xi-plan-pitch{height:430px;border-radius:9px}.bwo-xi-plan-player{width:76px}.bwo-xi-plan-photo,.bwo-xi-plan-photo>img{width:45px;height:45px}.bwo-xi-plan-name{max-width:76px;padding:2px 4px;font-size:8px}.bwo-xi-plan-meta span{padding:1px 3px;font-size:7px}.bwo-xi-plan-role{width:18px;height:18px;font-size:8px}.bwo-xi-team-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.bwo-xi-league-toolbar{align-items:stretch;flex-direction:column}.bwo-xi-league-pitch{height:430px}.bwo-xi-league-player{width:72px}.bwo-xi-league-player img,.bwo-xi-league-initial{width:38px;height:38px}.bwo-xi-league-player strong{max-width:72px;font-size:8px}.bwo-xi-league-prob{right:8px}.bwo-xi-league-alts{grid-template-columns:1fr}}
`;document.head.appendChild(e)}
function bwoMatchdayXIPosition(e){return({"1":"PT","2":"DF","3":"MC","4":"DL"})[String(e)]??"?"}
function bwoMatchdayXIProbability(e){let t=Number(e.consensus?.probability??e.jp?.probability??e.probability?.prob);return Number.isFinite(t)?Math.max(0,Math.min(98,t)):50}
function bwoMatchdayXIRisk(e){let t=bwoRoundNativeText(`${e.jp?.status??""} ${e.jp?.pick??""} ${e.jp?.form??""}`),n=e.consensus;return!!(e.physical?.unavailable||e.physical?.key==="risk"||bwoMatchdayXIProbability(e)<50||n&&n.available>=2&&!n.majorityStarter||/lesion|sancion|duda|riesgo|pufo|racha baja/.test(t))}
function bwoMatchdayXIValue(e){let t=e.projected==null?NaN:Number(e.projected),n=e.average==null?NaN:Number(e.average),r=bwoMatchdayXIProbability(e),o=Number.isFinite(t)?t:Number.isFinite(n)?n*Math.max(.15,r/100):0,a=e.jp?bwoRecJPBonus(e.jp):0,i=e.consensus,l=i?.available?((i.majorityStarter?1:-1)*i.confidence/100)*(Math.min(4,i.available)*.35):0;return e.physical?.unavailable?o-100:e.physical?.key==="risk"?o*.78+a+l:o+a+l}
function bwoMatchdayXIPlans(e){let t=e.players??[],n=t.filter(l=>l.lineup&&l.locked),r=[];for(let o of bwoMatchdayXIFormations){let a=[],i=!0;for(let l of["1","2","3","4"]){let s=n.filter(g=>g.position===l),u=o.slots[l];if(s.length>u){i=!1;break}let c=t.filter(g=>g.position===l&&!g.locked&&!s.some(b=>b.id===g.id)).sort((g,b)=>bwoMatchdayXIPlayerRank(b)-bwoMatchdayXIPlayerRank(g)),f=s.concat(c.slice(0,u-s.length));if(f.length<u){i=!1;break}a.push(...f)}if(i&&a.length===11){let l=a.reduce((s,u)=>s+(Number(u.projected)||0),0),s=a.reduce((u,c)=>u+bwoMatchdayXIPlayerRank(c),0),u=a.filter(bwoMatchdayXIRisk).length,c=a.filter(g=>!g.physical?.unavailable&&bwoMatchdayXIProbability(g)>=50&&(!g.consensus?.available||g.consensus.majorityStarter)).length;r.push({formation:o.name,players:a,projected:l,score:s,risks:u,reliable:c})}}return r.sort((o,a)=>a.score-o.score||a.projected-o.projected)}
function bwoMatchdayXIChanges(e,t){let n=new Set(t.players.map(a=>String(a.id))),r=new Set((e.lineup??[]).map(a=>String(a.id))),o=(e.lineup??[]).filter(a=>!n.has(String(a.id))&&!a.locked),a=t.players.filter(i=>!r.has(String(i.id)));return a.map(i=>{let l=o.findIndex(s=>s.position===i.position),s=l>=0?o.splice(l,1)[0]:null,u=(Number(i.projected)||0)-(Number(s?.projected)||0),c=s?.physical?.unavailable?`${s.name} no está disponible.`:s&&bwoMatchdayXIProbability(s)<50?`${s.name} tiene solo ${Math.round(bwoMatchdayXIProbability(s))}% de titularidad probable.`:s?`${i.name} ofrece una previsión superior para la misma posición.`:"Completa una plaza vacía del once actual.";return{in:i,out:s,gain:u,reason:c}}).sort((i,l)=>l.gain-i.gain)}
function bwoMatchdayXIKpi(e,t,n=""){let r=bwoBidsNode("article",`bwo-xi-kpi ${n}`);return r.append(bwoBidsNode("span",null,e),bwoBidsNode("strong",null,t)),r}
function bwoMatchdayXIMatch(e){let t=e.match??{};if(t.state!=="upcoming")return t.stateLabel??"Partido iniciado";let n="Hora pendiente",r=new Date(t.date);t.date&&!Number.isNaN(r.getTime())&&(n=new Intl.DateTimeFormat("es-ES",{weekday:"short",day:"numeric",hour:"2-digit",minute:"2-digit"}).format(r));return`${t.home?"vs":"@"} ${t.opponentName??"Rival"} · ${n}`}
function bwoMatchdayXIPlayerState(e){let t=bwoRoundNativeText(e.jp?.status);return e.physical?.unavailable||/lesion|sancion/.test(t)?{label:e.physical?.unavailable?e.physical.label:e.jp.status,tone:"bad"}:e.physical?.key==="risk"||/duda|riesgo|pufo|racha baja/.test(bwoRoundNativeText(`${t} ${e.jp?.pick??""} ${e.jp?.form??""}`))?{label:e.physical?.key==="risk"?e.physical.label:"En revisión",tone:"warn"}:bwoMatchdayXIProbability(e)<50?{label:"Titularidad baja",tone:"warn"}:{label:"Disponible",tone:""}}
function bwoMatchdayXIPlayerRow(e){let t=bwoBidsNode("tr"),n=bwoBidsNode("td"),r=bwoBidsNode("div","bwo-xi-player"),o=document.createElement("img");o.src=`https://cdn.biwenger.com/i/p/${e.id}.png`,o.alt="",o.loading="lazy",o.addEventListener("error",()=>o.style.visibility="hidden");let a=bwoBidsNode("div");a.append(bwoBidsNode("strong",null,e.name),bwoBidsNode("small",null,e.teamName||"Equipo sin dato")),r.append(o,a),n.appendChild(r);let i=bwoBidsNode("td"),l=bwoBidsNode("span","bwo-xi-pos",bwoMatchdayXIPosition(e.position));i.appendChild(l);let s=bwoBidsNode("td",null,bwoMatchdayXIMatch(e)),u=bwoBidsNode("td"),c=bwoMatchdayXIPlayerState(e),f=bwoBidsNode("span",`bwo-xi-state ${c.tone}`,c.label);u.appendChild(f);let g=bwoBidsNode("td"),h=bwoBidsNode("div","bwo-xi-prob"),E=bwoBidsNode("strong",null,`${Math.round(bwoMatchdayXIProbability(e))}% titular`),k=e.consensus?.available?bwoBidsNode("small",e.consensus.confidence>=90?"high":"",e.consensus.summary):bwoBidsNode("small",null,"Sin contraste externo");h.title=e.consensus?.signals?.map(d=>`${d.label}: ${d.starter?"titular":"suplente"} (${Math.round(d.probability)}%)`).join(" · ")??"",h.append(E,k),g.appendChild(h);let b=bwoBidsNode("td"),p=bwoBidsNode("strong","bwo-xi-main-score",e.projected==null?"—":`${Number(e.projected).toFixed(1)} pts`),y=bwoBidsNode("button","bwo-xi-detail","Ver ficha");y.type="button",y.addEventListener("click",()=>bwoClauseOpenDetail({id:e.id,name:e.name,catalog:e.catalog},y)),b.append(p,document.createTextNode(" "),y),t.append(n,i,s,u,g,b);return t}
function bwoMatchdayXIInstallPitch(e,t){let n=e.querySelector(".bwo-xi-layout>.bwo-xi-panel .bwo-xi-table");if(!n)return;let r=bwoBidsNode("div","bwo-xi-plan-wrap"),o=bwoBidsNode("div","bwo-xi-plan-pitch"),a=document.createElement("img");a.className="bwo-xi-pitch-ground",a.src="https://cdn.biwenger.com/cdn-cgi/image/f=avif/img/sports/football/ground.png",a.alt="Campo de fútbol",o.appendChild(a);let i={"1":88,"2":68,"3":43,"4":18},l=String(t.captain?.id??""),s=String(t.striker?.id??"");for(let a of["1","2","3","4"]){let n=t.players.filter(e=>String(e.position)===a).sort((e,t)=>bwoMatchdayXIPlayerRank(t)-bwoMatchdayXIPlayerRank(e));for(let[u,c]of n.entries()){let f=4+92*(u+1)/(n.length+1),g=bwoBidsNode("button","bwo-xi-plan-player");g.type="button",g.style.left=`${f}%`,g.style.top=`${i[a]}%`;let h=bwoBidsNode("span","bwo-xi-plan-photo"),E=document.createElement("img");E.src=`https://cdn.biwenger.com/i/p/${c.id}.png`,E.alt=c.name,E.loading="lazy",E.addEventListener("error",()=>E.style.visibility="hidden"),h.appendChild(E);let k=String(c.id);k===l&&h.appendChild(bwoBidsNode("span","bwo-xi-plan-role captain","C")),k===s&&h.appendChild(bwoBidsNode("span","bwo-xi-plan-role striker","⚽"));let b=bwoBidsNode("strong","bwo-xi-plan-name",c.name),p=bwoBidsNode("span","bwo-xi-plan-meta"),y=bwoMatchdayXIPlayerState(c),w=bwoBidsNode("span",y.tone||"good",`${Math.round(bwoMatchdayXIProbability(c))}%`),m=bwoBidsNode("span",c.projected==null?"":Number(c.projected)>=4?"good":"warn",c.projected==null?"—":`${Number(c.projected).toFixed(1)} pts`);p.append(w,m),g.append(h,b,p),g.title=`${c.name} · ${bwoMatchdayXIPosition(c.position)} · ${y.label} · ${bwoMatchdayXIMatch(c)}`,g.addEventListener("click",()=>bwoClauseOpenDetail({id:c.id,name:c.name,catalog:c.catalog},g)),o.appendChild(g)}}let u=bwoBidsNode("div","bwo-xi-plan-legend");u.append(bwoBidsNode("span",null,"C Capitán"),bwoBidsNode("span",null,"⚽ Ariete"),bwoBidsNode("span",null,"% Titularidad probable"),bwoBidsNode("span",null,"Pulsa un jugador para ver su ficha")),r.append(o,u),n.replaceWith(r);let c=r.closest(".bwo-xi-panel")?.querySelector(".bwo-xi-panel-head span");c&&(c.textContent=`${t.formation} · distribución táctica recomendada`)}
function bwoMatchdayXIBenchEnsureMobile(){if(document.getElementById("bwo-xi-bench-mobile"))return;let e=document.createElement("style");e.id="bwo-xi-bench-mobile",e.textContent='@media(max-width:680px){.bwo-xi-plan-bench{padding:11px 5px}.bwo-xi-plan-bench-grid{gap:2px}.bwo-xi-bench-player{padding:6px 2px}.bwo-xi-bench-photo,.bwo-xi-bench-photo img{width:44px;height:44px}.bwo-xi-bench-photo{font-size:24px}.bwo-xi-bench-player strong{max-width:72px;font-size:8px}.bwo-xi-bench-player small{font-size:7px}.bwo-xi-bench-pos{min-width:18px;height:18px;font-size:7px}}',document.head.appendChild(e)}
function bwoMatchdayXIBench(e){bwoMatchdayXIBenchEnsureMobile();let t=bwoBidsNode("section","bwo-xi-plan-bench"),n=bwoBidsNode("h3",null,"Suplentes · banquillo recomendado"),r=bwoBidsNode("div","bwo-xi-plan-bench-grid"),o=["1","2","3","4"];for(let[a,i]of o.entries()){let l=e.reserves?.[a]??null,s=bwoBidsNode("button",`bwo-xi-bench-player${l?"":" empty"}`);s.type="button",s.disabled=!l;let u=bwoBidsNode("span","bwo-xi-bench-photo");if(l){let e=document.createElement("img");e.src=`https://cdn.biwenger.com/i/p/${l.id}.png`,e.alt=l.name,e.loading="lazy",e.addEventListener("error",()=>e.replaceWith(document.createTextNode("👤"))),u.appendChild(e)}else u.appendChild(document.createTextNode("👤"));let c=String(l?.position??i),f=bwoBidsNode("span","bwo-xi-bench-pos",bwoMatchdayXIPosition(c));f.dataset.pos=c,u.appendChild(f);let g=bwoBidsNode("strong",null,l?.name??`Sin ${bwoMatchdayXIPosition(i)}`),h=l?`${Math.round(bwoMatchdayXIProbability(l))}% · ${l.projected==null?"—":`${Number(l.projected).toFixed(1)} pts`}`:"Plaza disponible";s.append(u,g,bwoBidsNode("small",null,h)),l&&(s.title=`${l.name} · suplente recomendado · ${bwoMatchdayXIMatch(l)}`,s.addEventListener("click",()=>bwoClauseOpenDetail({id:l.id,name:l.name,catalog:l.catalog},s))),r.appendChild(s)}return t.append(n,r),t}
var bwoMatchdayXIInstallPitchBase=bwoMatchdayXIInstallPitch;
bwoMatchdayXIInstallPitch=function(e,t){bwoMatchdayXIInstallPitchBase(e,t);let n=e.querySelector(".bwo-xi-plan-wrap"),r=n?.querySelector(".bwo-xi-plan-legend");n&&r&&r.insertAdjacentElement("beforebegin",bwoMatchdayXIBench(t))};
var bwoMatchdayXISaving=!1;
function bwoMatchdayXIGoTeam(){bwoCloseToolsPage();let e=document.querySelector("app-nav a[href='/team']")??document.querySelector("a[href='/team']");e?e.click():location.assign("/team")}
function bwoMatchdayXIIds(e){return(e??[]).map(t=>t?.id??t).filter(t=>t!=null&&t!==""&&Number(t)!==0).map(String)}
function bwoMatchdayXISlotIds(e){return(e??[]).map(t=>{let n=t?.id??t;return n==null||n===""?null:String(n)})}
function bwoMatchdayXILineupFromResponse(e,t=null){let n=e?.data??e,r=n?.lineup??n?.user?.lineup??null;if(!r&&n?.league?.standings){let o=n.league.standings.find(a=>t==null||String(a?.id)===String(t));r=o?.lineup??null}return r}
function bwoMatchdayXIPlayerRank(e){let t=bwoMatchdayXIValue(e),n=bwoMatchdayXIProbability(e),r=e.consensus,o=r?.available?((r.majorityStarter?1:-1)*r.confidence*.025+(r.starterVotes-r.benchVotes)*.32):0;return t+Math.min(98,Math.max(0,n))*.035+o-(e.physical?.unavailable?100:0)-(e.locked?2:0)}
function bwoMatchdayXIRoleRank(e,t){let n=bwoMatchdayXIPlayerRank(e),r=Number(e.projected)||0,o=e.consensus??{},a=t==="captain"?Number(o.captainVotes)||0:Number(o.strikerVotes)||0;return n+r*.7+bwoMatchdayXIProbability(e)*.018+a*6-(bwoMatchdayXIRisk(e)?5:0)}
function bwoMatchdayXICompleteSelection(e,t,n=4){let r=new Set(t.players.map(g=>String(g.id))),o=(e.players??[]).filter(g=>!r.has(String(g.id))&&!g.physical?.unavailable).sort((g,b)=>bwoMatchdayXIPlayerRank(b)-bwoMatchdayXIPlayerRank(g)),a=[],i=Math.max(0,Number(n)||0),l=["1","2","3","4"];for(let g=0;g<Math.min(i,l.length);g++)a.push(o.find(b=>b.position===l[g])??null);for(let g of o)if(a.length<i&&!a.some(b=>b&&String(b.id)===String(g.id)))a.push(g);let s=[...t.players].filter(g=>!g.physical?.unavailable),u=(s.filter(g=>!bwoMatchdayXIRisk(g)).length?s.filter(g=>!bwoMatchdayXIRisk(g)):s).sort((g,b)=>bwoMatchdayXIRoleRank(b,"captain")-bwoMatchdayXIRoleRank(g,"captain"))[0]??t.players[0]??null,c=s.filter(g=>g.position==="4").sort((g,b)=>bwoMatchdayXIRoleRank(b,"striker")-bwoMatchdayXIRoleRank(g,"striker")),f=c.find(g=>!u||String(g.id)!==String(u.id))??c[0]??s.find(g=>!u||String(g.id)!==String(u.id))??u;return{...t,reserves:a.slice(0,i),captain:u,striker:f,_allPlayers:e.players??[],_consensusMeta:e.consensusMeta??null}}
function bwoMatchdayXIRoundInfo(e,t){let n=e?.data??e,r=n?.round?.id??n?.activeRound?.id??n?.roundID??n?.league?.round?.id??n?.league?.activeRound?.id??n?.id??null,o=bwoMatchdayXILineupFromResponse(n,t);return{roundId:r,lineup:o,raw:n}}
function bwoMatchdayXIIdValue(e){return e==null?null:String(e?.id??e)}
function bwoMatchdayXILineupPayload(e,t=!1){let n=e.players.map(g=>{let b=Number(g.id);return Number.isFinite(b)?b:String(g.id)}),r=(e.reserves??[]).map(g=>{let b=Number(g.id);return Number.isFinite(b)?b:String(g.id)}),o=e.captain?{id:Number.isFinite(Number(e.captain.id))?Number(e.captain.id):String(e.captain.id)}:null,a=e.striker?{id:Number.isFinite(Number(e.striker.id))?Number(e.striker.id):String(e.striker.id)}:null;return t?{type:e.formation,playersID:n,reservesID:r,...o&&{captain:o},...a&&{striker:a}}:{type:e.formation,players:n.map(g=>({id:g})),reserves:r.map(g=>({id:g})),...o&&{captain:o},...a&&{striker:a}}}
async function bwoMatchdayXIHeaders(){let e=await Hi(),t=[...document.scripts].map(n=>n.src||"").join(" ").match(/\/app\/v(\d+)\//)?.[1]??document.querySelector('script[type="importmap"]')?.textContent?.match(/\/app\/v(\d+)\//)?.[1]??"631";return{...e,"X-Version":String(t)}}
async function bwoMatchdayXIResponseError(e,t){let n=await e.text().catch(()=>""),r=null;try{let o=JSON.parse(n);r=o?.data?.userMessage??o?.userMessage??o?.data?.message??o?.message}catch{}return new Error(r&&!/^unexpected error$/i.test(String(r))?String(r):`${t} (HTTP ${e.status})`)}
async function bwoMatchdayXISave(e,t){if(bwoMatchdayXISaving)return;let n=`Se abrirá Equipo y se guardará la formación ${e.formation} con:\n\nTitulares: ${e.players.map(g=>g.name).join(", ")}\n\nSuplentes: ${(e.reserves??[]).map(g=>g?.name??"Vacío").join(", ")||"sin suplentes"}\nCapitán: ${e.captain?.name??"—"}\nAriete: ${e.striker?.name??"—"}\n\n¿Quieres aplicar la alineación completa?`;if(!confirm(n))return;bwoMatchdayXISaving=!0;let r=t.textContent;t.disabled=!0,t.textContent="Abriendo Equipo…";try{let o=await bwoMatchdayXIHeaders(),a=bwoMatchdayXICompleteSelection({players:e._allPlayers??[...e.players,...(e.reserves??[])]},e,e.reserves?.length||4),i=a.players.map(h=>Number.isFinite(Number(h.id))?Number(h.id):String(h.id)),l=a.reserves.map(h=>h==null?0:Number.isFinite(Number(h.id))?Number(h.id):String(h.id)),s=a.captain?Number.isFinite(Number(a.captain.id))?Number(a.captain.id):String(a.captain.id):null,u=a.striker?Number.isFinite(Number(a.striker.id))?Number(a.striker.id):String(a.striker.id):null,c=`${qi}/user?fields=*,lineup(type,playersID,reservesID,captain,striker)`,f=async(h,E)=>{t.textContent=E;let k=await bwoApiFetch(c,{method:"PUT",headers:o,body:JSON.stringify({lineup:h})});if(!k.ok)throw await bwoMatchdayXIResponseError(k,E.replace("…",""));return k.json().catch(()=>null)},g={type:a.formation,playersID:i};await f(g,"Guardando once titular…");await f({...g,reservesID:l},"Asignando suplentes…");s&&await f({...g,reservesID:l,captain:s},"Asignando capitán…");u&&await f({...g,reservesID:l,...s&&{captain:s},striker:u},"Asignando ariete…");t.textContent="Comprobando alineación…";let b=await bwoApiFetch(c,{headers:o,cache:"no-store"});if(!b.ok)throw new Error(`La alineación se guardó, pero no se pudo comprobar (${b.status})`);let p=bwoMatchdayXILineupFromResponse(await b.json()),y=bwoMatchdayXIIds(p?.players??p?.playersID).sort(),w=i.map(String).sort();if(y.length!==11||y.join("|")!==w.join("|"))throw new Error("Biwenger no confirmó los 11 titulares");let d=bwoMatchdayXIIds(p?.reserves??p?.reservesID).sort(),v=l.filter(h=>Number(h)!==0).map(String).sort();if(d.length!==v.length||d.join("|")!==v.join("|"))throw new Error("Los titulares están guardados, pero Biwenger no confirmó los suplentes");let S=bwoMatchdayXIIdValue(p?.captain),x=bwoMatchdayXIIdValue(p?.striker);if(s&&S!==String(s))throw new Error("Titulares y suplentes guardados, pero Biwenger no confirmó el capitán");if(u&&x!==String(u))throw new Error("Titulares, suplentes y capitán guardados, pero Biwenger no confirmó el ariete");await Uf(w),bwoMatchdayXICache=null,bwoMatchdayXICacheAt=0,bwoRoundNativeDataCache=null,bwoRoundNativeDataAt=0,t.textContent="✓ Alineación completa guardada",bwoRecommendationToast(`Once, ${l.filter(h=>Number(h)!==0).length} suplentes, capitán y ariete guardados correctamente.`,"ok"),bwoMatchdayXIGoTeam()}catch(o){console.warn("[biwtools] No se pudo completar Once Jornada:",o),bwoRecommendationToast(o?.message??"No se pudo completar la alineación."),t.disabled=!1,t.textContent=r}finally{bwoMatchdayXISaving=!1}}
function bwoMatchdayXIRender(e,t,n=null){let r=t.querySelector(".bwo-xi-content");if(!r)return;let o=bwoMatchdayXIPlans(e);if(!o.length){r.replaceChildren(bwoBidsNode("div","bwo-xi-empty","No se puede construir un once reglamentario con los jugadores y partidos desbloqueados disponibles. Revisa que tu plantilla tenga al menos un portero, tres defensas, tres centrocampistas y un delantero."));return}let a=bwoMatchdayXICompleteSelection(e,o.find(m=>m.formation===n)??o[0]),i=bwoMatchdayXIChanges(e,a),l=(Number(a.projected)||0)-(Number(e.predicted)||0),s=bwoBidsNode("div","bwo-xi-toolbar"),u=bwoBidsNode("div","bwo-xi-toolbar-copy");u.append(bwoBidsNode("strong",null,"Configuración recomendada"),bwoBidsNode("span",null,"La formación automática maximiza previsión, disponibilidad y probabilidad de jugar."));let c=bwoBidsNode("select","bwo-xi-select");c.setAttribute("aria-label","Elegir formación");for(let m of o){let d=bwoBidsNode("option",null,`${m.formation}${m===o[0]?" · recomendada":""}`);d.value=m.formation,d.selected=m.formation===a.formation,c.appendChild(d)}c.addEventListener("change",()=>bwoMatchdayXIRender(e,t,c.value)),s.append(u,c);let f=bwoBidsNode("div","bwo-xi-kpis");f.append(bwoMatchdayXIKpi("Formación",a.formation,"good"),bwoMatchdayXIKpi("Previsión",`${a.projected.toFixed(1)} pts`,"good"),bwoMatchdayXIKpi("Frente a tu once",`${l>=0?"+":""}${l.toFixed(1)} pts`,l>=.1?"good":l<-.1?"bad":""),bwoMatchdayXIKpi("Titulares fiables",`${a.reliable}/11`,a.reliable>=10?"good":"warn"),bwoMatchdayXIKpi("En riesgo",String(a.risks),a.risks?"warn":"good"));let g=a.players.filter(m=>m.locked).length,b=bwoBidsNode("p",`bwo-xi-notice ${a.risks?"warn":"good"}`,g?`${e.stateLabel}: se han conservado ${g} jugador${g===1?"":"es"} cuyo partido ya está bloqueado. ${a.risks?"Revisa las alertas antes de confirmar el resto del once.":"El resto del once está disponible."}`:a.risks?`Hay ${a.risks} jugador${a.risks===1?"":"es"} con lesión, duda o baja probabilidad de titularidad. La propuesta los evita cuando existe una alternativa válida.`:"Once completo, sin bajas conocidas y con buena probabilidad de participación.");let p=bwoBidsNode("div","bwo-xi-layout"),y=bwoBidsNode("section","bwo-xi-panel"),w=bwoBidsNode("header","bwo-xi-panel-head");w.append(bwoBidsNode("h2",null,"Once recomendado"),bwoBidsNode("span",null,`${a.formation} · ordenado por posición`));let d=bwoBidsNode("table","bwo-xi-table"),v=bwoBidsNode("thead"),S=bwoBidsNode("tr");for(let m of["Jugador","Pos.","Partido","Estado","Titular","Previsión"])S.appendChild(bwoBidsNode("th",null,m));v.appendChild(S);let x=bwoBidsNode("tbody");for(let m of["1","2","3","4"])a.players.filter(d=>d.position===m).sort((d,v)=>bwoMatchdayXIValue(v)-bwoMatchdayXIValue(d)).forEach(d=>x.appendChild(bwoMatchdayXIPlayerRow(d)));d.append(v,x),y.append(w,d);let C=bwoBidsNode("aside","bwo-xi-side"),N=bwoBidsNode("section","bwo-xi-panel"),_=bwoBidsNode("header","bwo-xi-panel-head");_.append(bwoBidsNode("h2",null,"Cambios sugeridos"),bwoBidsNode("span",null,`${i.length} cambio${i.length===1?"":"s"}`));let L=bwoBidsNode("div","bwo-xi-change-list");if(!i.length)L.appendChild(bwoBidsNode("div","bwo-xi-empty","Tu once actual ya coincide con esta recomendación."));else for(let m of i){let d=bwoBidsNode("article","bwo-xi-change"),v=bwoBidsNode("div","bwo-xi-change-line");v.append(bwoBidsNode("strong",null,`${m.out?`${m.out.name} → `:"Añadir "}${m.in.name}`),bwoBidsNode("span","bwo-xi-gain",`${m.gain>=0?"+":""}${m.gain.toFixed(1)} pts`)),d.append(v,bwoBidsNode("p",null,m.reason)),L.appendChild(d)}N.append(_,L);let A=bwoBidsNode("section","bwo-xi-panel"),I=bwoBidsNode("header","bwo-xi-panel-head");I.append(bwoBidsNode("h2",null,"Roles y suplentes"),bwoBidsNode("span",null,"También se guardarán"));let D=bwoBidsNode("div","bwo-xi-alt-list");for(let[m,d,v]of[["Capitán",a.captain,"Mayor combinación de previsión, forma y fiabilidad"],["Ariete",a.striker,"Delantero con mejor expectativa de la jornada"]])if(d){let S=bwoBidsNode("article","bwo-xi-alt bwo-xi-role");S.append(bwoBidsNode("strong",null,`${m}: ${d.name}`),bwoBidsNode("span",null,d.projected==null?"—":`${Number(d.projected).toFixed(1)} pts`),bwoBidsNode("small",null,`${v} · ${Math.round(bwoMatchdayXIProbability(d))}% titular`)),D.appendChild(S)}if(!a.reserves.length)D.appendChild(bwoBidsNode("div","bwo-xi-empty","No hay suplentes disponibles."));else for(let[m,d]of a.reserves.entries()){let v=bwoBidsNode("article","bwo-xi-alt"),S=["PT","DF","MC","DL"][m]??`Puesto ${m+1}`;d?v.append(bwoBidsNode("strong",null,`${S}: ${d.name}`),bwoBidsNode("span",null,d.projected==null?"—":`${Number(d.projected).toFixed(1)} pts`),bwoBidsNode("small",null,`Suplente · ${bwoMatchdayXIPosition(d.position)} · ${Math.round(bwoMatchdayXIProbability(d))}% titular`)):v.append(bwoBidsNode("strong",null,`${S}: sin jugador compatible`),bwoBidsNode("span",null,"Vacío"),bwoBidsNode("small",null,"No se asignará otro jugador fuera de posición")),D.appendChild(v)}A.append(I,D),C.append(N,A),p.append(y,C);let P=bwoBidsNode("footer","bwo-xi-footer"),T=bwoBidsNode("p",null,"La propuesta usa Media AS + SofaScore, forma, disponibilidad, titularidad probable, rival y estado de la jornada. Al confirmar, guardará en Biwenger los 11 titulares, los suplentes, el capitán y el ariete."),O=bwoBidsNode("button","bwo-xi-round","Alinear y guardar");O.type="button",O.addEventListener("click",()=>bwoMatchdayXISave(a,O)),P.append(T,O),r.replaceChildren(s,f,g,p,P)}
var bwoMatchdayXIRenderConsensusBase=bwoMatchdayXIRender;
function bwoMatchdayXIFormationOverview(e,t,n=null){let r=t.querySelector(".bwo-xi-content"),o=r?.querySelector(".bwo-xi-source-strip");if(!r||!o)return;r.querySelector(".bwo-xi-formations")?.remove();let a=bwoMatchdayXIPlans(e).slice(0,3),i=n??a[0]?.formation;if(!a.length)return;let l=bwoBidsNode("section","bwo-xi-formations");for(let s of a){let u=bwoBidsNode("button",`bwo-xi-formation${s.formation===i?" active":""}`),c=s.players.filter(d=>bwoMatchdayXIRisk(d)).length,f=Math.max(0,11-c);u.type="button",u.append(bwoBidsNode("strong",null,`${s.formation}${s===a[0]?" · mejor opción":""}`),bwoBidsNode("span",null,`Previsión `),bwoBidsNode("b",null,`${s.projected.toFixed(1)} pts`),bwoBidsNode("span",null,` · ${f}/11 fiables · ${c} en riesgo`)),u.addEventListener("click",()=>bwoMatchdayXIRender(e,t,s.formation)),l.appendChild(u)}o.insertAdjacentElement("afterend",l)}
bwoMatchdayXIRender=function(e,t,n=null){bwoMatchdayXIRenderConsensusBase(e,t,n);let r=e.consensusMeta,o=t.querySelector(".bwo-xi-content");if(!o||!r)return;let a=r.sources.filter(d=>d.available),i=bwoBidsNode("div","bwo-xi-source-strip");Object.assign(i.style,{display:"flex",alignItems:"center",flexWrap:"wrap",gap:"7px",margin:"0 0 14px",padding:"10px 12px",border:"1px solid #303640",borderRadius:"10px",background:"#15191f",color:"#aeb6c3",fontSize:"10px"}),i.appendChild(bwoBidsNode("strong",null,`Consenso activo: ${a.length}/${r.total} fuentes`));for(let d of r.sources){let v=bwoBidsNode(d.available?"a":"span",null,`${d.available?"✓":"—"} ${d.label}`);Object.assign(v.style,{padding:"4px 7px",border:"1px solid "+(d.available?"rgba(95,203,114,.35)":"rgba(255,255,255,.09)"),borderRadius:"999px",color:d.available?"#7de99a":"#7f8794",textDecoration:"none"}),d.available&&(v.href=d.url,v.target="_blank",v.rel="noopener noreferrer"),i.appendChild(v)}let l=o.querySelector(".bwo-xi-kpis");l?.insertAdjacentElement("afterend",i);let s=bwoMatchdayXIPlans(e);if(!s.length)return;let u=bwoMatchdayXICompleteSelection(e,s.find(d=>d.formation===n)??s[0]);bwoMatchdayXIInstallPitch(t,u);let c=t.querySelectorAll(".bwo-xi-role small");u.captain&&c[0]&&(c[0].textContent=`Mayor previsión y fiabilidad · ${u.captain.consensus?.summary??"dato Biwenger"}`),u.striker&&c[1]&&(c[1].textContent=`Delantero con mejor expectativa · ${u.striker.consensus?.summary??"dato Biwenger"}`);let f=t.querySelector(".bwo-xi-footer p");f&&(f.textContent=`La propuesta cruza Media AS + SofaScore con las alineaciones de ${a.map(d=>d.label).join(", ")||"Biwenger"}. El 98 % solo aparece cuando las cuatro fuentes coinciden. Al confirmar se guardan titulares, suplentes, capitán y ariete.`);let g=t.querySelector(".bwo-xi-subtitle");g&&(g.textContent="Centro unificado: optimiza tu once, formación, banquillo y roles, y consulta desde aquí las alineaciones utilizadas en toda la liga."),bwoMatchdayXIFormationOverview(e,t,u.formation)};
async function bwoMatchdayXILoad(e=!1){if(!e&&bwoMatchdayXICache&&Date.now()-bwoMatchdayXICacheAt<9e4)return bwoMatchdayXICache;let t=!e&&bwoRoundNativeDataCache&&Date.now()-bwoRoundNativeDataAt<9e4?bwoRoundNativeDataCache:await bwoRoundData(),[n,r]=await Promise.all([!e&&t.jp instanceof Map?Promise.resolve(t.jp):bwoRoundJpData(t,e).catch(()=>new Map),bwoXIExternalData(t,e).catch(()=>({players:new Map,sources:[]}))]);t.jp=n;for(let o of t.players??[]){o.jp=n.get(String(o.id))??null,o.consensus=bwoXIConsensusBuild(o,o.jp,r.players.get(String(o.id))??[]);let a=bwoRoundProjected(o.average,o.physical,{prob:bwoMatchdayXIProbability(o)},o.match);a!=null&&(o.projected=a)}let o={key:"jp",label:"Jornada Perfecta",url:"https://www.jornadaperfecta.com/onces-posibles/",available:n.size>0,count:n.size};return t.consensusMeta={sources:[o,...(r.sources??[])],available:[o,...(r.sources??[])].filter(a=>a.available).length,total:4,updatedAt:Date.now()},t.predicted=(t.lineup??[]).reduce((a,i)=>a+(Number(i.projected)||0),0),bwoMatchdayXICache=t,bwoMatchdayXICacheAt=Date.now(),t}
async function bwoMatchdayXILeagueData(){let[e,t]=await Promise.all([fetch("https://biwtools.es/catalog.json",{cache:"no-store"}),fetch("https://biwtools.es/lineups.json",{cache:"no-store"})]);if(!e.ok||!t.ok)throw new Error(`Datos no disponibles: catálogo ${e.status}, onces ${t.status}`);let[n,r]=await Promise.all([e.json(),t.json()]),o=new Map,a=new Map,i=new Map(Object.entries(r.players??{}));for(let e of Array.isArray(n)?n:[]){a.set(String(e.id),e.name);let t=i.get(String(e.id));if(!t?.titular||!e.team_id||!e.team_name||null==t.x||null==t.y)continue;let n=String(e.team_id),r=o.get(n);r||(r={teamId:n,teamName:String(e.team_name),players:[],alternatives:[]},o.set(n,r)),r.players.push({id:String(e.id),name:e.name||"Jugador",injured:Number(t.lesion)>=0,prob:t.prob,x:Number(t.x),y:Number(t.y)})}for(let e of Array.isArray(r.extras)?r.extras:[]){let t=String(e.team_id??"");if(!t)continue;let n=o.get(t);n&&n.players.push({id:null,name:e.name||"Jugador",injured:Number(e.lesion)>=0,prob:e.prob,x:Number(e.x),y:Number(e.y)})}for(let[e,t]of Object.entries(r.alternatives??{})){let n=o.get(String(e));n&&(n.alternatives=(Array.isArray(t)?t:[]).map(e=>({id:e.id?String(e.id):null,name:e.id?a.get(String(e.id))??e.name:e.name||"Jugador",injured:Number(e.lesion)>=0,prob:e.prob,position:e.posicion})))}return[...o.values()].sort((e,t)=>e.teamName.localeCompare(t.teamName,"es"))}
function bwoMatchdayXILeagueInitial(e){let t=bwoBidsNode("span","bwo-xi-league-initial",String(e||"?").trim().charAt(0).toUpperCase());return t}
function bwoMatchdayXILeagueAvatar(e,t=""){if(e){let n=document.createElement("img");return n.src=`https://cdn.biwenger.com/i/p/${e}.png`,n.alt=t,n.loading="lazy",n.onerror=()=>n.replaceWith(bwoMatchdayXILeagueInitial(t)),n}return bwoMatchdayXILeagueInitial(t)}
function bwoMatchdayXILeagueGrid(e,t){if(!t?.isConnected)return;t.replaceChildren();let n=bwoBidsNode("div","bwo-xi-league-toolbar"),r=bwoBidsNode("div"),o=bwoBidsNode("strong","",`${e.length} equipos sincronizados`),a=bwoBidsNode("span","","Selecciona un equipo para consultar su once probable"),i=bwoBidsNode("span","","Fuente: lineups.json · actualización diaria");r.append(o,a),n.append(r,i),t.appendChild(n);if(!e.length){t.appendChild(bwoBidsNode("div","bwo-xi-empty","No hay alineaciones disponibles ahora mismo. Pulsa de nuevo cuando se hayan actualizado los datos de la jornada."));return}let l=bwoBidsNode("div","bwo-xi-team-grid");for(let n of e){let r=bwoBidsNode("button","bwo-xi-team-card");r.type="button";let o=document.createElement("img");o.src=`https://cdn.biwenger.com/i/t/${n.teamId}.png`,o.alt=n.teamName,o.loading="lazy";let a=bwoBidsNode("strong","",n.teamName),i=bwoBidsNode("span","",`${n.players.length} titulares · ${n.alternatives.length} alternativas`);r.append(o,a,i),r.addEventListener("click",()=>bwoMatchdayXILeagueTeam(n,t,e)),l.appendChild(r)}t.appendChild(l)}
function bwoMatchdayXILeagueProb(e){let t=String(e??"").trim();return t?t.endsWith("%")?t:`${t}%`:""}
function bwoMatchdayXILeagueTeam(e,t,n){if(!t?.isConnected)return;t.replaceChildren();let r=bwoBidsNode("div","bwo-xi-league-toolbar"),o=bwoBidsNode("button","bwo-xi-back","← Todos los equipos");o.type="button",o.addEventListener("click",()=>bwoMatchdayXILeagueGrid(n,t));let a=bwoBidsNode("div"),i=bwoBidsNode("strong","",e.teamName),l=bwoBidsNode("span","",`${e.players.length} titulares · ${e.alternatives.length} alternativas`);a.append(i,l),r.append(o,a),t.appendChild(r);let s=bwoBidsNode("div","bwo-xi-league-pitch"),u=document.createElement("img");u.src="https://cdn.biwenger.com/cdn-cgi/image/f=avif/img/sports/football/ground.png",u.alt="Campo de fútbol",s.appendChild(u);for(let t of e.players){let e=bwoBidsNode("div","bwo-xi-league-player");e.style.left=`${5+(Number(t.x)||0)*.9}%`,e.style.top=`${5+(Number(t.y)||0)*.9}%`;let n=bwoMatchdayXILeagueAvatar(t.id,t.name);e.appendChild(n);let r=bwoMatchdayXILeagueProb(t.prob);r&&e.appendChild(bwoBidsNode("span","bwo-xi-league-prob",r)),e.appendChild(bwoBidsNode("strong","",`${t.name}${t.injured?" · ⚕":""}`)),s.appendChild(e)}t.appendChild(s);let c=bwoBidsNode("div","bwo-xi-section-title","Alternativas del equipo");t.appendChild(c);if(!e.alternatives.length){t.appendChild(bwoBidsNode("div","bwo-xi-empty","No hay alternativas publicadas para este equipo."));return}let f=bwoBidsNode("div","bwo-xi-league-alts");for(let t of e.alternatives){let e=bwoBidsNode("div","bwo-xi-league-alt"),n=bwoMatchdayXILeagueAvatar(t.id,t.name),r=bwoBidsNode("strong","",`${t.name}${t.injured?" · ⚕":""}`),o=bwoBidsNode("span","",bwoMatchdayXILeagueProb(t.prob)||t.position||"Alternativa");e.append(n,r,o),f.appendChild(e)}t.appendChild(f)}
function bwoMatchdayXICloseLeagueLineups(){let e=document.querySelector(".bwo-xi-league-view"),t=document.querySelector(".bwo-tools-page-host>.bwo-xi-tool");e?.remove(),t&&(t.hidden=!1,t.scrollIntoView({block:"start"}))}
async function bwoMatchdayXIOpenLeagueLineups(){let e=document.querySelector(".bwo-tools-page-host"),t=e?.querySelector(":scope>.bwo-xi-tool");if(!e||!t)return;t.hidden=!0,e.querySelector(".bwo-xi-league-view")?.remove();let n=bwoBidsNode("section","bwo-xi-tool bwo-xi-league-view"),r=bwoBidsNode("header","bwo-xi-head"),o=bwoBidsNode("div","bwo-xi-head-main"),a=bwoBidsNode("button","bwo-xi-back","← Volver al optimizador"),i=bwoBidsNode("div"),l=bwoBidsNode("span","bwo-xi-eyebrow","BIWTOOLS · ONCES DE LA LIGA"),s=bwoBidsNode("h1","bwo-xi-title","Alineaciones y formaciones"),u=bwoBidsNode("p","bwo-xi-subtitle","Consulta los onces probables, alternativas y disponibilidad de cada equipo en una vista integrada."),c=bwoBidsNode("main","bwo-xi-content bwo-xi-league-content"),f=bwoBidsNode("div","bwo-tools-detail-loading","Cargando onces de la liga…");a.type="button",a.addEventListener("click",bwoMatchdayXICloseLeagueLineups),i.append(l,s,u),o.append(a,i),r.appendChild(o),c.appendChild(f),n.append(r,c),e.appendChild(n);try{let e=await bwoMatchdayXILeagueData();n.isConnected&&bwoMatchdayXILeagueGrid(e,c)}catch(e){console.warn("Biwtools: no se pudieron cargar los onces de la liga",e);let t=bwoBidsNode("div","bwo-xi-empty"),n=bwoBidsNode("strong","","No se pudieron cargar los onces de la liga"),r=bwoBidsNode("span","",e?.message||"Error de datos desconocido"),o=bwoBidsNode("button","bwo-xi-refresh","Reintentar");o.type="button",o.addEventListener("click",bwoMatchdayXIOpenLeagueLineups),t.append(n,r,o),c.replaceChildren(t)}}
async function bwoOpenMatchdayXI(e=!1){bwoMatchdayXIEnsureStyles();let t=document.querySelector(".bwo-tools-page-host");if(!t)return;let n=bwoBidsNode("section","bwo-xi-tool"),r=bwoBidsNode("header","bwo-xi-head"),o=bwoBidsNode("div","bwo-xi-head-main"),a=bwoBidsNode("button","bwo-xi-back","← Herramientas"),i=bwoBidsNode("div"),l=bwoBidsNode("span","bwo-xi-eyebrow","BIWTOOLS · CENTRO UNIFICADO DE ALINEACIONES"),s=bwoBidsNode("h1","bwo-xi-title","Once"),u=bwoBidsNode("p","bwo-xi-subtitle","Optimiza titulares, suplentes, capitán y ariete y consulta los onces de toda la liga desde una única herramienta."),c=bwoBidsNode("div","bwo-xi-head-actions"),f=bwoBidsNode("button","bwo-xi-league","Onces de la liga"),g=bwoBidsNode("button","bwo-xi-refresh",e?"Actualizando…":"Actualizar análisis");a.type=f.type=g.type="button",g.disabled=e,a.addEventListener("click",bwoClauseReturnTools),f.addEventListener("click",bwoMatchdayXIOpenLeagueLineups),g.addEventListener("click",()=>bwoOpenMatchdayXI(!0)),i.append(l,s,u),o.append(a,i),c.append(f,g),r.append(o,c);let b=bwoBidsNode("main","bwo-xi-content"),p=bwoBidsNode("div","bwo-xi-empty",e?"Actualizando plantilla y contrastando las cuatro fuentes…":"Analizando tu plantilla, alineaciones externas y todas las formaciones válidas…");b.appendChild(p),n.append(r,b),t.replaceChildren(n);try{let y=await bwoMatchdayXILoad(e);document.contains(n)&&bwoMatchdayXIRender(y,n)}catch(y){console.warn("[biwtools] No se pudo construir Once:",y),b.replaceChildren(bwoBidsNode("div","bwo-xi-empty","No se pudo preparar el once con la sesión actual. Espera unos segundos y pulsa Actualizar análisis; la herramienta respetará automáticamente el límite de peticiones de Biwenger."))}}
{let e=Math.max(0,bwoToolsCatalog.findIndex(t=>t.name==="Onces"||t.name==="Once Jornada"));for(let t=bwoToolsCatalog.length-1;t>=0;t--)(bwoToolsCatalog[t].name==="Onces"||bwoToolsCatalog[t].name==="Once Jornada")&&bwoToolsCatalog.splice(t,1);bwoToolsCatalog.splice(e,0,{icon:"🧠",name:"Once",category:"Jornada y rendimiento",status:"Centro unificado · consenso hasta 98 %",tone:"green",featured:!0,description:"Optimiza tu once, suplentes y roles y consulta las alineaciones y formaciones utilizadas en la liga desde el mismo módulo.",features:["Mejor formación automática","Consenso de 4 fuentes","Suplentes, capitán y ariete","Onces de toda la liga"],source:"Biwenger + FútbolFantasy + Comuniate + Jornada Perfecta + Analítica Fantasy"})}
var bwoOpenToolFromPageBeforeMatchdayXI=bwoOpenToolFromPage;
bwoOpenToolFromPage=function(e){return e==="Once"||e==="Once Jornada"||e==="Onces"?bwoOpenMatchdayXI(!1):bwoOpenToolFromPageBeforeMatchdayXI(e)};

/* Navegación móvil integrada: misma entrada Herramientas que en escritorio, sin FAB. */
function bwoMobileToolsNavEnsureStyles(){let e=document.getElementById("bwo-mobile-tools-nav-styles");if(e?.dataset.bwoVersion==="1.6.2")return;e||(e=document.createElement("style"),e.id="bwo-mobile-tools-nav-styles"),e.dataset.bwoVersion="1.6.2",e.textContent=`
 @media(max-width:768px){
  ...
  app-nav linear-tabs>ul[role="navigation"]{display:flex!important;width:100%!important;max-width:100vw!important;min-width:0!important;box-sizing:border-box!important;margin:0!important;padding:0 7px!important;overflow:hidden!important}
  ...
  app-nav linear-tabs>ul[role="navigation"]>li[data-bwo-tools-nav="1"]{display:block!important;flex-grow:1.18!important}
  ...
  app-nav linear-tabs>ul[role="navigation"]>li[data-bwo-tools-nav="1"]>a{color:#d4d7dd!important;font-size:0!important;letter-spacing:-.03em!important}
  app-nav linear-tabs>ul[role="navigation"]>li[data-bwo-tools-nav="1"]>a::after{content:"Herramientas"!important;display:block!important;max-width:100%!important;overflow:hidden!important;color:inherit!important;font-size:7.5px!important;font-weight:700!important;line-height:9px!important;text-overflow:clip!important;white-space:nowrap!important}
...
globalThis.__BIWTOOLS_BALANCE_FIX__="v1.9.0-decision-suite";

/* idb-keyval abre normalmente la base sin versión. Si otro bundle dejó creada
   `keyval-store` sin el almacén `keyval`, IndexedDB no dispara upgrade y toda
   herramienta que lea el histórico falla. Esta apertura valida el esquema y
   eleva la versión solo cuando hace falta, conservando los datos existentes. */
var bwoKeyvalDbPromise=null;
function bwoOpenKeyvalDatabase(){if(bwoKeyvalDbPromise)return bwoKeyvalDbPromise;let e="keyval-store",t="keyval";return bwoKeyvalDbPromise=new Promise((n,r)=>{let o=indexedDB.open(e);o.onupgradeneeded=()=>{o.result.objectStoreNames.contains(t)||o.result.createObjectStore(t)},o.onerror=()=>r(o.error??new Error("No se pudo abrir el almacenamiento local")),o.onsuccess=()=>{let a=o.result;if(a.objectStoreNames.contains(t)){a.onversionchange=()=>{a.close(),bwoKeyvalDbPromise=null,bu=void 0},n(a);return}let i=a.version+1;a.close();let l=indexedDB.open(e,i);l.onupgradeneeded=()=>{l.result.objectStoreNames.contains(t)||l.result.createObjectStore(t)},l.onerror=()=>r(l.error??new Error("No se pudo reparar el almacenamiento local")),l.onblocked=()=>r(new Error("El almacenamiento está bloqueado por otra pestaña de Biwenger")),l.onsuccess=()=>{let s=l.result;s.onversionchange=()=>{s.close(),bwoKeyvalDbPromise=null,bu=void 0},n(s)}}}),bwoKeyvalDbPromise.catch(()=>{bwoKeyvalDbPromise=null}),bwoKeyvalDbPromise}
bu=void 0;
kf=function(){return bu||(bu=(e,t)=>bwoOpenKeyvalDatabase().then(n=>{let r=n.transaction("keyval",e);return t(r.objectStore("keyval"))})),bu};

/* Fuentes JSON canónicas presentes en el bundle público. Los cargadores
   existentes usan `cn`, por lo que se fija aquí el mismo origen exacto. */
var bwoCanonicalJson=Object.freeze({
  catalog:"https://biwtools.es/catalog.json",
  fixtures:"https://biwtools.es/fixtures.json",
  injuries:"https://biwtools.es/injuries.json",
  lineups:"https://biwtools.es/lineups.json",
  priceHistory:"https://biwtools.es/price-history.json"
});
cn="https://biwtools.es";

/* Endpoint de liga idéntico al bundle original: solicita usuarios expandidos,
   aplica el segundo formato compatible y conserva `created` en el fallback. */
var bwoExpandedLeagueFallback=!1,bwoLeagueCreatedCache=new Map;
function bwoLeagueHasUsers(e){let t=e?.data?.users;return Array.isArray(t)&&t.some(n=>n?.id&&n?.name)}
function bwoLeagueKeepCreated(e,t){return e?.data==null||typeof e.data.created==="number"||typeof t!=="number"?e:{...e,data:{...e.data,created:t}}}
async function bwoFetchLeagueEndpoint(e,t){let n=await fetch(e,{headers:t});if(!n.ok)throw new Error(`getLeague failed: ${n.status} ${await n.text()}`);return n.json()}
zg=async function(){let e=await Hi(),t=ke();if(bwoExpandedLeagueFallback&&bwoLeagueCreatedCache.has(t))return bwoLeagueKeepCreated(await bwoFetchLeagueEndpoint(`${qi}/league`,e),bwoLeagueCreatedCache.get(t));let n=await bwoFetchLeagueEndpoint(`${qi}/league?fields=*,users(*)`,e);if(typeof n?.data?.created==="number"&&bwoLeagueCreatedCache.set(t,n.data.created),bwoLeagueHasUsers(n))return n;let r=await bwoFetchLeagueEndpoint(`${qi}/league?fields=*,users`,e);if(typeof r?.data?.created==="number"&&bwoLeagueCreatedCache.set(t,r.data.created),bwoLeagueHasUsers(r))return r;let o=await bwoFetchLeagueEndpoint(`${qi}/league`,e);return bwoLeagueHasUsers(o)?(bwoExpandedLeagueFallback=!0,bwoLeagueKeepCreated(o,bwoLeagueCreatedCache.get(t))):n};

/* Motor de saldos alineado con el bundle público 1.7.20. */
var bwoBalanceFixedDistributions={1:2e7,9:0,10:5e7,11:45e6,3:4e7,7:2e7,8:1e7};
var bwoBalanceTeamValueDistributions={12:5e7,2:4e7,5:2e7,6:1e7};
var bwoBalanceIncomingTypes=new Set(["compra","clausula_compra","cesion_pago","admin_compra","entrada_no_explicada","entrada_cesion_devuelta"]);
var bwoBalanceOutgoingTypes=new Set(["venta","clausula_venta","cesion_cobro","admin_venta","salida_no_explicada","salida_cesion_devuelta"]);
var bwoBalanceEventTypes=["market","transfer","roundFinished","exchange","loan","loanReturn","clauseIncrement","bonus","adminTransfer"];
var bwoBalanceCoreBoardEvent=Ug;

async function bwoBalanceStoreMovement(e,t=!1){if(!e?.player?.id)return;await zo({id:String(e.player.id),name:e.player.name??`Usuario ${e.player.id}`,icon:e.player.icon??null});let n={id:e.id,player_id:String(e.player.id),type:e.type,description:e.description,amount:Math.round(Number(e.amount??0)),occurred_at:e.occurredAt,raw_json:JSON.stringify(e.raw??{}),player_ref_id:e.footballPlayerId!=null?String(e.footballPlayerId):null},r=await z();if(t){let o=r.state.movements.findIndex(a=>a.id===n.id);if(o>=0){let a=r.state.movements[o];if(Nu(a)===Nu(n)||n.occurred_at<a.occurred_at)return;r.movementFingerprints.delete(Nu(a)),r.state.movements[o]=n,r.movementFingerprints.add(Nu(n));return}}await Tf(n)}
function bwoBalanceEventKey(e){return e?.player!=null?String(e.player):`${e?.from?.id??"x"}-${e?.to?.id??"x"}-${Number(e?.amount??0)}`}
Ug=async function(e,t={}){let n=new Date((e.date??Date.now()/1e3)*1e3).toISOString(),r=`${e.type}:${e.date}`,o=e.content;if(e.type==="loanReturn"){for(let a of Array.isArray(o)?o:[o]){if(a?.player==null)continue;a.to&&await bwoBalanceStoreMovement({id:`${r}:${a.player}:to`,player:a.to,type:"entrada_cesion_devuelta",description:"Recuperó un jugador cedido",amount:0,occurredAt:n,raw:e,footballPlayerId:a.player}),a.from&&await bwoBalanceStoreMovement({id:`${r}:${a.player}:from`,player:a.from,type:"salida_cesion_devuelta",description:"Devolvió un jugador cedido",amount:0,occurredAt:n,raw:e,footballPlayerId:a.player})}return}if(e.type==="clauseIncrement"){let a=Array.isArray(o)?o:[o];for(let i=0;i<a.length;i++){let l=o[i];if(!l?.user)continue;let s=Number(l.amount??0),u=s<0;await bwoBalanceStoreMovement({id:`${r}:${bwoBalanceEventKey(l)}:${i}`,player:l.user,type:u?"bajada_clausula":"subida_clausula",description:u?"Recuperó parte del depósito de una cláusula":"Subió la cláusula de un jugador",amount:-s,occurredAt:n,raw:e,footballPlayerId:l.player})}return}if(e.type==="adminTransfer"){for(let a of Array.isArray(o)?o:[o]){let i=Number(a.amount??0),l=bwoBalanceEventKey(a);a.to&&await bwoBalanceStoreMovement({id:`${r}:${l}:buyer`,player:a.to,type:"admin_compra",description:"Traspaso del administrador (pagó)",amount:-i,occurredAt:n,raw:e,footballPlayerId:a.player}),a.from&&await bwoBalanceStoreMovement({id:`${r}:${l}:seller`,player:a.from,type:"admin_venta",description:"Traspaso del administrador (cobró)",amount:i,occurredAt:n,raw:e,footballPlayerId:a.player})}return}if(e.type==="bonus"){for(let a of Array.isArray(o)?o:[o])a?.user&&await bwoBalanceStoreMovement({id:`${r}:${a.user.id}`,player:a.user,type:"abono",description:a.reason?`Abono: ${a.reason}`:"Abono/penalización del administrador",amount:Number(a.amount??0),occurredAt:n,raw:e});return}if(e.type==="roundFinished"){let a=o?.results??[],i=o?.round?.name??"",l=["bonusRoundPosition","bonusLeaguePosition","bonusRoundPointsPosition"],s=a.some(c=>{let f=c?.reason;return f!=null&&l.some(g=>g in f)}),u=o?.round?.part!=null&&!s||t.splitRound==="split"?i.trim():i.replace(/\s*\([^)]*\)\s*$/,"").trim(),c=u?`round:${u}`:r;for(let f of a)f?.user&&await bwoBalanceStoreMovement({id:`roundFinished:${c}:${f.user.id}`,player:f.user,type:"jornada",description:i||"Cierre de jornada",amount:Number(f.bonus??0),occurredAt:n,raw:e},!0);return}return bwoBalanceCoreBoardEvent(e)};
ec=bwoBalanceEventTypes;

$w=async function(){let e=0,t=await Fg().catch(()=>({}));for(let n of bwoBalanceEventTypes){let r=`lastSeenDate:${n}`,o=Number(await fn(r)??0),a=0,i=!1,l=o,s=!0;for(;!i;){s||await new Promise(c=>setTimeout(c,300)),s=!1;let u=await Ui({type:n,offset:a,limit:50}),f=u?.data??u??[];if(f.length===0)break;for(let c of f){if(c.date<=o){i=!0;break}await Ug(c,{splitRound:t?.splitRound}),e++}a===0&&f[0]&&(l=Math.max(l,f[0].date)),a+=50;if(f.length<50)break}await Le(r,String(l)),await Cu()}return e};

function bwoBalanceValidAnchorEvent(e){return e?.type!=="leagueReset"||e?.content?.type==="squadsAndBalances"}
async function bwoBalanceAnchorOfType(e){try{let t=await Ui({type:e,offset:0,limit:20}),n=(t?.data??t??[]).filter(bwoBalanceValidAnchorEvent).reduce((r,o)=>!r||(o.date??0)>(r.date??0)?o:r,null);return n?{date:new Date((n.date??0)*1e3).toISOString(),distribution:typeof n?.content?.distribution==="number"?n.content.distribution:null}:null}catch{return null}}
async function bwoBalanceGenericAnchor(){try{for(let e=0;e<1500;e+=50){let t=await Ui({offset:e,limit:50}),n=t?.data??t??[];if(!n.length)break;let r=n.find(o=>(o.type==="seasonFinished"||o.type==="leagueReset"||o.type==="seasonStarted")&&bwoBalanceValidAnchorEvent(o));if(r)return{date:new Date((r.date??0)*1e3).toISOString(),distribution:typeof r?.content?.distribution==="number"?r.content.distribution:null};if(n.length<50)break}}catch{}return null}
async function bwoBalanceResolveAnchor(e){let t=await fn("seasonAnchorDate"),n=await fn("seasonDistribution"),r=t?{date:t,distribution:n!=null&&n!==""?Number(n):null}:null,o=(await Promise.all([bwoBalanceAnchorOfType("seasonFinished"),bwoBalanceAnchorOfType("leagueReset"),bwoBalanceAnchorOfType("seasonStarted")])).filter(Boolean).sort((a,i)=>a.date.localeCompare(i.date)).pop()??null;return o&&(!r||o.date>r.date)?o:r??await bwoBalanceGenericAnchor()??{date:e,distribution:null}}
Dw=async function(){let e=await zg(),t=typeof e?.data?.created==="number"?new Date(e.data.created*1e3).toISOString():"1970-01-01T00:00:00.000Z";return(await bwoBalanceResolveAnchor(t)).date};

var bwoBalancePriceCache=new Map;
async function bwoBalancePriceAt(e,t){let n=String(e),r=bwoBalancePriceCache.get(n);if(!r){r=(async()=>{try{let o=await fetch(`https://cf.biwenger.com/api/v2/players/la-liga/${encodeURIComponent(n)}?lang=es&fields=prices`,{headers:{Accept:"application/json"}});if(!o.ok)throw new Error(String(o.status));return((await o.json())?.data?.prices??[]).map(([a,i])=>{let l=String(a).padStart(6,"0");return{date:`20${l.slice(0,2)}-${l.slice(2,4)}-${l.slice(4,6)}`,price:Number(i)}})}catch{let o=await Mr(n).catch(()=>null);return Array.isArray(o)?o.map(a=>({date:String(a.date??"").slice(0,10),price:Number(a.price)})).filter(a=>a.date&&Number.isFinite(a.price)):null}})(),bwoBalancePriceCache.set(n,r)}let o=await r;if(!o?.length)return null;let a=String(t).slice(0,10),i=o.find(l=>l.date===a);if(i)return{price:i.price,exact:!0};let l=o.filter(s=>s.date<=a).sort((s,u)=>u.date.localeCompare(s.date))[0];return l?{price:l.price,exact:!1}:{price:o[0].price,exact:!1}}
async function bwoBalanceMapLimit(e,t,n){let r=new Array(e.length),o=0;async function a(){for(;o<e.length;){let i=o++;r[i]=await n(e[i],i),o<e.length&&await new Promise(l=>setTimeout(l,120))}}return await Promise.all(Array.from({length:Math.min(t,e.length||1)},a)),r}
function bwoBalanceSquadAt(e,t,n){let r=new Set((e??[]).map(String)),o=!1,a=t.filter(i=>i.occurred_at>n).sort((i,l)=>l.occurred_at.localeCompare(i.occurred_at));for(let i of a){if(i.type==="intercambio"){o=!0;continue}if(!bwoBalanceIncomingTypes.has(i.type)&&!bwoBalanceOutgoingTypes.has(i.type))continue;if(!i.player_ref_id){o=!0;continue}bwoBalanceIncomingTypes.has(i.type)?r.delete(String(i.player_ref_id)):r.add(String(i.player_ref_id))}return{squad:[...r],unresolved:o}}
async function bwoBalanceBuildShadows(e,t,n,r){let o={};for(let l of r){let s=String(l.id),u=typeof l.joinDate==="number"?new Date(l.joinDate*1e3).toISOString():null,c=u!=null&&u>e,f=c?u:e,g=c?n:t,b=Number(bwoBalanceFixedDistributions[g]),p=Number(bwoBalanceTeamValueDistributions[g]);if(Number.isFinite(b)){o[s]={startDate:f,joinedAfterAnchor:c,distribution:g,teamValue:null,squadSize:null,movementsUndone:null,pricesApprox:null,balance:b,unresolved:!1};continue}if(!Number.isFinite(p)){o[s]={startDate:f,joinedAfterAnchor:c,distribution:g,teamValue:null,squadSize:null,movementsUndone:null,pricesApprox:null,balance:null,unresolved:!0};continue}let y;try{y=await qg(l.id)}catch{o[s]={startDate:f,joinedAfterAnchor:c,distribution:g,teamValue:null,squadSize:null,movementsUndone:null,pricesApprox:null,balance:null,unresolved:!0};continue}let w=m=>m.player_id===s,m=(await z()).state.movements.filter(w),d=bwoBalanceSquadAt(y,m,f),v=await bwoBalanceMapLimit(d.squad,4,x=>bwoBalancePriceAt(x,f)),M=0,B=0,E=d.unresolved;for(let x of v)x?(M+=x.price,x.exact||B++):E=!0;o[s]={startDate:f,joinedAfterAnchor:c,distribution:g,teamValue:M,squadSize:d.squad.length,movementsUndone:m.filter(x=>x.occurred_at>f&&(bwoBalanceIncomingTypes.has(x.type)||bwoBalanceOutgoingTypes.has(x.type))).length,pricesApprox:B,balance:p-M,unresolved:E}}let P=await z();P.state.shadowStartingBalances=o,P.state.bwoBalanceShadows=o,P.state.syncState.shadowStartingBalancesAnchor=e,P.state.syncState.bwoBalanceShadowAnchor=e,P.state.syncState.bwoBalanceShadowDistribution=String(t??""),P.state.syncState.bwoBalanceShadowBuiltAt=String(Date.now()),P.state.syncState.bwoBalanceShadowEngine="public-v1720-json-v3",await Cu();return o}

async function bwoBalancePrepareRebuild(){let e=await z(),t=e.state.syncState;if(t.bwoBalanceEngineV1728==="1")return;if(t.bwoBalanceEngineV1726!=="1"){t.bwoBalanceEngineV1726="rebuilding",t.bwoBalanceEngineV1725="rebuilding",t.bwoBalanceEngineV1724="rebuilding",t.bwoBalanceEngineV1723="rebuilding",t.bwoBalanceEngineV1721="rebuilding",e.state.movements=[],e.state.shadowStartingBalances={},e.state.bwoBalanceShadows={},e.state.startingBalances={},e.state.startingBalancesUnresolved={};for(let n of new Set([...bwoBalanceEventTypes,"market","transfer","roundFinished","exchange","loan"]))t[`lastSeenDate:${n}`]="";t.seasonAnchorDate="",t.seasonStartingBalance="",t.seasonDistribution="",t.shadowStartingBalancesAnchor="",t.bwoBalanceShadowAnchor="",t.bwoBalanceShadowSignature="",t.bwoBalanceShadowEngine="",delete e.state.balanceCorrections,delete t.seasonStartingBalanceLockedV3}else{let n=new Set(["subida_clausula","bajada_clausula","abono"]);t.bwoBalanceEngineV1727!=="1"&&n.add("jornada"),e.state.movements=e.state.movements.filter(r=>!n.has(r.type)),t["lastSeenDate:clauseIncrement"]="",t["lastSeenDate:bonus"]="",t.bwoBalanceEngineV1727!=="1"&&(t["lastSeenDate:roundFinished"]="")}e.movementIds.clear(),e.movementFingerprints.clear();for(let n of e.state.movements)e.movementIds.add(n.id),e.movementFingerprints.add(Nu(n));t.movementsHistoryCompleteV1="",t.bwoBalanceEngineV1727="rebuilding",t.bwoBalanceEngineV1728="rebuilding",await Cu()}

var bwoOriginalZw=zw;
Ri=async function(){let{state:e}=await z(),{date:t,startingBalance:n}=await qo(),r=String(e.syncState.myManagerId??""),o=e.syncState.myBalance,a=o!=null&&o!==""?Number(o):null,i=e.shadowStartingBalances??e.bwoBalanceShadows??{},l=new Map;for(let u of Object.values(i))u?.squadSize!=null&&l.set(u.squadSize,(l.get(u.squadSize)??0)+1);let s=null,c=0;for(let[u,f]of l)f>c&&(c=f,s=u);return Object.values(e.players).filter(u=>!e.inactive[u.id]).map(u=>{let f=i[u.id],g=!!f&&f.balance!=null&&!f.unresolved&&!f.pricesApprox&&(f.squadSize==null||s==null||f.squadSize===s),p=e.movements.filter(y=>y.player_id===u.id&&y.occurred_at>t).reduce((y,w)=>y+w.amount,0),m=u.id===r&&a!=null?a:(g?f.balance:n)+p,d=e.teamValue[u.id];return{id:u.id,name:u.name,icon:u.icon,team_value:d?.teamValue??null,squad_size:d?.squadSize??null,team_value_change_today:d?.changeToday??null,balance:Math.round(m),balance_exact:u.id===r&&a!=null||g}}).sort((u,f)=>f.balance-u.balance)};
zw=async function(){await bwoBalancePrepareRebuild();let e=await bwoOriginalZw(),t=await zg(),n=t?.data?.users??[],r=await Fg().catch(()=>({})),o=typeof t?.data?.created==="number"?new Date(t.data.created*1e3).toISOString():"1970-01-01T00:00:00.000Z",a=await bwoBalanceResolveAnchor(o),i=await Li({fresh:!0}),l=await rg(i.id,a.date),s=typeof r?.newSeason==="number"?r.newSeason:null,u=a.distribution??s,c=a.distribution==null&&s!=null,f=i.balance-l,g=bwoBalanceFixedDistributions[u],b=g!=null&&c&&Math.abs(g-f)>3e6,p=g!=null&&!b?g:f,h=b?null:u,y=await z(),w=y.state.syncState;w.seasonAnchorDate=a.date,w.seasonStartingBalance=String(Math.round(p)),w.seasonDistribution=u!=null?String(u):"",w.myManagerId=String(i.id),w.myBalance=String(Math.round(i.balance)),w.derivedStartingBalance=String(Math.round(f)),w.rejectedFlatCash=b?String(g):"",delete y.state.balanceCorrections;let m=y.state.shadowStartingBalances??{},d=`${a.date}|${h??""}|${typeof r?.newUsers==="number"?r.newUsers:""}|${n.map(v=>v.id).join(",")}`,v=w.shadowStartingBalancesAnchor===a.date&&w.bwoBalanceShadowEngine==="public-v1720-json-v3"&&w.bwoBalanceShadowSignature===d&&n.every(M=>m[String(M.id)]);return v||(m=await bwoBalanceBuildShadows(a.date,h,typeof r?.newUsers==="number"?r.newUsers:null,n),w.bwoBalanceShadowSignature=d),y.state.shadowStartingBalances=m,w.shadowStartingBalancesAnchor=a.date,w.bwoBalanceEngineV1721="1",w.bwoBalanceEngineV1722="1",w.bwoBalanceEngineV1723="1",w.bwoBalanceEngineV1724="1",w.bwoBalanceEngineV1725="1",w.bwoBalanceEngineV1726="1",w.bwoBalanceEngineV1727="1",w.bwoBalanceEngineV1728="1",await Cu(),e};
...
var bwoFastSyncPromise=null,bwoFastPaintRunning=!1;
async function bwoPaintCachedData(){if(bwoFastPaintRunning)return;bwoFastPaintRunning=!0;try{let e=bwoReadSessionKey();e&&e!==bwoSessionKey&&(bwoSessionKey=e,Ju=0,Ti=null,Wi=null,bwoCloseBidsPanel()),Av(),Rv(),bwoEnsureBidsNav(),bwoEnsureBidsFabItem(),bwoMaintainToolsPage();let t=location.pathname,n=[];t.includes("/market")?(dv(),wv()):t.includes("/league")&&n.push(gv(),yv()),ov(),n.push(Cv(),_v(),kv(),Tv()),await Promise.allSettled(n),await bwoEnhanceRoundPlanner().catch(e=>console.warn("[biwtools] No se pudo actualizar Jornada:",e)),await bwoEnhanceRecommendations().catch(e=>console.warn("[biwtools] No se pudieron actualizar Recomendaciones:",e))}catch(e){console.warn("[biwtools] No se pudo pintar la caché coordinada:",e)}finally{bwoFastPaintRunning=!1}}
bwoRefreshBalances=async function(){await bwoPaintCachedData();if(!bwoFastSyncPromise){let e=Wg().lastSyncAt;bwoFastSyncPromise=Promise.resolve().then(()=>Vg()).then(()=>{let t=Wg().lastSyncAt;return t!==e?bwoPaintCachedData():void 0}).catch(e=>console.warn("[biwtools] No se pudo completar la sincronización en segundo plano:",e)).finally(()=>{bwoFastSyncPromise=null})}};

/* Gobernador de red: serializa exclusivamente las llamadas de Biwtools a Biwenger,
   respeta el bloqueo 429 y evita que las vistas automáticas descarguen informes por jugador. */
var bwoApiQueue=Promise.resolve(),bwoApiLastAt=0,bwoApiBlockKey="biwtools:api-blocked-until",bwoApiBlockedUntil=Number(localStorage.getItem(bwoApiBlockKey)??0),bwoApiMinGap=700;
function bwoApiBlock(e){bwoApiBlockedUntil=Math.max(bwoApiBlockedUntil,Date.now()+e);try{localStorage.setItem(bwoApiBlockKey,String(bwoApiBlockedUntil))}catch{}}
async function bwoApiFetch(e,t={}){let n=async()=>{let r=Date.now();if(r<bwoApiBlockedUntil)throw new Error(`Biwenger ha limitado temporalmente las consultas de Biwtools. Espera ${Math.max(1,Math.ceil((bwoApiBlockedUntil-r)/6e4))} min sin recargar.`);bwoApiBlockedUntil&&r>=bwoApiBlockedUntil&&(bwoApiBlockedUntil=0,localStorage.removeItem(bwoApiBlockKey));let o=Math.max(0,bwoApiMinGap-(r-bwoApiLastAt));o&&await new Promise(l=>setTimeout(l,o)),bwoApiLastAt=Date.now();let a=await fetch(e,t),i=Number(a.headers.get("retry-after")),s=Number.isFinite(i)&&i>0?i*1e3:5*60*1e3;if(a.status===429)bwoApiBlock(s);else if(!a.ok)try{let l=(await a.clone().text()).toLocaleLowerCase("es");/m[aá]ximo de peticiones|demasiadas peticiones|too many requests/.test(l)&&bwoApiBlock(s)}catch{}return a},r=bwoApiQueue.then(n,n);return bwoApiQueue=r.then(()=>void 0,()=>void 0),r}

var bwoAccountInFlight=null;
Li=async function(e={}){if(Ti&&!e.fresh)return Ti;if(bwoAccountInFlight)return bwoAccountInFlight;return bwoAccountInFlight=(async()=>{let t=xu(),n=ke(),r=await bwoApiFetch("https://biwenger.as.com/api/v2/account",{headers:{Authorization:`Bearer ${t}`,"X-Lang":"es"}});if(!r.ok)throw new Tr(`No se pudo resolver la cuenta: ${r.status}`);let i=((await r.json())?.data?.leagues??[]).find(s=>String(s.id)===String(n)),l=i?.user?.id;if(l==null)throw new Tr("No se encontró tu id de miembro para la liga activa en /account.");return Ti={id:String(l),balance:Number(i?.user?.balance??0),favorites:(i?.user?.favorites??[]).map(String)},Ti})().finally(()=>{bwoAccountInFlight=null})};
Fg=async function(){let e=await bwoApiFetch(`${qi}/league?fields=*,settings`,{headers:await Hi()});if(!e.ok)throw new Error(`getLeagueSettings failed: ${e.status} ${await e.text()}`);return(await e.json())?.data?.settings??{}};
qg=async function(e){let t=await bwoApiFetch(`${qi}/user/${e}?fields=id,name,players`,{headers:await Hi()});if(!t.ok)throw new Error(`getUserSquad failed: ${t.status} ${await t.text()}`);return((await t.json())?.data?.players??[]).map(o=>o.id)};
Ui=async function(e={}){let t=new URLSearchParams;e.type&&t.set("type",e.type),t.set("offset",String(e.offset??0)),t.set("limit",String(e.limit??50));let n=ke(),r=await bwoApiFetch(`${qi}/league/${n}/board?${t}`,{headers:await Hi()});if(!r.ok)throw new Error(`getBoard failed: ${r.status} ${await r.text()}`);return r.json()};
bwoFetchLeagueEndpoint=async function(e,t){let n=await bwoApiFetch(e,{headers:t});if(!n.ok)throw new Error(`getLeague failed: ${n.status} ${await n.text()}`);return n.json()};

var bwoLiveMarketInFlight=null;
bwoFetchLiveMarket=async function(e=!1){if(bwoLiveMarketCache&&Date.now()-bwoLiveMarketAt<3e4)return bwoLiveMarketCache;if(bwoLiveMarketInFlight)return bwoLiveMarketInFlight;return bwoLiveMarketInFlight=(async()=>{let t=await bwoApiFetch(`${qi}/market`,{headers:await Hi(),cache:"no-store"});if(!t.ok)throw new Error(`getMarket failed: ${t.status} ${await t.text()}`);return bwoLiveMarketCache=await t.json(),bwoLiveMarketAt=Date.now(),bwoLiveMarketCache})().finally(()=>{bwoLiveMarketInFlight=null})};
bwoClauseUserSquad=async function(e,t){try{let n=await bwoApiFetch(`${qi}/user/${e.id}?fields=*,players`,{headers:t});if(!n.ok)throw new Error(String(n.status));let r=(await n.json())?.data??{};return{user:{...e,...r},players:Array.isArray(r.players)?r.players:[]}}catch(n){return{user:e,players:Array.isArray(e.players)?e.players:[],error:n}}};
bwoClauseFetchOfficial=async function(){let e=await bwoApiFetch(`${qi}/owners/league/clause`,{headers:await Hi()});if(!e.ok)throw new Error(`getLeagueClauses failed: ${e.status}`);let t=await e.json(),n=t?.data??t;return Array.isArray(n)?n:Array.isArray(n?.players)?n.players:Array.isArray(n?.items)?n.items:[]};

var bwoApiMemoValues=new Map,bwoApiMemoPromises=new Map;
async function bwoApiMemo(e,t,n){let r=bwoApiMemoValues.get(e);if(r&&Date.now()-r.at<t)return r.value;if(bwoApiMemoPromises.has(e))return bwoApiMemoPromises.get(e);let o=Promise.resolve().then(n).then(a=>(bwoApiMemoValues.set(e,{at:Date.now(),value:a}),a)).finally(()=>bwoApiMemoPromises.delete(e));return bwoApiMemoPromises.set(e,o),o}
function bwoApiLeagueKey(e){let t;try{t=ke()}catch{t="none"}return`${e}:${t}`}
var bwoZgRateSafe=zg,bwoFgRateSafe=Fg,bwoQgRateSafe=qg,bwoUiRateSafe=Ui,bwoClauseUserRateSafe=bwoClauseUserSquad,bwoClauseOfficialRateSafe=bwoClauseFetchOfficial;
zg=async function(){return bwoApiMemo(bwoApiLeagueKey("league"),3e4,()=>bwoZgRateSafe())};
Fg=async function(){return bwoApiMemo(bwoApiLeagueKey("settings"),6e4,()=>bwoFgRateSafe())};
qg=async function(e){return bwoApiMemo(bwoApiLeagueKey(`squad:${e}`),12e4,()=>bwoQgRateSafe(e))};
Ui=async function(e={}){let t=`${e.type??"all"}:${e.offset??0}:${e.limit??50}`;return bwoApiMemo(bwoApiLeagueKey(`board:${t}`),6e4,()=>bwoUiRateSafe(e))};
bwoClauseUserSquad=async function(e,t){return bwoApiMemo(bwoApiLeagueKey(`clause-user:${e.id}`),12e4,()=>bwoClauseUserRateSafe(e,t))};
bwoClauseFetchOfficial=async function(){return bwoApiMemo(bwoApiLeagueKey("clauses"),3e4,()=>bwoClauseOfficialRateSafe())};
var bwoApiMemoSession=null,bwoRefreshRateSafeBase=bwoRefreshBalances;
bwoRefreshBalances=async function(){let e=bwoReadSessionKey();if(e&&e!==bwoApiMemoSession){bwoApiMemoSession=e,bwoApiMemoValues.clear(),bwoApiMemoPromises.clear(),bwoLiveMarketCache=null,bwoRecommendationUpgradeCache=null,bwoClauseCache=null}return bwoRefreshRateSafeBase()};
var bwoVgRateSafe=Vg;
Vg=async function(e=15*60*1e3){return bwoVgRateSafe(e===0?12e4:Math.max(12e4,e))};

/* Las pantallas automáticas usan el agregado del catálogo. El informe individual queda
   reservado para cuando el usuario abre expresamente la ficha de un jugador. */
$t=function(){return!1};
ft=async function(e){let t=String(e);if(Pi.has(t))return Pi.get(t);try{let n=await bwoApiFetch(`https://cf.biwenger.com/api/v2/players/la-liga/${t}?lang=es&fields=reports`,{headers:{Accept:"application/json"}});if(!n.ok)throw new Error(`${n.status}`);let r=((await n.json())?.data?.reports??[]).filter(o=>o?.match?.competition?.slug==="la-liga"&&o?.points).map(o=>({date:new Date((o.match?.date??0)*1e3).toISOString(),roundShort:o.match?.round?.short??"",opponentName:(o.home?o.match?.away?.name:o.match?.home?.name)??"?",home:!!o.home,points:o.points,goals:Number(o.rawStats?.goals??0),assists:Number(o.rawStats?.assists??0),yellowCards:Number(o.rawStats?.yellowCard??0),redCards:Number(o.rawStats?.redCard??0),rawStats:o.rawStats??{}}));return Pi.set(t,r),r}catch(n){return console.warn("[biwtools] Informe individual no disponible:",n),Pi.set(t,[]),[]}};
bwoClauseNativePlayerMeta=async function(e){let t=String(e?.id??e?.catalog?.id??""),n=e?.catalog?.competition?.slug??e?.catalog?.competition_slug??e?.catalog?.competitionSlug??"la-liga";n==="primera-division"&&(n="la-liga");let r=e?.catalog?.slug??bwoClauseSlug(e?.name),o=e?.catalog?.url??e?.url;if(typeof o==="string"&&o.includes("/players/")){let a=o.startsWith("http")?new URL(o).pathname:o,i=a.match(/^\/([^/]+)\/players\/([^/?#]+)/);if(i)return{competition:i[1],slug:i[2],path:a}}if(!t)return{competition:n,slug:r,path:`/${n}/players/${r}`};if(bwoClauseNativeMetaCache.has(t))return bwoClauseNativeMetaCache.get(t);let a=(async()=>{try{let i=await bwoApiFetch(`https://cf.biwenger.com/api/v2/players/${encodeURIComponent(n)}/${encodeURIComponent(t)}?lang=es`,{headers:{Accept:"application/json"}});if(!i.ok)throw new Error(`${i.status}`);let l=(await i.json())?.data??{},s=String(l.slug??"").trim();return s&&(r=s),{competition:n,slug:r,path:`/${n}/players/${r}`}}catch(i){return console.warn(`[biwtools] No se pudo obtener el slug oficial de ${e?.name??t}:`,i),{competition:n,slug:r,path:`/${n}/players/${r}`}}})();return bwoClauseNativeMetaCache.set(t,a),a};
Rw=async function(e){let t=await _r(String(e));return t?{price:Number(t.price??0),position:Number(t.position??0),price_increment:Number(t.price_increment??0)}:{price:0,position:0,price_increment:0}};
bwoBalancePriceAt=async function(e,t){let n=String(e),r=await Mr(n).catch(()=>null);if(!Array.isArray(r)||!r.length)return null;let o=r.map(a=>({date:String(a.date??"").slice(0,10),price:Number(a.price)})).filter(a=>a.date&&Number.isFinite(a.price)),i=String(t).slice(0,10),s=o.find(a=>a.date===i);if(s)return{price:s.price,exact:!0};let u=o.filter(a=>a.date<=i).sort((a,l)=>l.date.localeCompare(a.date))[0];return u?{price:u.price,exact:!1}:o[0]?{price:o[0].price,exact:!1}:null};

/* Deduplicación de los JSON compartidos cuando varias mejoras visuales arrancan a la vez. */
var bwoHBase=H,bwoHInFlight=null,bwoMiBase=Mi,bwoMiInFlight=null,bwoMrBase=Mr,bwoMrInFlight=null,bwoXfBase=xf,bwoXfInFlight=null,bwoSfBase=Sf,bwoSfInFlight=null;
H=async function(){if(Cr&&Date.now()-ff<9e5)return Cr;return bwoHInFlight||(bwoHInFlight=Promise.resolve(bwoHBase()).finally(()=>{bwoHInFlight=null}))};
Mi=async function(){if(Ro&&Date.now()-yf<9e5)return Ro;return bwoMiInFlight||(bwoMiInFlight=Promise.resolve(bwoMiBase()).finally(()=>{bwoMiInFlight=null}))};
Mr=async function(e){if(Ao&&Date.now()-vf<9e5)return Ao.get(String(e));bwoMrInFlight||(bwoMrInFlight=Promise.resolve(bwoMrBase(e)).finally(()=>{bwoMrInFlight=null}));await bwoMrInFlight;return Ao?.get(String(e))};
xf=async function(){if(jo&&Date.now()-hf<9e5)return jo;return bwoXfInFlight||(bwoXfInFlight=Promise.resolve(bwoXfBase()).finally(()=>{bwoXfInFlight=null}))};
Sf=async function(){if($o&&Date.now()-bf<9e5)return $o;return bwoSfInFlight||(bwoSfInFlight=Promise.resolve(bwoSfBase()).finally(()=>{bwoSfInFlight=null}))};

/* BIWTOOLS PRO SUITE v1.9.0: módulos lógicos en un único bundle. */
var bwoSuiteVersion="1.9.0",bwoSuiteMetrics={startedAt:Date.now(),requests:0,errors:0,rateLimits:0,totalMs:0,cacheHits:0,cacheMisses:0};
function bwoSuiteLeagueId(){try{return String(ke?.()??"default")}catch{return"default"}}
function bwoSuiteMoney(e){return Number.isFinite(Number(e))?Number(e).toLocaleString("es-ES")+" €":"—"}
function bwoSuiteEscape(e){return String(e??"").replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}
function bwoSuiteStorageKey(e){return`biwtools:pro:${e}:${bwoSuiteLeagueId()}`}
function bwoSuiteRead(e,t=[]){try{let n=localStorage.getItem(bwoSuiteStorageKey(e));return n?JSON.parse(n):t}catch{return t}}
function bwoSuiteWrite(e,t){try{return localStorage.setItem(bwoSuiteStorageKey(e),JSON.stringify(t)),!0}catch{return!1}}
function bwoSuiteConfidence(e){let t=e?.sampleConfidence==="alta"?90:e?.sampleConfidence==="media"?65:e?.sampleConfidence==="baja"?35:Number(e?.sampleConfidence)||0,n=Math.max(0,Math.min(100,Math.round(.46*t+.34*(Number(e?.probabilityConsensus)||0)+20*(e?.sampleReliable?1:.35))));return{value:n,label:n>=80?"Alta":n>=58?"Media":"Baja",tone:n>=80?"good":n>=58?"warn":"bad"}}
function bwoSuiteEnsureStyles(){if(!zv||zv.querySelector("#bwo-suite-styles"))return;let e=document.createElement("style");e.id="bwo-suite-styles",e.textContent=`.bwo-suite-page{padding:18px;min-height:620px;color:#f7f7fa;background:#101217;font-family:inherit}.bwo-suite-head{display:flex;gap:16px;align-items:center;justify-content:space-between;padding:22px;border:1px solid #343740;border-radius:17px;background:linear-gradient(115deg,#1c222c,#280d14)}.bwo-suite-head h2{font-size:27px;margin:3px 0}.bwo-suite-head p{margin:0;color:#b8c2d2;max-width:850px}.bwo-suite-tabs{display:flex;gap:8px;flex-wrap:wrap;margin:16px 0}.bwo-suite-tabs button,.bwo-suite-btn{appearance:none;border:1px solid #3c424e;border-radius:10px;background:#20242c;color:#fff;padding:10px 14px;font-weight:750;cursor:pointer}.bwo-suite-tabs button.active,.bwo-suite-btn.primary{background:#7b101f;border-color:#e22642}.bwo-suite-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(270px,1fr));gap:12px}.bwo-suite-card{border:1px solid #303540;border-radius:14px;background:#181c23;padding:15px;min-width:0}.bwo-suite-card h3{font-size:16px;margin:0 0 8px}.bwo-suite-kpis{display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:10px;margin:14px 0}.bwo-suite-kpi{border:1px solid #303540;border-radius:12px;padding:12px;background:#15181e}.bwo-suite-kpi small{display:block;color:#9aa6b7;font-size:10px;text-transform:uppercase;font-weight:800;letter-spacing:.06em}.bwo-suite-kpi strong{display:block;font-size:20px;margin-top:5px}.bwo-suite-row{display:grid;grid-template-columns:minmax(160px,1.4fr) repeat(3,minmax(90px,.7fr)) auto;gap:12px;align-items:center;padding:12px;border-bottom:1px solid #2b3039}.bwo-suite-row:last-child{border-bottom:0}.bwo-suite-player{display:flex;align-items:center;gap:10px;min-width:0}.bwo-suite-player img{width:42px;height:42px;object-fit:cover;border-radius:50%;background:#292d35}.bwo-suite-player b,.bwo-suite-player small{display:block;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.bwo-suite-player small,.bwo-suite-muted{color:#9da8ba;font-size:11px}.bwo-suite-pill{display:inline-flex;border-radius:999px;padding:5px 9px;font-size:10px;font-weight:850;background:#28303b}.bwo-suite-pill.good{background:#123d2a;color:#70f39d}.bwo-suite-pill.warn{background:#493713;color:#ffd76a}.bwo-suite-pill.bad{background:#491b20;color:#ff8c99}.bwo-suite-alert{border-left:4px solid #e2ad2d;padding:13px 14px;background:#1c1d20;border-radius:9px;margin:8px 0}.bwo-suite-alert.high{border-color:#f02c48}.bwo-suite-alert.good{border-color:#42d77d}.bwo-suite-empty{text-align:center;color:#aab3c2;padding:48px 20px}.bwo-suite-confidence{margin-top:8px}.bwo-suite-confidence>div{height:6px;border-radius:9px;background:#2c313b;overflow:hidden}.bwo-suite-confidence i{display:block;height:100%;background:linear-gradient(90deg,#e33449,#edb62e,#42dc7e)}.bwo-suite-confidence small{display:flex;justify-content:space-between;color:#aab3c2;margin-top:4px}.bwo-suite-diag{margin:12px 20px 20px;border:1px solid #343a45;border-radius:14px;background:#181d25;padding:15px}.bwo-suite-tests{display:grid;grid-template-columns:repeat(auto-fit,minmax(210px,1fr));gap:8px;margin-top:10px}.bwo-suite-test{padding:10px;border-radius:9px;background:#11151b;border:1px solid #303640}.bwo-suite-test.ok{border-color:#245c3a}.bwo-suite-test.fail{border-color:#792838}.bwo-suite-xi-editor{margin:14px 0;border:1px solid #343a45;border-radius:13px;padding:13px;background:#171b22}.bwo-suite-xi-editor select{max-width:280px;background:#222832;color:#fff;border:1px solid #424958;border-radius:8px;padding:9px;margin:4px}.bwo-suite-version{display:flex;flex-wrap:wrap;gap:8px;margin-top:11px}.bwo-suite-version span{font-size:10px;border:1px solid #424753;border-radius:99px;padding:5px 9px;color:#c4ccda}@media(max-width:720px){.bwo-suite-page{padding:10px}.bwo-suite-head{align-items:flex-start;flex-direction:column;padding:16px}.bwo-suite-head h2{font-size:22px}.bwo-suite-row{grid-template-columns:1fr 1fr;gap:8px}.bwo-suite-row .bwo-suite-player{grid-column:1/-1}.bwo-suite-row .bwo-suite-action{grid-column:1/-1}.bwo-suite-btn{min-height:42px;width:100%}}`,zv.append(e)}
var bwoSuiteEnsureShadowStyles=bwoSuiteEnsureStyles;bwoSuiteEnsureStyles=function(){bwoSuiteEnsureShadowStyles();if(!document.getElementById("bwo-suite-document-styles")){let e=zv?.querySelector("#bwo-suite-styles")?.cloneNode(!0);e&&(e.id="bwo-suite-document-styles",document.head.append(e))}};
function bwoSuitePlayerImage(e){return e?.image??e?.photo??e?.img??""}function bwoSuitePlayerName(e){return e?.name??e?.playerName??e?.slug??"Jugador"}function bwoSuitePlayerPosition(e){return e?.position??e?.positionName??e?.pos??"—"}
function bwoSuiteTrackRead(){return bwoSuiteRead("investments",[])}
function bwoSuiteTrackPlayer(e,t){let n=bwoSuiteTrackRead(),o=String(e?.id??e?.playerId??bwoSuitePlayerName(e)),i=n.findIndex(e=>String(e.id)===o),a={id:o,name:bwoSuitePlayerName(e),position:bwoSuitePlayerPosition(e),image:bwoSuitePlayerImage(e),entry:Number(t??e?.price??e?.value??0),target:Math.round(Number(t??e?.price??e?.value??0)*1.12),addedAt:Date.now()};i>=0?n[i]={...n[i],...a}:n.unshift(a),bwoSuiteWrite("investments",n.slice(0,60))}
function bwoSuiteUntrack(e){bwoSuiteWrite("investments",bwoSuiteTrackRead().filter(t=>String(t.id)!==String(e)))}
async function bwoSuiteMarketData(e=!1){let t=await bwoRecBuildData(e),n=Math.max(1,...t.candidates.map(e=>Number(e?.games??e?.played??e?.appearances??0)||0)),o=t.candidates.map(e=>({...e,_suiteScore:bwoRecScore(e,t.jp?.get?.(String(e.id)),n)}));return{...t,scored:o}}
function bwoSuiteBuildAlerts(e){let t=[],n=e.scored??[],r=Number(e.maxBid)||0;for(let e of n){let n=e._suiteScore??{},o=Number(e.price??e.value??0),i=Number(e.priceIncrement??e.increment??e.trend??0);n.appearanceRate<.45&&t.push({level:"high",title:`${bwoSuitePlayerName(e)}: poca participación`,text:`Solo disputa aproximadamente el ${Math.round(100*n.appearanceRate)}% de las jornadas analizadas.`}),i>0&&o>0&&o<=r&&t.push({level:"good",title:`${bwoSuitePlayerName(e)} sube de valor`,text:`Tendencia +${bwoSuiteMoney(i)} y entra en tu capacidad máxima de puja.`})}for(let n of e.own??[]){let e=String(n?.status??n?.fitness??"").toLowerCase(),r=Number(n?.priceIncrement??n?.increment??0);(/injur|lesion|doubt|sanc|baja/.test(e)||r<0)&&t.push({level:"high",title:`Revisar a ${bwoSuitePlayerName(n)}`,text:/injur|lesion|doubt|sanc|baja/.test(e)?"Presenta una incidencia deportiva para la próxima jornada.":`Pierde ${bwoSuiteMoney(Math.abs(r))} de valor; conviene revisar su venta.`})}return t.slice(0,18)}
function bwoSuiteCurrentPrice(e,t){let n=(e??[]).find(e=>String(e.id)===String(t.id));return Number(n?.price??n?.value??t.entry??0)}
function bwoSuiteRenderHeader(e,t){return`<div class="bwo-suite-head"><div><small style="color:#ff4057;font-weight:900;letter-spacing:.08em">BIWTOOLS · SUITE PROFESIONAL</small><h2>${bwoSuiteEscape(e)}</h2><p>${bwoSuiteEscape(t)}</p><div class="bwo-suite-version"><span>v${bwoSuiteVersion}</span><span>Un solo bundle</span><span>Datos de liga activos</span></div></div><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="bwo-suite-btn" data-suite-back>← Herramientas</button><button class="bwo-suite-btn primary" data-suite-refresh>Actualizar análisis</button></div></div>`}
async function bwoSuiteOpenMarketCenter(e="market"){bwoSuiteEnsureStyles();let t=document.querySelector(".bwo-tools-page-host")??document.createElement("section");t.className="bwo-tools-page-host bwo-suite-page",t.replaceChildren();let n=document.createElement("div");n.innerHTML=bwoSuiteRenderHeader("Centro de mercado","Oportunidades, inversiones seguidas y avisos accionables reunidos en una vista rápida.")+`<div class="bwo-suite-tabs"><button data-suite-tab="market">Mercado</button><button data-suite-tab="portfolio">Inversiones</button><button data-suite-tab="alerts">Alertas</button></div><div data-suite-content><div class="bwo-suite-empty">Analizando los datos actuales…</div></div>`,t.append(n),t.isConnected||zv.append(t);let o=null,i=async(a=!1)=>{try{o=await bwoSuiteMarketData(a);let i=n.querySelector("[data-suite-content]");e==="portfolio"?bwoSuiteRenderPortfolio(i,o):e==="alerts"?bwoSuiteRenderAlerts(i,o):bwoSuiteRenderMarket(i,o),n.querySelectorAll("[data-suite-tab]").forEach(t=>t.classList.toggle("active",t.dataset.suiteTab===e))}catch(e){n.querySelector("[data-suite-content]").innerHTML=`<div class="bwo-suite-empty">No se pudieron cargar los datos: ${bwoSuiteEscape(e?.message??e)}</div>`}};n.addEventListener("click",t=>{let a=t.target.closest("[data-suite-tab]");a&&(e=a.dataset.suiteTab,i(!1));let r=t.target.closest("[data-suite-track]");if(r&&o){let e=o.scored.find(e=>String(e.id)===r.dataset.suiteTrack);e&&(bwoSuiteTrackPlayer(e,Number(e.price??e.value??0)),bwoSuiteRenderPortfolio(n.querySelector("[data-suite-content]"),o))}let s=t.target.closest("[data-suite-remove]");s&&(bwoSuiteUntrack(s.dataset.suiteRemove),bwoSuiteRenderPortfolio(n.querySelector("[data-suite-content]"),o));let l=t.target.closest("[data-suite-bid]");if(l&&o){let e=o.scored.find(e=>String(e.id)===l.dataset.suiteBid);e&&bwoRecommendationOpenNativeBid(e,Number(l.dataset.amount)||Number(e.price??e.value??0))}t.target.closest("[data-suite-back]")&&bwoClauseReturnTools(),t.target.closest("[data-suite-refresh]")&&i(!0)}),await i(!1)}
function bwoSuiteRenderMarket(e,t){let n=[...t.scored].sort((e,t)=>(t._suiteScore?.rating??0)-(e._suiteScore?.rating??0)).slice(0,15);e.innerHTML=`<div class="bwo-suite-kpis"><div class="bwo-suite-kpi"><small>Jugadores analizados</small><strong>${t.scored.length}</strong></div><div class="bwo-suite-kpi"><small>Puja máxima</small><strong>${bwoSuiteMoney(t.maxBid)}</strong></div><div class="bwo-suite-kpi"><small>Saldo</small><strong>${bwoSuiteMoney(t.balance)}</strong></div><div class="bwo-suite-kpi"><small>En seguimiento</small><strong>${bwoSuiteTrackRead().length}</strong></div></div><div class="bwo-suite-card">${n.map(e=>{let n=e._suiteScore??{},o=bwoSuiteConfidence(n),i=Number(e.price??e.value??0),a=Math.round(i*(1+Math.max(.01,Math.min(.18,(Number(n.rating)||0)/700)))),r=i<=Number(t.maxBid);return`<div class="bwo-suite-row"><div class="bwo-suite-player">${bwoSuitePlayerImage(e)?`<img src="${bwoSuiteEscape(bwoSuitePlayerImage(e))}">`:""}<div><b>${bwoSuiteEscape(bwoSuitePlayerName(e))}</b><small>${bwoSuiteEscape(bwoSuitePlayerPosition(e))} · fiabilidad ${o.value}%</small></div></div><div><small class="bwo-suite-muted">Precio</small><b>${bwoSuiteMoney(i)}</b></div><div><small class="bwo-suite-muted">Puja sugerida</small><b>${bwoSuiteMoney(a)}</b></div><div><span class="bwo-suite-pill ${r?"good":"bad"}">${r?"Asequible":"Fuera de presupuesto"}</span></div><div class="bwo-suite-action"><button class="bwo-suite-btn" data-suite-track="${bwoSuiteEscape(e.id)}">Seguir</button> <button class="bwo-suite-btn primary" data-suite-bid="${bwoSuiteEscape(e.id)}" data-amount="${a}">Pujar</button></div></div>`}).join("")||'<div class="bwo-suite-empty">No hay jugadores de mercado disponibles.</div>'}</div>`}
function bwoSuiteRenderPortfolio(e,t){let n=bwoSuiteTrackRead();e.innerHTML=`<div class="bwo-suite-kpis"><div class="bwo-suite-kpi"><small>Inversiones seguidas</small><strong>${n.length}</strong></div><div class="bwo-suite-kpi"><small>Capital registrado</small><strong>${bwoSuiteMoney(n.reduce((e,t)=>e+Number(t.entry||0),0))}</strong></div><div class="bwo-suite-kpi"><small>Valor actual</small><strong>${bwoSuiteMoney(n.reduce((e,n)=>e+bwoSuiteCurrentPrice(t.scored,n),0))}</strong></div></div><div class="bwo-suite-grid">${n.map(n=>{let o=bwoSuiteCurrentPrice(t.scored,n),i=o-Number(n.entry||0),a=Number(n.entry||0)?Math.round(100*i/Number(n.entry)):0;return`<article class="bwo-suite-card"><div class="bwo-suite-player">${n.image?`<img src="${bwoSuiteEscape(n.image)}">`:""}<div><h3>${bwoSuiteEscape(n.name)}</h3><small>${bwoSuiteEscape(n.position)}</small></div></div><p>Compra: <b>${bwoSuiteMoney(n.entry)}</b><br>Valor actual: <b>${bwoSuiteMoney(o)}</b><br>Resultado: <b style="color:${i>=0?"#5eed8e":"#ff6576"}">${i>=0?"+":""}${bwoSuiteMoney(i)} (${a}%)</b></p><button class="bwo-suite-btn" data-suite-remove="${bwoSuiteEscape(n.id)}">Dejar de seguir</button></article>`}).join("")||'<div class="bwo-suite-empty">Todavía no sigues ninguna inversión. Añade jugadores desde Mercado.</div>'}</div>`}
function bwoSuiteRenderAlerts(e,t){let n=bwoSuiteBuildAlerts(t);e.innerHTML=`<div class="bwo-suite-kpis"><div class="bwo-suite-kpi"><small>Avisos activos</small><strong>${n.length}</strong></div><div class="bwo-suite-kpi"><small>Prioridad alta</small><strong>${n.filter(e=>e.level==="high").length}</strong></div><div class="bwo-suite-kpi"><small>Oportunidades</small><strong>${n.filter(e=>e.level==="good").length}</strong></div></div><div class="bwo-suite-card">${n.map(e=>`<div class="bwo-suite-alert ${e.level}"><b>${bwoSuiteEscape(e.title)}</b><div class="bwo-suite-muted">${bwoSuiteEscape(e.text)}</div></div>`).join("")||'<div class="bwo-suite-empty">No hay avisos críticos con los datos actuales.</div>'}</div>`}
try{bwoToolsCatalog&&!bwoToolsCatalog.some(e=>e.name==="Centro de mercado")&&bwoToolsCatalog.splice(Math.max(1,bwoToolsCatalog.length-2),0,{icon:"🧭",name:"Centro de mercado",category:"Mercado",status:"Operativo",description:"Mercado, inversiones, pujas sugeridas y oportunidades en una vista profesional.",features:["Puja sugerida","Seguimiento de inversiones","Capacidad económica"],source:"Biwenger · análisis local",tone:"green",featured:!0},{icon:"🔔",name:"Alertas",category:"Control",status:"Operativo",description:"Avisos inteligentes sobre forma, valor, participación, lesiones y oportunidades.",features:["Riesgos de jornada","Cambios de valor","Oportunidades asequibles"],source:"Biwenger · motor Biwtools",tone:"amber",featured:!1})}catch{}
try{var bwoSuiteOpenToolBase=bwoOpenToolFromPage;bwoOpenToolFromPage=function(e){return e==="Centro de mercado"?bwoSuiteOpenMarketCenter("market"):e==="Alertas"?bwoSuiteOpenMarketCenter("alerts"):bwoSuiteOpenToolBase(e)}}catch{}
/* Telemetría ligera: no añade consultas; mide las que ya realiza el bundle. */
try{var bwoSuiteApiFetchBase=bwoApiFetch;bwoApiFetch=async function(...e){let t=performance.now();bwoSuiteMetrics.requests++;try{let n=await bwoSuiteApiFetchBase(...e);return n.status===429&&bwoSuiteMetrics.rateLimits++,n}catch(e){throw bwoSuiteMetrics.errors++,e}finally{bwoSuiteMetrics.totalMs+=performance.now()-t}};var bwoSuiteApiMemoBase=bwoApiMemo;bwoApiMemo=async function(e,t,n){bwoApiMemoValues.has(e)||bwoApiMemoPromises.has(e)?bwoSuiteMetrics.cacheHits++:bwoSuiteMetrics.cacheMisses++;return bwoSuiteApiMemoBase(e,t,n)}}catch{}
function bwoSuiteEndpointList(){let e=typeof bwoCanonicalJson==="object"&&bwoCanonicalJson?bwoCanonicalJson:{catalog:"https://biwtools.es/catalog.json",lineups:"https://biwtools.es/lineups.json",fixtures:"https://biwtools.es/fixtures.json",injuries:"https://biwtools.es/injuries.json",priceHistory:"https://biwtools.es/price-history.json"};return Object.entries(e).map(([e,t])=>({name:e==="priceHistory"?"price-history.json":`${e}.json`,url:t}))}
async function bwoSuiteEndpointTest(){let e=[];for(let{name:t,url:n}of bwoSuiteEndpointList())try{let r=await Promise.race([fetch(n,{cache:"no-store"}),new Promise((e,t)=>setTimeout(()=>t(new Error("Tiempo agotado")),6500))]);if(!r.ok)throw new Error(`HTTP ${r.status}`);let o=await r.json(),a=Array.isArray(o)?o.length:Array.isArray(o?.data)?o.data.length:Object.keys(o??{}).length;e.push({name:t,ok:o!=null,message:`JSON válido · ${a} elemento${a===1?"":"s"}`})}catch(n){e.push({name:t,ok:!1,message:n?.message??String(n)})}return e}
function bwoSuiteBundleBase(){let e=[...document.scripts].map(e=>e.src).find(e=>/biwtools\.bundle\.professional\.js/i.test(e));return e?e.split("?")[0].replace(/biwtools\.bundle\.professional\.js$/i,""):"https://cdn.jsdelivr.net/gh/djtork3-art/BWTOOLS@main/"}
async function bwoSuiteSelfTests(e){let t=[{name:"Runtime único",ok:!!globalThis.__BIWTOOLS_ACTIVE_RUNTIME__,message:globalThis.__BIWTOOLS_ACTIVE_RUNTIME__??"No disponible"},{name:"Liga activa",ok:bwoSuiteLeagueId()!=="default",message:`ID ${bwoSuiteLeagueId()}`},{name:"Límite de API",ok:Date.now()>=Number(bwoApiBlockedUntil||0),message:Date.now()>=Number(bwoApiBlockedUntil||0)?"Sin bloqueo activo":"Biwenger ha limitado temporalmente las consultas"},{name:"Almacenamiento",ok:bwoSuiteWrite("health",{at:Date.now()}),message:"Lectura/escritura local"},{name:"Caché de API",ok:!!bwoApiMemoValues&&!!bwoApiMemoPromises,message:`${bwoApiMemoValues?.size??0} respuestas memorizadas`}];return e&&(t.push(...await bwoSuiteEndpointTest())),t}
function bwoSuiteEnhanceDiagnostics(){let e=zv?.querySelector(".bwo-diag-pro");if(!e||e.querySelector(".bwo-suite-diag"))return;bwoSuiteEnsureStyles();let t=document.createElement("section");t.className="bwo-suite-diag",t.innerHTML=`<h3 style="margin:0">Pruebas automáticas y rendimiento</h3><p class="bwo-suite-muted">Comprueba runtime, liga, almacenamiento, caché y archivos de datos sin modificar tu equipo ni tus saldos.</p><div class="bwo-suite-kpis"><div class="bwo-suite-kpi"><small>Peticiones de esta sesión</small><strong>${bwoSuiteMetrics.requests}</strong></div><div class="bwo-suite-kpi"><small>Aciertos de caché</small><strong>${bwoSuiteMetrics.cacheHits}</strong></div><div class="bwo-suite-kpi"><small>Errores</small><strong>${bwoSuiteMetrics.errors}</strong></div><div class="bwo-suite-kpi"><small>Tiempo medio</small><strong>${bwoSuiteMetrics.requests?Math.round(bwoSuiteMetrics.totalMs/bwoSuiteMetrics.requests):0} ms</strong></div></div><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="bwo-suite-btn primary" data-suite-test>Ejecutar pruebas completas</button><button class="bwo-suite-btn" data-suite-cache>Vaciar caché temporal</button></div><div class="bwo-suite-tests" data-suite-tests></div>`;e.append(t);let n=t.querySelector("[data-suite-tests]"),r=async e=>{n.innerHTML='<div class="bwo-suite-muted">Ejecutando comprobaciones…</div>';let t=await bwoSuiteSelfTests(e);n.innerHTML=t.map(e=>`<div class="bwo-suite-test ${e.ok?"ok":"fail"}"><b>${e.ok?"✓":"!"} ${bwoSuiteEscape(e.name)}</b><div class="bwo-suite-muted">${bwoSuiteEscape(e.message)}</div></div>`).join("")};t.querySelector("[data-suite-test]").addEventListener("click",()=>r(!0)),t.querySelector("[data-suite-cache]").addEventListener("click",()=>{bwoApiMemoValues.clear(),bwoApiMemoPromises.clear(),bwoLiveMarketCache=null,bwoRecommendationUpgradeCache=null,bwoClauseCache=null,r(!1)}),r(!1)}
/* Control de versión y estado en el centro de herramientas. */
try{var bwoSuiteBuildToolsBase=bwoBuildToolsPage;bwoBuildToolsPage=function(){let e=bwoSuiteBuildToolsBase(),t=e.querySelector(".bwo-tools-page-kicker");t&&(t.textContent=`Biwtools ${bwoSuiteVersion} · Suite profesional`);let n=e.querySelector(".bwo-tools-page-head>div");if(n){let r=bwoBidsNode("div","bwo-suite-version");r.innerHTML=`<span>Runtime protegido</span><span>${bwoToolsCatalog.length} módulos</span><span>Caché inteligente</span>`,n.append(r)}return e}}catch{}
/* Fiabilidad visible para cada comparación de recomendaciones. */
try{var bwoSuiteRecRenderBase=bwoRecRenderCard;bwoRecRenderCard=function(e,t){bwoSuiteRecRenderBase(e,t);let n=bwoRecRows(e,t),r=zv.querySelector(".bwo-rec-upgrade");if(!r)return;[...r.querySelectorAll("tbody tr")].forEach((e,t)=>{let o=n[t];if(!o||e.querySelector(".bwo-suite-confidence"))return;let a=o.sampleConfidence==="alta"?88:o.sampleConfidence==="media"?66:o.sampleConfidence==="baja"?38:15,i=document.createElement("div");i.className="bwo-suite-confidence",i.innerHTML=`<div><i style="width:${a}%"></i></div><small><span>Confianza del análisis</span><b>${a}% · ${bwoSuiteEscape(o.sampleConfidence)}</b></small>`;(e.querySelector("td:nth-child(3)")??e.lastElementChild)?.append(i)})}}catch{}
/* Exposición estable de módulos para soporte y futuras ampliaciones. */
globalThis.BiwtoolsPro={version:bwoSuiteVersion,metrics:bwoSuiteMetrics,modules:{diagnostics:{run:bwoSuiteSelfTests},market:{open:bwoSuiteOpenMarketCenter},investments:{read:bwoSuiteTrackRead,add:bwoSuiteTrackPlayer,remove:bwoSuiteUntrack},alerts:{build:bwoSuiteBuildAlerts},confidence:{calculate:bwoSuiteConfidence}},clearTemporaryCache(){bwoApiMemoValues.clear(),bwoApiMemoPromises.clear()},status(){return{version:bwoSuiteVersion,runtime:globalThis.__BIWTOOLS_ACTIVE_RUNTIME__,leagueId:bwoSuiteLeagueId(),metrics:{...bwoSuiteMetrics}}}};
/* Editor interactivo del once: permite sustituciones compatibles y conserva el guardado nativo. */
var bwoSuiteXIOverrides=new Map,bwoSuiteXIContext=null;
function bwoSuiteXIRecalculate(e){let t=e.players.reduce((e,t)=>e+(Number(t.projected)||0),0),n=e.players.filter(e=>bwoMatchdayXIRisk(e)).length,r=e.players.filter(e=>bwoMatchdayXIProbability(e)>=60&&!bwoMatchdayXIRisk(e)).length,o=[...e.players].filter(e=>!e.physical?.unavailable).sort((e,t)=>bwoMatchdayXIRoleRank(t,"captain")-bwoMatchdayXIRoleRank(e,"captain"))[0]??e.players[0],a=e.players.filter(e=>e.position==="4").sort((e,t)=>bwoMatchdayXIRoleRank(t,"striker")-bwoMatchdayXIRoleRank(e,"striker"))[0]??o;return{...e,projected:t,risks:n,reliable:r,captain:o,striker:a}}
try{var bwoSuiteXICompleteBase=bwoMatchdayXICompleteSelection;bwoMatchdayXICompleteSelection=function(e,t,n=4){let r=bwoSuiteXICompleteBase(e,t,n),o=new Map((r._allPlayers??[]).map(e=>[String(e.id),e])),a=[...r.players],i=[...r.reserves];for(let[e,t]of bwoSuiteXIOverrides){let n=a.findIndex(t=>String(t.id)===String(e)),l=o.get(String(t));if(n<0||!l||String(l.position)!==String(a[n].position)||l.physical?.unavailable)continue;let s=a[n];a[n]=l,i=i.filter(e=>e&&String(e.id)!==String(l.id));let u=i.findIndex(e=>!e||String(e.position)===String(s.position));u>=0?i[u]=s:i.length<n&&i.push(s)}return bwoSuiteXIRecalculate({...r,players:a,reserves:i.slice(0,n)})}}catch{}
function bwoSuiteXIEditor(e,t){if(!e||e.querySelector(".bwo-suite-xi-editor"))return;let n=t._allPlayers??[],r=new Set(t.players.map(e=>String(e.id))),o=document.createElement("div");o.className="bwo-suite-xi-editor";let a=document.createElement("strong");a.textContent="Ajuste manual del once";let i=document.createElement("p");i.className="bwo-suite-muted",i.textContent="Cambia un titular por otro jugador de su misma posición. La previsión, el capitán, el ariete y el banquillo se recalculan antes de guardar.";let l=document.createElement("select"),s=document.createElement("select"),u=document.createElement("button");u.className="bwo-suite-btn primary",u.textContent="Aplicar cambio";let c=document.createElement("button");c.className="bwo-suite-btn",c.textContent="Restablecer automático";for(let e of t.players){let t=document.createElement("option");t.value=String(e.id),t.textContent=`Sale: ${e.name} (${bwoMatchdayXIPosition(e.position)})`,l.append(t)}let f=()=>{let e=t.players.find(e=>String(e.id)===l.value);s.replaceChildren();for(let t of n.filter(t=>e&&String(t.position)===String(e.position)&&!r.has(String(t.id))&&!t.physical?.unavailable).sort((e,t)=>bwoMatchdayXIPlayerRank(t)-bwoMatchdayXIPlayerRank(e))){let e=document.createElement("option");e.value=String(t.id),e.textContent=`Entra: ${t.name} · ${Number(t.projected||0).toFixed(1)} pts`,s.append(e)}};l.addEventListener("change",f),f(),u.addEventListener("click",()=>{l.value&&s.value&&(bwoSuiteXIOverrides.set(l.value,s.value),bwoSuiteXIContext&&bwoMatchdayXIRender(bwoSuiteXIContext.data,bwoSuiteXIContext.host,t.formation))}),c.addEventListener("click",()=>{bwoSuiteXIOverrides.clear(),bwoSuiteXIContext&&bwoMatchdayXIRender(bwoSuiteXIContext.data,bwoSuiteXIContext.host,t.formation)}),o.append(a,i,l,s,u,c),e.append(o)}
try{var bwoSuiteXIRenderBase=bwoMatchdayXIRender;bwoMatchdayXIRender=function(e,t,n=null){bwoSuiteXIContext={data:e,host:t},bwoSuiteXIRenderBase(e,t,n)}}catch{}
try{var bwoSuiteXIInstallBase=bwoMatchdayXIInstallPitch;bwoMatchdayXIInstallPitch=function(e,t){bwoSuiteXIInstallBase(e,t);let n=e.querySelector(".bwo-xi-plan-wrap")?.closest(".bwo-xi-panel")??e.querySelector(".bwo-xi-content");bwoSuiteXIEditor(n,t)}}catch{}
/* Los diagnósticos y estilos se activan también en vistas React ya montadas. */
try{var bwoSuiteEnsureFabBase=bwoEnsureBidsFabItem;bwoEnsureBidsFabItem=function(){let e=bwoSuiteEnsureFabBase();return queueMicrotask(()=>{bwoSuiteEnsureStyles(),bwoSuiteEnhanceDiagnostics()}),e}}catch{}
/* v1.9 · Arranque seguro, recuperación y normalización del catálogo. */
function bwoSuiteRecordFailure(e,t){let n=bwoSuiteRead("failures",[]);n.unshift({module:e,message:String(t?.message??t),at:Date.now()}),bwoSuiteWrite("failures",n.slice(0,25))}
function bwoSuiteRecovery(e,t){bwoSuiteRecordFailure(e,t);let n=document.querySelector(".bwo-tools-page-host");if(!n)return;let r=bwoBidsNode("section","bwo-suite-page"),o=bwoBidsNode("div","bwo-suite-card");o.innerHTML=`<h2>Herramienta aislada</h2><p>El módulo <b>${bwoSuiteEscape(e)}</b> encontró un problema y se ha detenido sin afectar al resto de Biwtools.</p><pre style="white-space:pre-wrap;color:#ff97a5">${bwoSuiteEscape(t?.message??t)}</pre>`;let a=bwoBidsNode("button","bwo-suite-btn primary","Volver a Herramientas");a.addEventListener("click",bwoClauseReturnTools),o.append(a),r.append(o),n.replaceChildren(r)}
try{var bwoSuiteSafeToolsBuildBase=bwoBuildToolsPage;bwoBuildToolsPage=function(){for(let e of bwoToolsCatalog)e.icon??="🧩",e.category??="General",e.status??="Operativo",e.description??="Módulo de análisis Biwtools",Array.isArray(e.features)||(e.features=[]),e.source??="Biwenger",e.tone??="blue";try{return bwoSuiteSafeToolsBuildBase()}catch(e){bwoSuiteRecordFailure("Centro de herramientas",e);let t=bwoBidsNode("section","bwo-tools-page-host bwo-suite-page");return t.innerHTML=`<div class="bwo-suite-card"><h2>Centro de herramientas recuperado</h2><p>${bwoSuiteEscape(e?.message??e)}</p></div>`,t}}}catch{}
try{var bwoSuiteSafeOpenBase=bwoOpenToolFromPage;bwoOpenToolFromPage=function(e){try{let t=bwoSuiteSafeOpenBase(e);return t&&typeof t.then==="function"?t.catch(t=>bwoSuiteRecovery(e,t)):t}catch(t){return bwoSuiteRecovery(e,t)}}}catch{}

/* Estado temporal, frescura, preferencias, simulación e historial. */
var bwoSuiteSimulation={buys:new Set,sells:new Set},bwoSuiteFreshness=new Map;
function bwoSuiteAlertPrefs(){return{participation:!0,injury:!0,value:!0,opportunity:!0,participationLimit:45,...bwoSuiteRead("alert-prefs",{})}}
function bwoSuiteDecisionRead(){return bwoSuiteRead("decisions",[])}
function bwoSuiteDecisionAdd(e,t={}){let n=bwoSuiteDecisionRead(),r={id:`${Date.now()}-${Math.random().toString(36).slice(2,7)}`,type:e,at:Date.now(),...t};return n.unshift(r),bwoSuiteWrite("decisions",n.slice(0,120)),r.id}
function bwoSuiteDecisionUpdate(e,t){let n=bwoSuiteDecisionRead(),r=n.findIndex(t=>t.id===e);r>=0&&(n[r]={...n[r],...t},bwoSuiteWrite("decisions",n))}
function bwoSuiteAgo(e){let t=Math.max(0,Date.now()-Number(e||0)),n=Math.floor(t/1e3);return n<60?`hace ${n} s`:n<3600?`hace ${Math.floor(n/60)} min`:n<86400?`hace ${Math.floor(n/3600)} h`:`hace ${Math.floor(n/86400)} días`}
function bwoSuiteFreshStrip(e,t="Biwenger"){let n=document.createElement("div");return n.className="bwo-suite-fresh",n.innerHTML=`<span class="bwo-suite-pill good">● Datos actualizados</span><span>Fuente: ${bwoSuiteEscape(t)}</span><span>${bwoSuiteAgo(e)}</span>`,n}
function bwoSuiteExplainScore(e){let t=e?._suiteScore??e??{},n=[{label:"Media fiable",value:Math.max(0,Math.min(100,Math.round((Number(t.reliableRoundAverage)||0)*12)))},{label:"Participación",value:Math.round(100*(Number(t.appearanceRate)||0))},{label:"Titularidad",value:Math.round(Number(t.probabilityConsensus)||0)},{label:"Regularidad",value:Math.max(0,Math.min(100,Math.round((Number(t.consistency)||0)*10)))},{label:"Muestra",value:t.sampleReliable?100:t.sampleConfidence==="media"?65:35}];return n}

/* Alertas configurables: se filtran sin añadir peticiones. */
var bwoSuiteAlertBuilderBase=bwoSuiteBuildAlerts;bwoSuiteBuildAlerts=function(e){let t=bwoSuiteAlertPrefs(),n=bwoSuiteAlertBuilderBase(e).filter(e=>!/poca participación/i.test(e.title));if(t.participation)for(let r of e.scored??[]){let e=r._suiteScore??{},o=Math.round(100*(Number(e.appearanceRate)||0));o<Number(t.participationLimit||45)&&n.push({level:"high",title:`${bwoSuitePlayerName(r)}: poca participación`,text:`Participación ${o}% · límite configurado ${t.participationLimit}%.`})}return n.filter(e=>e.level==="good"?t.opportunity:/participaci/i.test(e.title)?t.participation:/incidencia|lesion|sanc|baja/i.test(e.text)?t.injury:t.value).slice(0,24)};
function bwoSuiteRenderAlertSettings(e){let t=bwoSuiteAlertPrefs();e.innerHTML=`<div class="bwo-suite-card"><h3>Configurar alertas</h3><p class="bwo-suite-muted">Los avisos se calculan localmente y no generan consultas adicionales.</p>${[["participation","Baja participación"],["injury","Lesiones, sanciones y dudas"],["value","Caídas de valor"],["opportunity","Oportunidades del mercado"]].map(([e,n])=>`<label class="bwo-suite-setting"><input type="checkbox" data-alert-pref="${e}" ${t[e]?"checked":""}> ${n}</label>`).join("")}<label class="bwo-suite-setting">Avisar por debajo de <input type="number" min="5" max="95" value="${t.participationLimit}" data-alert-limit> % de participación</label><button class="bwo-suite-btn primary" data-alert-save>Guardar preferencias</button></div>`}
function bwoSuiteRenderHistory(e){let t=bwoSuiteDecisionRead();e.innerHTML=`<div class="bwo-suite-kpis"><div class="bwo-suite-kpi"><small>Decisiones registradas</small><strong>${t.length}</strong></div><div class="bwo-suite-kpi"><small>Pujas abiertas</small><strong>${t.filter(e=>e.type==="bid").length}</strong></div><div class="bwo-suite-kpi"><small>Onces aplicados</small><strong>${t.filter(e=>e.type==="lineup").length}</strong></div></div><div class="bwo-suite-card">${t.map(e=>`<div class="bwo-suite-alert ${e.ok===!1?"high":"good"}"><b>${e.type==="bid"?"Puja consultada":e.type==="lineup"?"Once aplicado":"Decisión"}${e.name?` · ${bwoSuiteEscape(e.name)}`:""}</b><div class="bwo-suite-muted">${bwoSuiteEscape(e.detail??"")} · ${bwoSuiteAgo(e.at)}</div></div>`).join("")||'<div class="bwo-suite-empty">El historial comenzará al abrir una puja o aplicar un once.</div>'}</div>`}
function bwoSuiteRenderSimulator(e,t){let n=t.scored.filter(e=>bwoSuiteSimulation.buys.has(String(e.id))),r=t.own.filter(e=>bwoSuiteSimulation.sells.has(String(e.id))),o=n.reduce((e,t)=>e+Number(t.price??t.value??0),0),a=r.reduce((e,t)=>e+Number(t.price??t.value??0),0),i=Number(t.balance)+a-o,l=n.reduce((e,t)=>e+(Number(t._suiteScore?.projected)||0),0),s=r.reduce((e,t)=>e+(Number(t.projected??t.recentAverage??0)||0),0);e.innerHTML=`<div class="bwo-suite-kpis"><div class="bwo-suite-kpi"><small>Compras</small><strong>${bwoSuiteMoney(o)}</strong></div><div class="bwo-suite-kpi"><small>Ventas</small><strong>${bwoSuiteMoney(a)}</strong></div><div class="bwo-suite-kpi"><small>Saldo resultante</small><strong style="color:${i>=0?"#65ee91":"#ff7181"}">${bwoSuiteMoney(i)}</strong></div><div class="bwo-suite-kpi"><small>Impacto previsto</small><strong>${l-s>=0?"+":""}${(l-s).toFixed(1)} pts</strong></div></div><div class="bwo-suite-grid"><div class="bwo-suite-card"><h3>Fichajes simulados</h3>${t.scored.slice(0,20).map(e=>`<label class="bwo-suite-setting"><input type="checkbox" data-sim-buy="${bwoSuiteEscape(e.id)}" ${bwoSuiteSimulation.buys.has(String(e.id))?"checked":""}> ${bwoSuiteEscape(bwoSuitePlayerName(e))} · ${bwoSuiteMoney(e.price??e.value)}</label>`).join("")}</div><div class="bwo-suite-card"><h3>Ventas simuladas</h3>${t.own.map(e=>`<label class="bwo-suite-setting"><input type="checkbox" data-sim-sell="${bwoSuiteEscape(e.id)}" ${bwoSuiteSimulation.sells.has(String(e.id))?"checked":""}> ${bwoSuiteEscape(bwoSuitePlayerName(e))} · ${bwoSuiteMoney(e.price??e.value)}</label>`).join("")}</div></div>`}
function bwoSuiteExport(){let e={version:bwoSuiteVersion,exportedAt:new Date().toISOString(),leagueId:bwoSuiteLeagueId(),investments:bwoSuiteTrackRead(),alerts:bwoSuiteAlertPrefs(),decisions:bwoSuiteDecisionRead()},t=new Blob([JSON.stringify(e,null,2)],{type:"application/json"}),n=URL.createObjectURL(t),r=document.createElement("a");r.href=n,r.download=`biwtools-${bwoSuiteLeagueId()}-${Date.now()}.json`,r.click(),setTimeout(()=>URL.revokeObjectURL(n),1e3)}
function bwoSuiteImport(e){let t=new FileReader;t.onload=()=>{try{let n=JSON.parse(String(t.result));Array.isArray(n.investments)&&bwoSuiteWrite("investments",n.investments),n.alerts&&bwoSuiteWrite("alert-prefs",n.alerts),Array.isArray(n.decisions)&&bwoSuiteWrite("decisions",n.decisions),bwoRecommendationToast("Configuración restaurada correctamente.","ok")}catch{bwoRecommendationToast("El archivo de copia no es válido.")}},t.readAsText(e)}
function bwoSuiteRenderSettings(e){e.innerHTML=`<div class="bwo-suite-grid"><div class="bwo-suite-card"><h3>Copia de seguridad</h3><p class="bwo-suite-muted">Exporta inversiones, preferencias de alertas e historial. No incluye tu token ni datos de sesión.</p><button class="bwo-suite-btn primary" data-suite-export>Exportar JSON</button><label class="bwo-suite-btn" style="display:inline-block">Importar JSON<input type="file" accept="application/json" data-suite-import hidden></label></div><div class="bwo-suite-card"><h3>Estado del sistema</h3><p>Versión <b>${bwoSuiteVersion}</b><br>Peticiones: <b>${bwoSuiteMetrics.requests}</b><br>Errores: <b>${bwoSuiteMetrics.errors}</b><br>Límites: <b>${bwoSuiteMetrics.rateLimits}</b></p></div></div>`}
var bwoSuiteEnsureStylesV19Base=bwoSuiteEnsureStyles;bwoSuiteEnsureStyles=function(){bwoSuiteEnsureStylesV19Base();if(!document.getElementById("bwo-suite-v19-styles")){let e=document.createElement("style");e.id="bwo-suite-v19-styles",e.textContent=`.bwo-suite-fresh{display:flex;gap:10px;align-items:center;flex-wrap:wrap;margin:10px 0;color:#aab4c4;font-size:11px}.bwo-suite-setting{display:block;padding:9px 4px;border-bottom:1px solid #2d333d;cursor:pointer}.bwo-suite-setting input[type=checkbox]{accent-color:#e11d3d;margin-right:8px}.bwo-suite-setting input[type=number]{width:75px;background:#10141a;color:#fff;border:1px solid #3b4350;border-radius:7px;padding:7px}.bwo-rec-explain{margin-top:7px;padding:7px;border:1px solid #323945;border-radius:8px}.bwo-rec-explain summary{cursor:pointer;color:#b9c4d5;font-size:10px;font-weight:800}.bwo-rec-explain div{display:grid;grid-template-columns:1fr 42px;gap:4px;font-size:10px;margin-top:4px}.bwo-suite-scenarios{display:flex;gap:7px;flex-wrap:wrap;margin:8px 0}.bwo-suite-scenarios button.active{background:#7b101f;border-color:#e22642}@media(max-width:720px){.bwo-suite-fresh{align-items:flex-start;flex-direction:column}.bwo-suite-setting{font-size:13px}}`,document.head.append(e);let t=e.cloneNode(!0);t.id="bwo-suite-v19-shadow-styles",zv?.append(t)}};

/* Centro de mercado v1.9: mercado, simulador, inversiones, alertas, historial y ajustes. */
bwoSuiteOpenMarketCenter=async function(e="market"){bwoSuiteEnsureStyles();let t=document.querySelector(".bwo-tools-page-host");if(!t)throw new Error("El contenedor de Herramientas no está disponible");t.className="bwo-tools-page-host bwo-suite-page",t.replaceChildren();let n=document.createElement("div"),r=[["market","Mercado"],["simulator","Simulador"],["portfolio","Inversiones"],["alerts","Alertas"],["alert-settings","Configurar alertas"],["history","Historial"],["settings","Copia y sistema"]];n.innerHTML=bwoSuiteRenderHeader("Centro de mercado","Decide, simula y controla tus operaciones sin abandonar Biwenger.")+`<div class="bwo-suite-tabs">${r.map(([e,t])=>`<button data-suite-tab="${e}">${t}</button>`).join("")}</div><div data-suite-fresh></div><div data-suite-content><div class="bwo-suite-empty">Analizando los datos actuales…</div></div>`,t.append(n);let o=null,a=async(i=!1)=>{let a=n.querySelector("[data-suite-content]");try{if(!["history","settings","alert-settings"].includes(e)||!o)o=await bwoSuiteMarketData(i),o.loadedAt=Date.now();let l=n.querySelector("[data-suite-fresh]");l.replaceChildren(bwoSuiteFreshStrip(o?.loadedAt??Date.now(),"Biwenger · Biwtools"));e==="market"?bwoSuiteRenderMarket(a,o):e==="simulator"?bwoSuiteRenderSimulator(a,o):e==="portfolio"?bwoSuiteRenderPortfolio(a,o):e==="alerts"?bwoSuiteRenderAlerts(a,o):e==="alert-settings"?bwoSuiteRenderAlertSettings(a):e==="history"?bwoSuiteRenderHistory(a):bwoSuiteRenderSettings(a),n.querySelectorAll("[data-suite-tab]").forEach(t=>t.classList.toggle("active",t.dataset.suiteTab===e))}catch(e){a.innerHTML=`<div class="bwo-suite-empty">No se pudieron cargar los datos: ${bwoSuiteEscape(e?.message??e)}</div>`}};n.addEventListener("click",t=>{let r=t.target.closest("[data-suite-tab]");if(r)return e=r.dataset.suiteTab,void a(!1);let i=t.target.closest("[data-suite-track]");if(i&&o){let t=o.scored.find(e=>String(e.id)===i.dataset.suiteTrack);t&&(bwoSuiteTrackPlayer(t,Number(t.price??t.value??0)),e="portfolio",a(!1))}let l=t.target.closest("[data-suite-remove]");l&&o&&(bwoSuiteUntrack(l.dataset.suiteRemove),a(!1));let s=t.target.closest("[data-suite-bid]");if(s&&o){let e=o.scored.find(e=>String(e.id)===s.dataset.suiteBid),t=Number(s.dataset.amount)||Number(e?.price??e?.value??0);e&&(bwoSuiteDecisionAdd("bid",{name:bwoSuitePlayerName(e),detail:`Importe recomendado ${bwoSuiteMoney(t)}`}),bwoRecommendationOpenNativeBid(e,t))}if(t.target.matches("[data-sim-buy]")){let e=t.target.dataset.simBuy;t.target.checked?bwoSuiteSimulation.buys.add(e):bwoSuiteSimulation.buys.delete(e),bwoSuiteRenderSimulator(n.querySelector("[data-suite-content]"),o)}if(t.target.matches("[data-sim-sell]")){let e=t.target.dataset.simSell;t.target.checked?bwoSuiteSimulation.sells.add(e):bwoSuiteSimulation.sells.delete(e),bwoSuiteRenderSimulator(n.querySelector("[data-suite-content]"),o)}if(t.target.closest("[data-alert-save]")){let e={...bwoSuiteAlertPrefs()};n.querySelectorAll("[data-alert-pref]").forEach(t=>e[t.dataset.alertPref]=t.checked),e.participationLimit=Number(n.querySelector("[data-alert-limit]")?.value)||45,bwoSuiteWrite("alert-prefs",e),bwoRecommendationToast("Preferencias de alertas guardadas.","ok")}t.target.closest("[data-suite-export]")&&bwoSuiteExport(),t.target.closest("[data-suite-back]")&&bwoClauseReturnTools(),t.target.closest("[data-suite-refresh]")&&a(!0)}),n.addEventListener("change",e=>{e.target.matches("[data-suite-import]")&&e.target.files?.[0]&&bwoSuiteImport(e.target.files[0])}),await a(!1)};

/* Recomendaciones explicables. */
try{var bwoSuiteExplainRenderBase=bwoRecRenderCard;bwoRecRenderCard=function(e,t){bwoSuiteExplainRenderBase(e,t);let n=bwoRecRows(e,t),r=zv.querySelector(".bwo-rec-upgrade");r&&[...r.querySelectorAll("tbody tr")].forEach((e,t)=>{let r=n[t],o=e.querySelector("td:nth-child(2)");if(!r||!o||o.querySelector(".bwo-rec-explain"))return;let a=bwoSuiteExplainScore(r),i=document.createElement("details");i.className="bwo-rec-explain",i.innerHTML=`<summary>Por qué se recomienda</summary>${a.map(e=>`<div><span>${bwoSuiteEscape(e.label)}</span><b>${e.value}%</b></div>`).join("")}`,o.append(i)})}}catch{}

/* Gestor central: la cola nativa sigue limitando frecuencia y esta capa elimina GET simultáneos duplicados. */
var bwoSuiteInflightGets=new Map,bwoSuiteRequestWindow=[];
try{var bwoSuiteManagedFetchBase=bwoApiFetch;bwoApiFetch=async function(e,t={}){let n={...t},r=n.bwoPriority??((n.method??"GET").toUpperCase()==="GET"?"background":"user");delete n.bwoPriority;let o=Date.now();bwoSuiteRequestWindow=bwoSuiteRequestWindow.filter(e=>o-e<6e4);let a=(n.method??"GET").toUpperCase(),i=String(e);if(a==="GET"&&bwoSuiteInflightGets.has(i)){bwoSuiteMetrics.cacheHits++;let e=await bwoSuiteInflightGets.get(i);return e.clone()}if(bwoSuiteRequestWindow.length>55&&r!=="user")await new Promise(e=>setTimeout(e,Math.min(2500,700*(bwoSuiteRequestWindow.length-54))));bwoSuiteRequestWindow.push(Date.now());let l=Promise.resolve(bwoSuiteManagedFetchBase(e,n));if(a==="GET"){bwoSuiteInflightGets.set(i,l);setTimeout(()=>bwoSuiteInflightGets.delete(i),1800);let e=await l;return e.clone()}return l}}catch{}

/* Tres estrategias para el Once Jornada. */
var bwoSuiteXIScenario="balanced";
try{var bwoSuiteXIRankBase=bwoMatchdayXIPlayerRank;bwoMatchdayXIPlayerRank=function(e){let t=bwoSuiteXIRankBase(e),n=bwoMatchdayXIProbability(e),r=Number(e.projected)||0;return bwoSuiteXIScenario==="safe"?t+n*.13-(bwoMatchdayXIRisk(e)?22:0):bwoSuiteXIScenario==="offensive"?t+r*1.35+n*.025:t}}catch{}
try{var bwoSuiteXIEditorScenarioBase=bwoSuiteXIEditor;bwoSuiteXIEditor=function(e,t){bwoSuiteXIEditorScenarioBase(e,t);let n=e?.querySelector(".bwo-suite-xi-editor");if(!n||n.querySelector(".bwo-suite-scenarios"))return;let r=document.createElement("div");r.className="bwo-suite-scenarios";for(let[e,o,a]of[["safe","Once seguro","Prioriza jugar"],["balanced","Equilibrado","Seguridad y puntos"],["offensive","Ofensivo","Mayor techo"]]){let i=document.createElement("button");i.className=`bwo-suite-btn${bwoSuiteXIScenario===e?" active":""}`,i.textContent=o,i.title=a,i.addEventListener("click",()=>{bwoSuiteXIScenario=e,bwoSuiteXIOverrides.clear(),bwoSuiteXIContext&&bwoMatchdayXIRender(bwoSuiteXIContext.data,bwoSuiteXIContext.host,t.formation)}),r.append(i)}n.insertBefore(r,n.children[2]??null)}}catch{}
try{var bwoSuiteXISaveHistoryBase=bwoMatchdayXISave;bwoMatchdayXISave=async function(e,t){let n=bwoSuiteDecisionAdd("lineup",{name:e.formation,detail:`Escenario ${bwoSuiteXIScenario} · ${Number(e.projected||0).toFixed(1)} puntos previstos`});try{let r=await bwoSuiteXISaveHistoryBase(e,t),o=/guardad/i.test(t?.textContent??"");return bwoSuiteDecisionUpdate(n,{ok:o,detail:`Escenario ${bwoSuiteXIScenario} · ${Number(e.projected||0).toFixed(1)} pts · ${o?"guardado":"no confirmado"}`}),r}catch(e){throw bwoSuiteDecisionUpdate(n,{ok:!1,detail:e?.message??String(e)}),e}}}catch{}

/* Pruebas de interfaz y coherencia sin modificar el estado de la liga. */
try{var bwoSuiteSelfTestsBase=bwoSuiteSelfTests;bwoSuiteSelfTests=async function(e){let t=await bwoSuiteSelfTestsBase(e),n=[];try{let e=bwoBuildToolsPage();n.push({name:"Render de Herramientas",ok:!!e.querySelector(".bwo-tools-card-grid"),message:`${e.querySelectorAll(".bwo-tool-card").length} tarjetas construidas`})}catch(e){n.push({name:"Render de Herramientas",ok:!1,message:e?.message??String(e)})}let r=bwoToolsCatalog.filter(e=>!e.name||!e.category||!Array.isArray(e.features));n.push({name:"Catálogo de módulos",ok:r.length===0,message:r.length?`${r.length} módulos incompletos`:"Estructura completa"});let o=[...document.querySelectorAll("[data-bwo-tools-nav=\"1\"]")];n.push({name:"Navbar sin duplicados",ok:o.length<=1,message:`${o.length} acceso${o.length===1?"":"s"} visible${o.length===1?"":"s"}`});let a=document.documentElement.scrollWidth<=document.documentElement.clientWidth+4;n.push({name:"Desbordamiento horizontal",ok:a,message:a?"Vista ajustada":"Hay contenido más ancho que la pantalla"});if(bwoSuiteXIContext){let e=bwoMatchdayXIPlans(bwoSuiteXIContext.data),t=e[0]&&bwoMatchdayXICompleteSelection(bwoSuiteXIContext.data,e[0]);n.push({name:"Once reglamentario",ok:t?.players?.length===11,message:t?`${t.players.length} titulares · ${t.reserves?.filter(Boolean).length??0} suplentes`:"Sin datos del once"})}return[...t,...n]}}catch{}
try{Object.assign(globalThis.BiwtoolsPro.modules,{simulator:{state:bwoSuiteSimulation},history:{read:bwoSuiteDecisionRead,add:bwoSuiteDecisionAdd},backup:{export:bwoSuiteExport,import:bwoSuiteImport},requests:{inflight:bwoSuiteInflightGets,window:bwoSuiteRequestWindow},lineup:{get scenario(){return bwoSuiteXIScenario},set scenario(e){["safe","balanced","offensive"].includes(e)&&(bwoSuiteXIScenario=e)}}}),globalThis.BiwtoolsPro.modules.diagnostics.run=bwoSuiteSelfTests,globalThis.BiwtoolsPro.version=bwoSuiteVersion}catch{}
/* Frescura visible en el resto de herramientas. */
try{var bwoSuiteFreshFetchBase=bwoApiFetch;bwoApiFetch=async function(...e){let t=await bwoSuiteFreshFetchBase(...e);return t?.ok&&(bwoSuiteMetrics.lastDataAt=Date.now()),t}}catch{}
function bwoSuiteEnhanceFreshness(){let e=bwoSuiteMetrics.lastDataAt??bwoSuiteMetrics.startedAt,t=[...zv.querySelectorAll(".bwo-comp,.bwo-xi-tool"),...document.querySelectorAll(".bwo-clause-tool,.bwo-xi-tool")];for(let n of t){if(n.querySelector(":scope > .bwo-suite-fresh"))continue;let t=bwoSuiteFreshStrip(e,"Biwenger · fuentes Biwtools"),r=n.querySelector(":scope > header,:scope > .bwo-comp-head");r?.insertAdjacentElement("afterend",t)}}
try{var bwoSuiteEnsureFreshBase=bwoEnsureBidsFabItem;bwoEnsureBidsFabItem=function(){let e=bwoSuiteEnsureFreshBase();return queueMicrotask(bwoSuiteEnhanceFreshness),e}}catch{}
try{for(let e of globalThis.__BIWTOOLS_RUNTIME_CLEANUPS__??[])e()}catch(e){console.warn("[biwtools] No se pudo limpiar por completo la instancia anterior:",e)}
globalThis.__BIWTOOLS_RUNTIME_CLEANUPS__=[];
var bwoRuntimeToken=`v1.9.0-${Date.now()}-${Math.random().toString(36).slice(2)}`;
...
var Or=document.createElement("div");Or.id=y1,Or.dataset.bwoRuntime=bwoRuntimeToken;var zv=Or.attachShadow({mode:"open"}),Fv=document.createElement("style");Fv.textContent=Ov;zv.appendChild(Fv);var qv=document.createElement("div");zv.appendChild(qv);document.body.appendChild(Or);var b1=(0,Dv.createRoot)(qv);b1.render((0,A.jsx)(h1,{}));var bwoShadowObserver=new MutationObserver(()=>bwoRuntimeActive()&&bwoEnsureBidsFabItem());bwoShadowObserver.observe(zv,{childList:!0,subtree:!0});Iv();Ef();bwoBindToolsExit();bwoEnsureBidsNav();bwoEnsureBidsFabItem();Bv();bwoRefreshBalances();var bwoRefreshTimer=setInterval(()=>bwoRuntimeActive()&&bwoRefreshBalances(),matchMedia("(max-width: 768px)").matches?20000:15000),bwoVisibilityTimer=setInterval(()=>bwoRuntimeActive()&&rv(),matchMedia("(max-width: 768px)").matches?1200:800),bwoPositionTimer=setInterval(()=>bwoRuntimeActive()&&ic(),matchMedia("(max-width: 768px)").matches?1200:800),bwoScrollHandler=()=>bwoRuntimeActive()&&ic();window.addEventListener("scroll",bwoScrollHandler,{capture:!0,passive:!0});Or.style.pointerEvents="auto";function Hv(){if(!bwoRuntimeActive())return;bwoRemoveLegacyRuntime(),bwoEnsureBidsNav(),bwoEnsureBidsFabItem(),bwoMaintainToolsPage();if(Or.classList.contains("bwo-tools-embedded"))return;let e=document.querySelectorAll(".cdk-overlay-pane"),t=e.length>0?e[e.length-1]:null,n=document.querySelector(".cdk-overlay-container"),r=t??n??document.body;r.lastElementChild!==Or&&r.appendChild(Or)}var bwoBodyObserver=new MutationObserver(Hv);bwoBodyObserver.observe(document.body,{childList:!0,subtree:!0});var bwoHostTimer=setInterval(Hv,matchMedia("(max-width: 768px)").matches?2000:1500);globalThis.__BIWTOOLS_RUNTIME_CLEANUPS__=[()=>clearInterval(bwoRefreshTimer),()=>clearInterval(bwoVisibilityTimer),()=>clearInterval(bwoPositionTimer),()=>clearInterval(bwoHostTimer),()=>bwoShadowObserver.disconnect(),()=>bwoBodyObserver.disconnect(),()=>window.removeEventListener("scroll",bwoScrollHandler,{capture:!0}),()=>b1.unmount()];})();
